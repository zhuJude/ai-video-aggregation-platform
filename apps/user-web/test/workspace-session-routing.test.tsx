...
const workspaceProps = vi.hoisted(() => vi.fn());

vi.mock('../lib/auth/server-session', () => ({
  readAuthenticatedServerSessionState: readState,
}));
vi.mock('next/navigation', () => ({ redirect: redirectTo }));
vi.mock('../components/studio/studio-workspace-entry', () => ({
  StudioWorkspaceEntry: (props: { readonly initialModelId?: string }) => {
    workspaceProps(props);
    return <div>真实工作台</div>;
  },
}));
...
it('renders Studio only for an active session', async () => {
  ...
  render(await StudioPage({ searchParams: Promise.resolve({ model: 'kling-2-1-pro' }) }));
  expect(screen.getByText('真实工作台')).toBeVisible();
  expect(workspaceProps).toHaveBeenCalledWith(
    expect.objectContaining({ initialModelId: 'kling-2-1-pro' }),
  );
...
it('preserves a strict model preselection through refresh and login', async () => {
  const { default: StudioPage } = await import('../app/studio/page');
  const returnTo = '/studio?model=kling-2-1-pro';

  readState.mockResolvedValue({ kind: 'needs-refresh' });
  await expect(
    StudioPage({ searchParams: Promise.resolve({ model: 'kling-2-1-pro' }) }),
  ).rejects.toThrow(`NEXT_REDIRECT:/auth/session/refresh?returnTo=${encodeURIComponent(returnTo)}`);

  readState.mockResolvedValue({ kind: 'invalid' });
  await expect(
    StudioPage({ searchParams: Promise.resolve({ model: 'kling-2-1-pro' }) }),
  ).rejects.toThrow(`NEXT_REDIRECT:/login?returnTo=${encodeURIComponent(returnTo)}`);
});
