...
import { isCoherentPointsAdjustment, isMainlandPhone, isPointsString, isUtcIso8601Z } from './frozen-scalars';
...
async function requireEligibleApprover(port: WalletAdjustmentRequestPort, authorization: Awaited<ReturnType<typeof requireAdminAuthorization>>, requestContext: OutboundRequestContext, userId: string, approverId: string): Promise<void> {
  ...
  const response: unknown = await port.getEligibleApprovers({ dataScope: authorization.claims.dataScope, requestContext, trustedSessionToken: authorization.trustedSessionToken, userId });
  if (!Array.isArray(response) || response.length > 100) throw new Error('复核人无效');
  const approvers = response as readonly unknown[];
  const valid = approvers.every((candidate): candidate is Readonly<{ displayName: string; id: string }> => {
  ...
  if (!valid || !approvers.some((approver) => isSameUuidV7(approver.id, approverId))) throw new Error('复核人无效');
}
...
function requiredTrimmedText(formData: FormData, name: string, maximumLength: number): string {
  ...
  if (!normalized || normalized.length > maximumLength || containsSensitivePhoneLikeValue(normalized)) {
...
function requiredRawString(formData: FormData, name: string, maximumLength: number): string {
  const value = formData.get(name);
  if (typeof value !== 'string' || !value || value.length > maximumLength) throw new Error(`${name}无效`);
  return value;
}
...
function requiredIntent(formData: FormData, name: string, errorMessage: string): string {
  const value = formData.get(name);
  if (typeof value !== 'string' || value.length !== 36 || !isUuidV7(value)) throw new Error(errorMessage);
  return value;
}
...
export function createWalletAdjustmentRequestAction({
  ...
  return async function walletAdjustmentRequestAction(
    ...
    if (!PREVIEW_TOKEN.test(previewToken) || containsSensitivePhoneLikeValue(previewToken)) throw new Error('preview 凭证无效');
    ...
    const resourceScope = await scopePort.getUserScope({ requestContext, trustedSessionToken: authorization.trustedSessionToken, userId });
    assertAdminDataScope(authorization.claims, resourceScope);
    await requireEligibleApprover(adjustmentPort, authorization, requestContext, userId, approverId);
    ...
    if (!isUuidV7(result.requestId) || !isUuidV7(result.auditRecordId)) throw new Error('调整申请响应无效');
    return { auditRecordId: result.auditRecordId, ok: true, requestId: result.requestId, status: result.status };
  };
}

export function createWalletAdjustmentPreviewAction({ adjustmentPort, createCorrelationId: makeCorrelationId = createUuidV7, createTraceId: makeTraceId = createTraceId, guardContext, scopePort }: WalletAdjustmentActionDependencies) {
  return async function walletAdjustmentPreviewAction(formData: FormData): Promise<WalletAdjustmentPreview> {
    ...
    await requireEligibleApprover(adjustmentPort, authorization, requestContext, userId, approverId);
    const preview = await adjustmentPort.previewAdjustment({ approverId, audit: { idempotencyKey: previewIntentId }, direction, points, reason, requestContext, trustedSessionToken: authorization.trustedSessionToken, userId });
    const expiresAt = Date.parse(preview.expiresAt);
    if (!PREVIEW_TOKEN.test(preview.previewToken) || containsSensitivePhoneLikeValue(preview.previewToken) || !isUtcIso8601Z(preview.expiresAt) || expiresAt <= Date.now() || expiresAt > Date.now() + MAX_DESCRIPTOR_MS || !isPointsString(preview.before) || !isPointsString(preview.after) || !isPointsString(preview.points) || preview.points !== pointsText || preview.direction !== direction || !isCoherentPointsAdjustment(preview.before, preview.after, preview.points, preview.direction) || ![preview.impact, preview.policy].every((value) => isPhoneFreeBoundedText(value, 256))) throw new Error('调整预览无效');
    return preview;
  };
}
...
export function createUserCsvExportAction({
  ...
  return async function userCsvExportAction(
    ...
    if (formData.get('exactPhone') !== null || formData.get('phone') !== null || formData.get('searchHandle') !== null || formData.get('searchHandleExpiresAt') !== null) throw new Error('精确手机号模式无效');
    if ([submittedQuery, ...['status', 'tag', 'registrationSource', 'spendingTier'].map((name) => formData.get(name))].some(containsSensitivePhoneLikeValue)) throw new Error('敏感查询参数无效');
    ...
    if (usesSearchDescriptor) {
      ...
      if (submittedQuery !== null || ['status', 'tag', 'registrationSource', 'spendingTier'].some((name) => formData.get(name) !== null)) throw new Error('搜索句柄不可与普通查询条件混用');
      const verified = await verifyExactPhoneSearchDescriptor(submittedSearchDescriptor, { now, scope: authorization.claims.dataScope, sessionInstanceId: authorization.claims.sessionInstanceId, signingKey: descriptorSigningKey, subjectId: authorization.claims.subjectId });
      verifiedSearchHandle = verified.handle;
    }
    const intentId = requiredIntent(formData, 'intentId', '业务意图无效');
    const filters = usesSearchDescriptor ? undefined : validateUserFilters({ status: formData.get('status') ?? undefined, tag: formData.get('tag') ?? undefined, registrationSource: formData.get('registrationSource') ?? undefined, spendingTier: formData.get('spendingTier') ?? undefined });
    const query = typeof submittedQuery === 'string' ? submittedQuery.trim() || undefined : undefined;
    ...
    if (!isUuidV7(result.auditRecordId) || !isPhoneFreeHttpsUrl(result.downloadUrl) || !isUtcIso8601Z(result.expiresAt) || expiresAt <= Date.now() || expiresAt > Date.now() + MAX_DESCRIPTOR_MS) throw new Error('导出下载凭证无效');
    return { auditRecordId: result.auditRecordId, downloadUrl: result.downloadUrl, expiresAt: result.expiresAt, ok: true };
  };
}
