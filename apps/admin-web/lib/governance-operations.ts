...
export interface GovernanceOperationsPort {
  listContent(
    input: BaseRequest & Readonly<{ cursor?: string; status?: string }>,
  ): Promise<unknown>;
  ...
  updateRole(
    input: BaseRequest &
      Readonly<{
        ...
        audit: AuditCommand;
        confirmed: true;
        dataScope: 'ALL' | 'ASSIGNED' | 'OWN' | null;
        expectedVersion: number;
        operation: 'DELETE' | 'UPDATE';
        permissions: readonly string[];
        preflightToken: string;
  ...
  getAuditExportPreview(
    input: BaseRequest & AuditFilters & Readonly<{ format: 'CSV' }>,
  ): Promise<unknown>;
  requestAuditExport(
    input: BaseRequest &
      AuditFilters &
  ...
  executeSystemOperation(
    input: BaseRequest &
      Readonly<{
        ...
        resourceId: string;
        draft?:
          | Readonly<{ enabled: boolean; flagKey: string; rolloutBps: number }>
          | Readonly<{
  ...
  try {
    ...
    if (
      Reflect.ownKeys(value).some(
        (key) => typeof key === 'symbol' || (key !== 'length' && !/^\d+$/u.test(key)),
      )
    )
      return null;
...
const SECRET_TEXT =
  /(?:authorization\s*[:=]\s*(?:bearer|basic)|(?:set-)?cookie\s*[:=]|wechatpay-signature\s*:|api[-_ ]?key\s*[:=]|(?:password|private[-_ ]?key|secret|token|signature)\s*[:=]|[?&#](?:signature|token|secret)=)/iu;
...
export function parsePublicHttpsUrl(value: unknown): string | null {
  ...
  try {
    ...
    if (
      ...
      url.hostname.endsWith('.internal.example')
    )
      return null;
    return url.toString();
  } catch {
...
export function parseTrustedObservabilityOrigins(
  ...
  if (
    parsed.some(
      (candidate, index) =>
        !candidate || candidate !== `${String(source[index]).replace(/\/$/u, '')}/`,
    )
  ) {
...
export function parseRichTextDocument(value: unknown): RichTextDocument | null {
  ...
  return Object.freeze({
    blocks: Object.freeze(
      blocks as RichTextDocument['blocks'] extends readonly (infer T)[] ? T[] : never,
    ),
  });
}
...
function parseContentItem(value: unknown): ContentItem | null {
  ...
  const allowedOperations = allowedSource?.map((operation) =>
    member(operation, CONTENT_OPERATIONS),
  );
  ...
  const planPoints =
    typeof draft?.planPoints === 'string' && POINTS.test(draft.planPoints)
      ? draft.planPoints
      : null;
  ...
  const draftPreviews = draftPreviewsSource?.map((value) => {
    ...
    if (
      ...
      !resultStatus ||
      resultVersion === null ||
      (operation === 'SAVE_DRAFT' ? resultStatus !== 'DRAFT' : resultStatus !== 'DRAFT_VALIDATED')
    )
      return null;
  ...
  const operation =
    previewRecord === null
      ? null
      : member(previewRecord.operation, ['PUBLISH', 'REORDER', 'RETIRE', 'ROLLBACK'] as const);
  const preflightToken =
    previewRecord === null ? null : safeText(previewRecord.preflightToken, 500);
  const renderedDocument =
    previewRecord === null ? null : parseRichTextDocument(previewRecord.renderedDocument);
  const resultStatus =
    previewRecord === null
      ? null
      : member(previewRecord.resultStatus, ['PUBLISHED', 'RETIRED'] as const);
  ...
  if (
    ...
    !title ||
    !draftPreviews ||
    draftPreviews.some((preview) => !preview) ||
    !isUuidV7(item.id) ||
    ...
    !status ||
    version === null ||
    (item.publishedVersion !== null && publishedVersion === null) ||
    !validation ||
    ...
    (item.preview !== null &&
      (!previewRecord ||
        ...
        !resultStatus ||
        resultVersion === null)) ||
    (allowedOperations.filter((candidate) =>
      publicationOperations.includes(candidate as PublicationOperation),
    ).length > 0 &&
      item.preview === null) ||
    allowedOperations
      .filter((operation) => operation === 'SAVE_DRAFT' || operation === 'VALIDATE')
      .some((operation) => !draftPreviews.some((preview) => preview?.operation === operation)) ||
    (operation && !allowedOperations.includes(operation)) ||
    (operation === 'PUBLISH' &&
      (status !== 'DRAFT_VALIDATED' || !validation.valid || resultStatus !== 'PUBLISHED')) ||
    (operation === 'ROLLBACK' && (status !== 'PUBLISHED' || resultStatus !== 'PUBLISHED')) ||
    (operation === 'RETIRE' && (status !== 'PUBLISHED' || resultStatus !== 'RETIRED')) ||
    (operation === 'REORDER' && (status !== 'PUBLISHED' || resultStatus !== 'PUBLISHED'))
  )
    return null;
  return Object.freeze({
    ...
    draftPreviews: Object.freeze(
      draftPreviews as ContentItem['draftPreviews'] extends readonly (infer T)[] ? T[] : never,
    ),
    id: item.id,
    ...
    preview:
      previewRecord === null
        ? null
        : Object.freeze({
            diff: diff as readonly string[],
            expiresAt: expiresAt as string,
            operation: operation as NonNullable<ContentItem['preview']>['operation'],
            preflightToken: preflightToken as string,
            renderedDocument: renderedDocument as RichTextDocument,
            resultStatus: resultStatus as 'PUBLISHED' | 'RETIRED',
...
export function parseContentDirectory(value: unknown): ContentDirectory {
  ...
  if (
    ...
    (nextCursor === null && payload.nextCursor !== null) ||
    !sourceUpdatedAt
  )
    throw new Error('内容响应无效');
  return Object.freeze({
    items: Object.freeze(items as ContentItem[]),
    nextCursor,
    sourceUpdatedAt,
  });
}
...
function parseTicketItem(value: unknown): TicketItem | null {
  ...
  const messages = messagesSource?.map((value) => {
    ...
    const attachments = attachmentsSource?.map((value) => {
      ...
      return attachment && isUuidV7(attachment.fileId) && mimeType && name && sizeBytes !== null
        ? Object.freeze({ fileId: attachment.fileId, mimeType, name, sizeBytes })
        : null;
    });
    return message &&
      authorMasked &&
      ...
      ? Object.freeze({
          attachments: Object.freeze(
            attachments as TicketMessage['attachments'] extends readonly (infer T)[] ? T[] : never,
          ),
          authorMasked,
          authorType,
          body,
  ...
  const transitionPreviews = transitionSource?.map((value) => {
    ...
    return preview && expiresAt && impact && preflightToken && resultVersion !== null && to
      ? Object.freeze({ expiresAt, impact, preflightToken, resultVersion, to })
      : null;
  });
  if (
    !item ||
    ...
    new Set(allowedTransitions).size !== allowedTransitions.length ||
    !assignedAdminIds ||
    !isUuidV7(item.id) ||
    (item.ownerAdminId !== null && !isUuidV7(item.ownerAdminId)) ||
    !status ||
    (item.resolvedAt !== null && !resolvedAt) ||
    (status === 'RESOLVED' || status === 'CLOSED') !== (resolvedAt !== null) ||
    version === null ||
    !messages ||
    messages.some((message) => !message) ||
    !messagePreviews ||
    messagePreviews.some((preview) => !preview) ||
    !transitionPreviews ||
    transitionPreviews.some((preview) => !preview) ||
    ...
    allowedTransitions.some(
      (to) =>
        !canTransitionTicket(
          ...
          resolvedAt,
          messages.some(
            (message) => message?.authorType === 'ADMIN' && message.visibility === 'PUBLIC_REPLY',
          ),
        ),
    )
  )
    return null;
  return Object.freeze({
    allowedTransitions: Object.freeze(allowedTransitions as TicketStatus[]),
    assignedAdminIds,
    id: item.id,
    messagePreviews: Object.freeze(
      messagePreviews as TicketItem['messagePreviews'] extends readonly (infer T)[] ? T[] : never,
    ),
    messages: Object.freeze(messages as TicketMessage[]),
    ownerAdminId: item.ownerAdminId,
    resolvedAt,
    status,
    transitionPreviews: Object.freeze(
      transitionPreviews as TicketItem['transitionPreviews'] extends readonly (infer T)[]
        ? T[]
        : never,
    ),
    version,
...
export function parseTicketDirectory(value: unknown): TicketDirectory {
  ...
  if (
    ...
    throw new Error('工单响应无效');
  return Object.freeze({
    items: Object.freeze(items as TicketItem[]),
    nextCursor,
    sourceUpdatedAt,
  });
}
...
function parsePermissionList(value: unknown): readonly string[] | null {
  ...
  return source &&
    source.every((permission) => /^[a-z][a-z0-9-]*:[a-z][a-z0-9-]*$/u.test(permission))
    ? source
    : null;
}
...
function parseRole(value: unknown): RoleRecord | null {
  ...
  const previewRecord =
    role?.preview === null
      ? null
      : exactRecord(role?.preview, [
          'actorImpacted',
          'added',
          'expiresAt',
          'impactedAdminIdsMasked',
          'operation',
          'preflightToken',
          'proposedDataScope',
          'proposedPermissions',
  ...
  const expiresAt = previewRecord === null ? null : utc(previewRecord.expiresAt);
  const impacted =
    previewRecord === null ? null : stringList(previewRecord.impactedAdminIdsMasked, 200, 120);
  const operation =
    previewRecord === null ? null : member(previewRecord.operation, ['DELETE', 'UPDATE'] as const);
  const preflightToken =
    previewRecord === null ? null : safeText(previewRecord.preflightToken, 500);
  const proposedDataScope =
    previewRecord === null
      ? null
      : previewRecord.proposedDataScope === null
        ? null
        : member(previewRecord.proposedDataScope, DATA_SCOPES);
  const proposedPermissions =
    previewRecord === null ? null : parsePermissionList(previewRecord.proposedPermissions);
  ...
  if (
    ...
    !dataScope ||
    !isUuidV7(role.id) ||
    typeof role.isSuperAdmin !== 'boolean' ||
    !name ||
    (role.ownerAdminId !== null && !isUuidV7(role.ownerAdminId)) ||
    !permissions ||
    version === null ||
    (role.preview !== null &&
      (!previewRecord ||
        typeof previewRecord.actorImpacted !== 'boolean' ||
        !added ||
        !expiresAt ||
        !impacted ||
        !operation ||
        ...
        (operation === 'DELETE' && (proposedPermissions.length > 0 || added.length > 0))))
  )
    return null;
  return Object.freeze({
    adminCount,
    ...
    name,
    ownerAdminId: role.ownerAdminId,
    permissions,
    preview:
      previewRecord === null
        ? null
        : Object.freeze({
            actorImpacted: previewRecord.actorImpacted as boolean,
            added: added as readonly string[],
            expiresAt: expiresAt as string,
            impactedAdminIdsMasked: impacted as readonly string[],
            operation: operation as 'DELETE' | 'UPDATE',
            preflightToken: preflightToken as string,
            proposedDataScope,
            proposedPermissions: proposedPermissions as readonly string[],
            removed: removed as readonly string[],
            resultVersion: resultVersion as number,
          }),
    version,
  });
...
function parseAdminAccount(value: unknown): AdminAccountRecord | null {
  ...
  const operation =
    previewRecord === null
      ? null
      : member(previewRecord.operation, [
          'UPDATE_ASSIGNMENTS',
          'UPDATE_SCOPE',
          'UPDATE_STATUS',
        ] as const);
  const preflightToken =
    previewRecord === null ? null : safeText(previewRecord.preflightToken, 500);
  const proposedDataScope =
    previewRecord === null ? null : member(previewRecord.proposedDataScope, DATA_SCOPES);
  const proposedRoleIds = previewRecord === null ? null : uuidList(previewRecord.proposedRoleIds);
  const proposedStatus =
    previewRecord === null
      ? null
      : member(previewRecord.proposedStatus, ['ACTIVE', 'DISABLED'] as const);
  const resultVersion = previewRecord === null ? null : integer(previewRecord.resultVersion);
  if (
    !admin ||
    !dataScope ||
    ...
    (mfa.lastVerifiedAt !== null && !lastVerifiedAt) ||
    !roleIds ||
    !status ||
    version === null ||
    (admin.preview !== null &&
      (!previewRecord ||
        ...
        !proposedRoleIds ||
        !proposedStatus ||
        typeof previewRecord.removesLastSuperAdmin !== 'boolean' ||
        resultVersion === null))
  )
    return null;
  return Object.freeze({
    ...
    displayNameMasked,
    id: admin.id,
    mfa: Object.freeze({ enabled: mfa.enabled, lastVerifiedAt }),
    preview:
      previewRecord === null
        ? null
        : Object.freeze({
            actorImpacted: previewRecord.actorImpacted as boolean,
            expiresAt: expiresAt as string,
            operation: operation as NonNullable<AdminAccountRecord['preview']>['operation'],
            preflightToken: preflightToken as string,
            proposedDataScope: proposedDataScope as NonNullable<
              AdminAccountRecord['preview']
            >['proposedDataScope'],
            proposedRoleIds: proposedRoleIds as readonly string[],
            proposedStatus: proposedStatus as 'ACTIVE' | 'DISABLED',
            removesLastSuperAdmin: previewRecord.removesLastSuperAdmin as boolean,
            resultVersion: resultVersion as number,
          }),
    roleIds,
    status,
...
export function parseIamDirectory(value: unknown): IamDirectory {
  ...
  if (
    !payload ||
    !isUuidV7(payload.actorAdminId) ||
    !admins ||
    admins.some((admin) => !admin) ||
    !grantablePermissions ||
    !roles ||
  ...
  const hasDanglingAssignment = parsedAdmins.some((admin) =>
    admin.roleIds.some((roleId) => !roleById.has(roleId.toLowerCase())),
  );
  const hasInconsistentRoleCounts = parsedRoles.some((role) => {
    const assignedFromAdmins = parsedAdmins
      .filter((admin) =>
        admin.roleIds.some((roleId) => roleId.toLowerCase() === role.id.toLowerCase()),
      )
      .map((admin) => admin.id.toLowerCase());
    const declared = role.assignedAdminIds.map((adminId) => adminId.toLowerCase());
    return (
      role.adminCount !== assignedFromAdmins.length ||
      declared.length !== assignedFromAdmins.length ||
      declared.some((adminId) => !assignedFromAdmins.includes(adminId))
    );
  });
  const derivedSuperAdminCount = parsedAdmins.filter(
    (admin) =>
      admin.status === 'ACTIVE' &&
      admin.roleIds.some((roleId) => roleById.get(roleId.toLowerCase())?.isSuperAdmin),
  ).length;
  if (
    hasDanglingAssignment ||
    hasInconsistentRoleCounts ||
    !parsedAdmins.some(
      (admin) => admin.id.toLowerCase() === String(payload.actorAdminId).toLowerCase(),
    ) ||
    derivedSuperAdminCount !== superAdminCount
  )
    throw new Error('权限响应无效');
  return Object.freeze({
    actorAdminId: payload.actorAdminId,
    admins: Object.freeze(admins as AdminAccountRecord[]),
    grantablePermissions,
    roles: Object.freeze(roles as RoleRecord[]),
    sourceUpdatedAt,
    superAdminCount,
  });
}
...
export function parseAuditDirectory(value: unknown): AuditDirectory {
  ...
  const exportRecord =
    payload?.exportPreview === null
      ? null
      : exactRecord(payload?.exportPreview, [
          'expiresAt',
          'filterFingerprint',
          'filters',
          'preflightToken',
  ...
  const exportFiltersRecord =
    exportRecord === null
      ? null
      : exactRecord(exportRecord.filters, ['action', 'actor', 'from', 'resource', 'to', 'traceId']);
  const exportFilters =
    exportFiltersRecord === null
      ? null
      : (Object.fromEntries(
          Object.entries(exportFiltersRecord).map(([key, value]) => [
            key,
            value === null ? null : safeText(value, key === 'traceId' ? 32 : 160),
          ]),
  ...
  const exportFingerprint =
    exportRecord === null ? null : safeText(exportRecord.filterFingerprint, 200);
  ...
  const items = source?.map((value) => {
    ...
    return item &&
      action &&
      actorIdMasked &&
      (item.afterSummary === null || afterSummary) &&
      at &&
      (item.beforeSummary === null || beforeSummary) &&
      isUuidV7(item.id) &&
      ipMasked &&
      reason &&
      resourceIdMasked &&
    ...
    (payload.exportPreview !== null &&
      (!exportRecord ||
        ...
        Object.values(exportFilters).some(
          (value, index) =>
            value === null &&
            Object.values(exportFiltersRecord as Record<string, unknown>)[index] !== null,
        ) ||
        !exportExpiresAt ||
        !exportFingerprint ||
        !exportToken ||
        exportRecord.resultStatus !== 'QUEUED'))
  )
    throw new Error('审计响应无效');
  return Object.freeze({
    exportPreview:
      exportRecord === null
        ? null
        : Object.freeze({
            expiresAt: exportExpiresAt as string,
            filterFingerprint: exportFingerprint as string,
            filters: Object.freeze(exportFilters as Record<keyof AuditFilters, string | null>),
            preflightToken: exportToken as string,
            resultStatus: 'QUEUED' as const,
          }),
    items: Object.freeze(
      items as AuditDirectory['items'] extends readonly (infer T)[] ? T[] : never,
    ),
    nextCursor,
    sourceUpdatedAt,
  });
...
function parseFlagList(value: unknown) {
  ...
  const items = source?.map((candidate) => {
    ...
    return flag &&
      ...
      key &&
      /^[a-z][a-z0-9._-]*$/u.test(key) &&
      rolloutBps !== null
      ? Object.freeze({ enabled: flag.enabled, key, rolloutBps })
      : null;
  });
  return items && !items.some((item) => !item)
    ? Object.freeze(items as { enabled: boolean; key: string; rolloutBps: number }[])
    : null;
}
...
function parseConfigHistory(value: unknown) {
  ...
  return items && !items.some((item) => !item)
    ? Object.freeze(items as { publishedAt: string; version: number }[])
    : null;
}
...
function parseValidation(value: unknown) {
  ...
  return validation &&
    ...
    ? Object.freeze({ errors, valid: validation.valid })
    : null;
}
...
function parsePublicSettings(value: unknown) {
  ...
  return settings && publicCallbackUrl && publicDomain
    ? Object.freeze({ publicCallbackUrl, publicDomain })
    : null;
}

export function parseSystemSnapshot(
  value: unknown,
  trustedOrigins = parseTrustedObservabilityOrigins(
    process.env.ADMIN_OBSERVABILITY_ALLOWED_ORIGINS,
  ),
): SystemSnapshot {
  const payload = exactRecord(value, [
    'alerts',
  ...
  const alerts = alertsSource?.map((value) => {
    ...
    return item &&
      ...
      severity &&
      summary
      ? Object.freeze({ closeCondition, id: item.id, ownerMasked, runbookUrl, severity, summary })
      : null;
  });
  ...
  const services = servicesSource?.map((value) => {
    ...
    return item && latencyMs !== null && name && status
      ? Object.freeze({ latencyMs, name, status })
      : null;
  });
  ...
  const releases = releasesSource?.map((value) => {
    ...
    return item &&
      ...
      service &&
      version
      ? Object.freeze({ deployedAt, digest: item.digest, environment, service, version })
      : null;
  });
  ...
  const queues = queueSource?.map((value) => {
    ...
    const currentOutcome =
      previewRecord === null ? null : safeText(previewRecord.currentOutcome, 500);
    const expiresAt = previewRecord === null ? null : utc(previewRecord.expiresAt);
    ...
    return item &&
      ...
      (item.preview === null ||
        (previewRecord &&
          typeof previewRecord.billingSafe === 'boolean' &&
          businessKey &&
          currentOutcome &&
          expiresAt &&
          impact &&
          typeof previewRecord.idempotencySafe === 'boolean' &&
          preflightToken &&
          typeof previewRecord.purchaseSafe === 'boolean' &&
          resultVersion !== null))
      ? Object.freeze({
          depth,
          dlq,
          ...
          preview:
            previewRecord === null
              ? null
              : Object.freeze({
                  billingSafe: previewRecord.billingSafe as boolean,
                  businessKey: businessKey as string,
                  currentOutcome: currentOutcome as string,
                  expiresAt: expiresAt as string,
                  impact: impact as string,
                  idempotencySafe: previewRecord.idempotencySafe as boolean,
                  preflightToken: preflightToken as string,
                  purchaseSafe: previewRecord.purchaseSafe as boolean,
                  resultVersion: resultVersion as number,
                }),
          version,
        })
  ...
  const config = exactRecord(payload?.config, [
    'allowedOperations',
    'featureFlags',
    'flagsVersion',
    'preview',
  ...
  const flagRecord = exactRecord(config?.featureFlags, [
    'current',
    'diff',
    'draft',
    'history',
  ...
  const settingsRecord = exactRecord(config?.publicSettings, [
    'current',
    'diff',
    'draft',
    'history',
  ...
  const secretReferences = secretSource?.map((candidate) => {
    ...
    const name = safeText(item?.name, 100);
    return item &&
      typeof item.kmsReference === 'string' &&
      KMS_REFERENCE.test(item.kmsReference) &&
      masked &&
      masked.includes('***') &&
      name
      ? Object.freeze({ kmsReference: item.kmsReference, masked, name })
      : null;
  });
  const previewRecord =
    config?.preview === null
      ? null
      : exactRecord(config?.preview, [
          'expiresAt',
          'impact',
          'operation',
          'preflightToken',
  ...
  const configOperation =
    previewRecord === null
      ? null
      : member(
          previewRecord.operation,
          SYSTEM_OPERATIONS.filter((item) => item !== 'REDRIVE_DLQ'),
        );
  ...
  const invalidFlagPublication =
    allowedOperations?.includes('PUBLISH_FLAG') && flagValidation?.valid !== true;
  const invalidSettingPublication =
    allowedOperations?.includes('PUBLISH_SETTING') && settingsValidation?.valid !== true;
  const missingRedriveCapability =
    queues?.some((queue) => queue?.preview !== null) && !allowedOperations?.includes('REDRIVE_DLQ');
  if (
    !payload ||
    !sourceUpdatedAt ||
    !alerts ||
    alerts.some((item) => !item) ||
    !services ||
    services.some((item) => !item) ||
    !freshness ||
    freshness.some((item) => !item) ||
    !releases ||
    releases.some((item) => !item) ||
    !links ||
    links.some((item) => !item) ||
    !queues ||
    queues.some((item) => !item) ||
    !config ||
    !allowedOperations ||
    allowedOperations.some((operation) => !operation) ||
    new Set(allowedOperations).size !== allowedOperations.length ||
    ...
    !settingsValidation ||
    !secretReferences ||
    secretReferences.some((item) => !item) ||
    invalidFlagPublication ||
    invalidSettingPublication ||
    missingRedriveCapability ||
    (config.preview !== null &&
      (!previewRecord ||
        !configExpiresAt ||
        !configImpact ||
        !configOperation ||
        !configToken ||
        configResultVersion === null)) ||
    (allowedOperations.filter((operation) => operation !== 'REDRIVE_DLQ').length > 0 &&
      config.preview === null)
  )
    throw new Error('系统运行响应无效');
  return Object.freeze({
    alerts: Object.freeze(
      alerts as SystemSnapshot['alerts'] extends readonly (infer T)[] ? T[] : never,
    ),
    config: Object.freeze({
      allowedOperations: Object.freeze(allowedOperations as SystemOperation[]),
      ...
      preview:
        previewRecord === null
          ? null
          : Object.freeze({
              expiresAt: configExpiresAt as string,
              impact: configImpact as string,
              operation: configOperation as Exclude<SystemOperation, 'REDRIVE_DLQ'>,
              preflightToken: configToken as string,
              resultVersion: configResultVersion as number,
            }),
      publicSettings: Object.freeze({
        ...
        secretReferences: Object.freeze(
          secretReferences as NonNullable<
            SystemSnapshot['config']['publicSettings']['secretReferences']
    ...
    freshness: Object.freeze(
      freshness as SystemSnapshot['freshness'] extends readonly (infer T)[] ? T[] : never,
    ),
    links: Object.freeze(
      links as SystemSnapshot['links'] extends readonly (infer T)[] ? T[] : never,
    ),
    queues: Object.freeze(
      queues as SystemSnapshot['queues'] extends readonly (infer T)[] ? T[] : never,
    ),
    releases: Object.freeze(
      releases as SystemSnapshot['releases'] extends readonly (infer T)[] ? T[] : never,
    ),
    services: Object.freeze(
      services as SystemSnapshot['services'] extends readonly (infer T)[] ? T[] : never,
    ),
    sourceUpdatedAt,
  });
}
...
function parseReceipt(
  ...
  const keys = [
    ...
    ...(expected.version === undefined ? [] : ['version']),
  ];
  ...
  if (
    ...
    receipt[expected.resourceKey] !== expected.resourceId ||
    ...
    (expected.version !== undefined && receipt.version !== expected.version)
  )
...
export async function loadContentDirectory(
  ...
  const view = parseContentDirectory(
    await input.port.listContent({
      ...base(authorization),
      ...(cursor ? { cursor } : {}),
      ...(status ? { status } : {}),
    }),
  );
  if (authorization.claims.dataScope !== 'ALL')
    for (const item of view.items) assertAdminDataScope(authorization.claims, item);
  return view;
}

export async function loadTicketDirectory(
  input: Readonly<{
    context?: ServerGuardContext;
    cursor?: string;
    port: GovernanceOperationsPort;
  ...
  const status = optionalFilter(input.status, 40);
  const view = parseTicketDirectory(
    await input.port.listTickets({
      ...base(authorization),
      ...(cursor ? { cursor } : {}),
      ...(query ? { query } : {}),
      ...(status ? { status } : {}),
...
export async function loadIamDirectory(
  input: Readonly<{ context?: ServerGuardContext; port: GovernanceOperationsPort }>,
): Promise<IamDirectory> {
  ...
  if (directory.actorAdminId.toLowerCase() !== authorization.claims.subjectId.toLowerCase())
    throw new Error('权限权威上下文不一致');
  return directory;
}

export async function loadAuditDirectory(
  input: Readonly<{
    action?: string;
    actor?: string;
    context?: ServerGuardContext;
  ...
  const cursor = optionalFilter(input.cursor, 500);
  return parseAuditDirectory(
    await input.port.listAudit({
      ...base(authorization),
      ...filters,
      ...(cursor ? { cursor } : {}),
    }),
...
export async function loadSystemSnapshot(
  input: Readonly<{
    context?: ServerGuardContext;
    port: GovernanceOperationsPort;
    trustedObservabilityOrigins?: readonly string[];
  }>,
  ...
  return parseSystemSnapshot(
    await input.port.getSystemSnapshot(base(authorization)),
    input.trustedObservabilityOrigins,
  );
}
...
function authoritativeDraftPreview(
  item: ContentItem,
  operation: 'SAVE_DRAFT' | 'VALIDATE',
  command: Readonly<{
    expectedVersion: number;
  ...
  if (
    ...
    !item.allowedOperations.includes(operation) ||
    !preview ||
    preview.preflightToken !== command.preflightToken ||
    Date.parse(preview.expiresAt) <= Date.now()
  )
    throw new Error('内容草稿预检已失效');
  return preview;
}

export function createContentDraftAction(
  ...
  return async (form: FormData) => {
    ...
    const body = bodyText
      ? Object.freeze({
          blocks: Object.freeze([Object.freeze({ text: bodyText, type: 'PARAGRAPH' as const })]),
        })
      : null;
    const rawPoints = form.get('planPoints');
    const planPoints = typeof rawPoints === 'string' && POINTS.test(rawPoints) ? rawPoints : null;
    if (!contentId || !isUuidV7(contentId) || !title || !body || !planPoints)
      throw new Error('内容草稿字段无效');
    const requestBase = base(authorization);
    const item = parseContentItem(
      await dependencies.port.getContent({ ...requestBase, contentId }),
    );
    ...
    const result = await dependencies.port.saveContentDraft({
      ...requestBase,
      actorId: authorization.claims.subjectId,
      audit: { idempotencyKey: command.intentId, reason: command.reason },
      body,
      confirmed: true,
      contentId,
    ...
    return parseReceipt(result, {
      idempotencyKey: command.intentId,
...
export function createContentValidationAction(
  dependencies: Readonly<{ context?: ServerGuardContext; port: GovernanceOperationsPort }>,
) {
  return async (form: FormData) => {
    const command = commandFields(
      form,
      ['confirmed', 'contentId', 'expectedVersion', 'intentId', 'preflightToken', 'reason'],
      '内容校验',
    );
    ...
    const result = await dependencies.port.validateContentDraft({
      ...requestBase,
      actorId: authorization.claims.subjectId,
      audit: { idempotencyKey: command.intentId, reason: command.reason },
      confirmed: true,
...
export function createContentPublicationAction(
  dependencies: Readonly<{ context?: ServerGuardContext; port: GovernanceOperationsPort }>,
) {
  return async (form: FormData) => {
    ...
    if (
      ...
      Date.parse(preview.expiresAt) <= Date.now() ||
      (operation === 'PUBLISH' &&
        (item.status !== 'DRAFT_VALIDATED' ||
          !item.validation.valid ||
          preview.resultStatus !== 'PUBLISHED'))
    )
      throw new Error('内容发布预检已失效');
    const result = await dependencies.port.executeContentOperation({
      ...requestBase,
      actorId: authorization.claims.subjectId,
      audit: { idempotencyKey: command.intentId, reason: command.reason },
      confirmed: true,
...
const ticketMessagePermission: Readonly<Record<TicketVisibility, string>> = Object.freeze({
  INTERNAL_NOTE: 'tickets:internal-note',
  PUBLIC_REPLY: 'tickets:public-reply',
});

export function createTicketMessageAction(
  dependencies: Readonly<{ context?: ServerGuardContext; port: GovernanceOperationsPort }>,
) {
  return async (form: FormData) => {
    ...
    const authorization = await requireAdminAuthorization(
      ticketMessagePermission[visibility],
      dependencies.context,
    );
    ...
    if (
      ...
      !isUuidV7(ticketId) ||
      !body ||
      !attachmentFileIds.every(isUuidV7) ||
      new Set(attachmentFileIds).size !== attachmentFileIds.length
    )
      throw new Error('工单消息字段无效');
    const requestBase = base(authorization);
    ...
    if (
      ...
      throw new Error('工单消息预检已失效');
    const mutation = {
      ...requestBase,
      actorId: authorization.claims.subjectId,
      attachmentFileIds: Object.freeze(attachmentFileIds),
      audit: {
        idempotencyKey: command.intentId,
    ...
    const receipt = exactRecord(result, [
      'auditRecordId',
      'idempotencyKey',
      'messageVisibility',
      'ok',
    ...
    if (
      ...
      receipt.idempotencyKey !== command.intentId ||
      receipt.messageVisibility !== visibility ||
      receipt.version !== preview.resultVersion ||
      !isUuidV7(receipt.auditRecordId) ||
      !isUuidV7(receipt.requestId)
    )
      throw new Error('运营操作回执无效');
...
export function createTicketTransitionAction(
  dependencies: Readonly<{ context?: ServerGuardContext; port: GovernanceOperationsPort }>,
) {
  return async (form: FormData) => {
    const command = commandFields(
    ...
    if (!ticketId || !isUuidV7(ticketId) || !expectedStatus || !to)
      throw new Error('工单状态字段无效');
    const requestBase = base(authorization);
    const item = parseTicketItem(await dependencies.port.getTicket({ ...requestBase, ticketId }));
    if (!item) throw new Error('工单详情响应无效');
    assertAdminDataScope(authorization.claims, item);
    ...
    if (
      ...
      item.version !== command.expectedVersion ||
      !item.allowedTransitions.includes(to) ||
      !canTransitionTicket(
        item.status,
        to,
        item.resolvedAt,
      ...
      throw new Error('工单状态预检已失效');
    const result = await dependencies.port.transitionTicket({
      ...requestBase,
      actorId: authorization.claims.subjectId,
      audit: { idempotencyKey: command.intentId, reason: command.reason },
      confirmed: true,
...
export function createRoleUpdateAction(
  dependencies: Readonly<{ context?: ServerGuardContext; port: GovernanceOperationsPort }>,
) {
  return async (form: FormData) => {
    ...
    const authorization = await requireAdminAuthorization(
      operation === 'DELETE' ? 'iam:role-delete' : 'iam:role-write',
      dependencies.context,
    );
    if (authorization.claims.dataScope !== 'ALL') throw new Error('权限管理仅允许全部数据范围');
    ...
    if (!roleId || !isUuidV7(roleId) || !permissions || (operation === 'UPDATE' && !dataScope))
      throw new Error('角色变更字段无效');
    const requestBase = base(authorization);
    ...
    if (
      !role ||
      role.version !== command.expectedVersion ||
      !role.preview ||
      role.preview.operation !== operation ||
      role.preview.preflightToken !== command.preflightToken ||
      Date.parse(role.preview.expiresAt) <= Date.now()
    )
      throw new Error('角色变更预检已失效');
    if (
      operation === 'UPDATE' &&
      permissions.some(
        (permission) =>
          !directory.grantablePermissions.includes(permission) ||
          !hasPermission(authorization.claims, permission),
      )
    )
      throw new Error('不可授予未持有权限');
    if (
      permissions.length !== role.preview.proposedPermissions.length ||
      permissions.some(
        (permission, index) => permission !== role.preview?.proposedPermissions[index],
      ) ||
      dataScope !== role.preview.proposedDataScope
    )
      throw new Error('角色变更预检已失效');
    const scopeRank = { ALL: 3, ASSIGNED: 2, OWN: 1 } as const;
    const actorImpacted = role.assignedAdminIds.some(
      (adminId) => adminId.toLowerCase() === authorization.claims.subjectId.toLowerCase(),
    );
    if (
      actorImpacted &&
      (role.preview.added.length > 0 ||
        (dataScope && scopeRank[dataScope] > scopeRank[role.dataScope]))
    )
      throw new Error('不可通过当前角色为自己提权');
    if (operation === 'DELETE' && role.isSuperAdmin && directory.superAdminCount === 1)
      throw new Error('不可删除最后一名超级管理员');
    const result = await dependencies.port.updateRole({
      ...requestBase,
      actorId: authorization.claims.subjectId,
      audit: { idempotencyKey: command.intentId, reason: command.reason },
      confirmed: true,
    ...
    return parseReceipt(result, {
      ...
      resourceKey: 'roleId',
      operation,
      version: role.preview.resultVersion,
    });
  };
}

export function createAdminUpdateAction(
  dependencies: Readonly<{ context?: ServerGuardContext; port: GovernanceOperationsPort }>,
) {
  return async (form: FormData) => {
    const operation = member(form.get('operation'), [
      'UPDATE_ASSIGNMENTS',
      'UPDATE_SCOPE',
      'UPDATE_STATUS',
    ] as const);
    ...
    if (!adminId || !isUuidV7(adminId) || !dataScope || !roleIds || !status)
      throw new Error('管理员变更字段无效');
    const requestBase = base(authorization);
    const directory = parseIamDirectory(await dependencies.port.getIamDirectory(requestBase));
    if (directory.actorAdminId.toLowerCase() !== authorization.claims.subjectId.toLowerCase())
      throw new Error('权限权威上下文不一致');
    ...
    if (
      !admin ||
      admin.version !== command.expectedVersion ||
      !preview ||
      preview.operation !== operation ||
      preview.preflightToken !== command.preflightToken ||
      Date.parse(preview.expiresAt) <= Date.now() ||
      dataScope !== preview.proposedDataScope ||
      status !== preview.proposedStatus ||
      roleIds.length !== preview.proposedRoleIds.length ||
      roleIds.some((id, index) => id !== preview.proposedRoleIds[index])
    )
      throw new Error('管理员变更预检已失效');
    const targetRoles = roleIds.map((roleId) =>
      directory.roles.find((role) => role.id.toLowerCase() === roleId.toLowerCase()),
    );
    ...
    if (
      ...
      throw new Error('不可分配未持有权限的角色');
    const currentRoles = admin.roleIds.map((roleId) =>
      directory.roles.find((role) => role.id.toLowerCase() === roleId.toLowerCase()),
    );
    ...
    if (
      actorImpacted &&
      status === 'ACTIVE' &&
      ([...targetPermissions].some((permission) => !currentPermissions.has(permission)) ||
        ({ ALL: 3, ASSIGNED: 2, OWN: 1 } as const)[dataScope] >
          ({ ALL: 3, ASSIGNED: 2, OWN: 1 } as const)[admin.dataScope])
    ) {
      throw new Error('不可为自己提权');
    }
    const currentIsSuperAdmin =
      admin.status === 'ACTIVE' && currentRoles.some((role) => role?.isSuperAdmin);
    const targetIsSuperAdmin =
      status === 'ACTIVE' && targetRoles.some((role) => role?.isSuperAdmin);
    if (currentIsSuperAdmin && !targetIsSuperAdmin && directory.superAdminCount === 1)
      throw new Error('不可移除最后一名超级管理员');
    const result = await dependencies.port.updateAdmin({
      ...requestBase,
      actorId: authorization.claims.subjectId,
      adminId,
      audit: { idempotencyKey: command.intentId, reason: command.reason },
...
function parseAuditFilters(source: AuditFilters): AuditFilters {
  ...
  const from =
    source.from === undefined || source.from === '' ? undefined : (utc(source.from) ?? undefined);
  const to =
    source.to === undefined || source.to === '' ? undefined : (utc(source.to) ?? undefined);
  if (
    (source.traceId && (!traceId || !isTraceId(traceId))) ||
    (source.from && !from) ||
    (source.to && !to) ||
    (from && to && Date.parse(from) > Date.parse(to))
  )
    throw new Error('审计筛选条件无效');
  return {
    ...(action ? { action } : {}),
    ...(actor ? { actor } : {}),
    ...(from ? { from } : {}),
...
function parseAuditExportPreview(value: unknown) {
  ...
  if (
    ...
    !filterFingerprint ||
    !preflightToken ||
    preview.resultStatus !== 'QUEUED'
  )
    throw new Error('审计导出预检响应无效');
  return Object.freeze({
    expiresAt,
    filterFingerprint,
    preflightToken,
...
export function createAuditExportAction(
  dependencies: Readonly<{ context?: ServerGuardContext; port: GovernanceOperationsPort }>,
) {
  return async (form: FormData) => {
    if (
    ...
    const requestBase = base(authorization);
    const preview = parseAuditExportPreview(
      await dependencies.port.getAuditExportPreview({ ...requestBase, ...filters, format: 'CSV' }),
    );
    if (
      preview.preflightToken !== preflightToken ||
      preview.filterFingerprint !== filterFingerprint ||
      Date.parse(preview.expiresAt) <= Date.now()
    )
      throw new Error('审计导出预检已失效');
    const result = await dependencies.port.requestAuditExport({
      ...requestBase,
      ...filters,
      actorId: authorization.claims.subjectId,
    ...
    if (
      ...
      receipt.ok !== true ||
      !isUuidV7(receipt.auditRecordId) ||
      !isUuidV7(receipt.exportJobId) ||
      !isUuidV7(receipt.requestId) ||
      receipt.idempotencyKey !== intentId ||
      receipt.filterFingerprint !== filterFingerprint ||
      receipt.status !== preview.resultStatus
    )
      throw new Error('运营操作回执无效');
    return Object.freeze({ ...receipt });
  };
...
export function createSystemMutationAction(
  ...
  return async (form: FormData) => {
    ...
    const allowedFields = [
      ...
      ...(operation === 'SAVE_SETTING_DRAFT'
        ? ['publicCallbackUrl', 'publicDomain', 'replacementSecret']
        : []),
    ];
    ...
    const snapshot = parseSystemSnapshot(
      await dependencies.port.getSystemSnapshot(requestBase),
      dependencies.trustedObservabilityOrigins,
    );
    let preview: {
      billingSafe?: boolean;
      businessKey?: string;
    ...
    if (operation === 'REDRIVE_DLQ') {
      ...
      if (
        !snapshot.config.allowedOperations.includes('REDRIVE_DLQ') ||
        !queue ||
        queue.version !== command.expectedVersion
      )
        throw new Error('系统操作预检已失效');
      preview = queue.preview;
      if (!preview?.idempotencySafe || !preview.billingSafe || !preview.purchaseSafe)
        throw new Error('DLQ 重放安全条件不满足');
    } else {
      if (
        !snapshot.config.allowedOperations.includes(operation) ||
        snapshot.config.preview?.operation !== operation ||
        ...
        (operation.includes('FLAG')
          ? snapshot.config.flagsVersion
          : snapshot.config.settingsVersion) !== command.expectedVersion
      )
        throw new Error('系统操作预检已失效');
    ...
    if (
      ...
      throw new Error('系统操作预检已失效');
    let draft:
      | NonNullable<Parameters<GovernanceOperationsPort['executeSystemOperation']>[0]['draft']>
      | undefined;
    if (operation === 'SAVE_FLAG_DRAFT') {
      ...
      const rolloutBps =
        rolloutRaw && /^\d{1,5}$/u.test(rolloutRaw) ? Number(rolloutRaw) : Number.NaN;
      if (
        !flagKey ||
        !/^[a-z][a-z0-9._-]*$/u.test(flagKey) ||
        !Number.isSafeInteger(rolloutBps) ||
        rolloutBps > 10_000 ||
        typeof enabled !== 'string' ||
        !['true', 'false'].includes(enabled)
      )
        throw new Error('系统操作字段无效');
      ...
      if (
        ...
        rawSecret.length < 16 ||
        rawSecret.length > 1_024
      )
        throw new Error('系统操作字段无效');
      draft = { publicCallbackUrl, publicDomain, replacementSecret: rawSecret };
    }
    const result = await dependencies.port.executeSystemOperation({
      ...requestBase,
      actorId: authorization.claims.subjectId,
      audit: { idempotencyKey: command.intentId, reason: command.reason },
      confirmed: true,
    ...
    if (operation === 'REDRIVE_DLQ') {
      ...
      const bound = exactRecord(result, [
        'auditRecordId',
        'businessKey',
        'currentOutcome',
        'idempotencyKey',
      ...
      if (
        !bound ||
        bound.ok !== true ||
        !isUuidV7(bound.auditRecordId) ||
        !isUuidV7(bound.requestId) ||
        bound.idempotencyKey !== command.intentId ||
        bound.operation !== operation ||
        bound.resourceId !== resourceId ||
        bound.version !== preview.resultVersion ||
        bound.businessKey !== queuePreview.businessKey ||
        bound.currentOutcome !== queuePreview.currentOutcome
      )
        throw new Error('运营操作回执无效');
