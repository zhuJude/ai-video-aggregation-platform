...
import { isValidSearchHandle, isValidShortLivedSearchExpiry, validateUserFilters, type ExactPhoneLookupPort, type UserFilters } from './user-view-loaders';
import {
...
import { isCoherentPointsAdjustment, isMainlandPhone, isMaskedMainlandPhone, isPointsString, isUtcIso8601Z } from './frozen-scalars';
import { createOutboundRequestContext, isOutboundRequestContext, parseOutboundRequestContext, type OutboundRequestContext } from './outbound-request-context';
...
const detailTabIds = new Set<UserDetailTabId>([
  'account', 'tasks', 'wallet', 'orders', 'tickets', 'audit',
]);
...
const TASK_STATUSES = ['QUOTED', 'RESERVED', 'QUEUED', 'SUBMITTING', 'RUNNING', 'SUCCEEDED', 'FAILED', 'CANCELED', 'EXPIRED', 'SETTLED', 'REFUNDED'] as const;
...
function detailRows(value: unknown, allowedStatuses: ReadonlySet<string>): readonly Readonly<{ createdAt: string; id: string; status: string }>[] | null {
  ...
  const rows = value.map((item) => {
    ...
    return isUuidV7(id) && status && allowedStatuses.has(status) && isUtcIso8601Z(createdAt) ? { createdAt, id, status } : null;
  });
  return rows.every((row) => row !== null) ? rows : null;
}

function boundedList(value: unknown, map: (candidate: Record<string, unknown>) => object | null): readonly object[] | null {
  if (!Array.isArray(value) || value.length > 100) return null;
  const entries = value.map((item) => item && typeof item === 'object' ? map(item as Record<string, unknown>) : null);
...
function parseDirectoryRows(value: unknown): readonly object[] | null {
  return boundedList(value, (row) => {
    ...
    const tags = row.tags;
    if (
      !isUuidV7(id) || !displayName || !isMaskedMainlandPhone(phoneMasked) || row.phone !== undefined || !status || !directoryStatuses.has(status) ||
      (registrationSource !== undefined && (typeof registrationSource !== 'string' || !directoryRegistrationSources.has(registrationSource))) ||
      (spendingTier !== undefined && (typeof spendingTier !== 'string' || !directorySpendingTiers.has(spendingTier))) ||
      (tags !== undefined && (!Array.isArray(tags) || tags.length > 16 || !tags.every((tag) => typeof tag === 'string' && directoryTagPattern.test(tag) && !containsSensitivePhoneLikeValue(tag))))
    ) return null;
    return {
      displayName,
      id,
...
function parseUserDetail(payload: unknown): UserDetailView | null {
  ...
  if (!candidate.user || typeof candidate.user !== 'object' || !Array.isArray(candidate.tabs) || candidate.tabs.length < 1 || candidate.tabs.length > detailTabIds.size || !Array.isArray(candidate.deniedTabs) || typeof candidate.canRequestWalletAdjustment !== 'boolean' || !Array.isArray(candidate.allowedStatusTransitions) || !Array.isArray(candidate.eligibleApprovers)) return null;
  ...
  if (!isUuidV7(userId) || !displayName || !isMaskedMainlandPhone(phoneMasked) || user.phone !== undefined || !identityStatus || !directoryStatuses.has(identityStatus)) return null;
  ...
  for (const value of candidate.tabs) {
    ...
    if (typeof id !== 'string' || !detailTabIds.has(id as UserDetailTabId) || seen.has(id) || typeof status !== 'string' || !detailTabStatuses.has(status as UserDetailTabStatus)) return null;
    seen.add(id);
    if (id === 'account') {
      if (!tab.account || typeof tab.account !== 'object' || !tab.session || typeof tab.session !== 'object') return null;
      ...
      const tags = boundedList(account.tags, (tag) => { const value = phoneFreeText(tag.value ?? tag.name, 64); return value ? { value } : null; });
      ...
      const devices = boundedList(session.devices, (device) => { const deviceId = boundedText(device.id, 128); const platform = phoneFreeText(device.platform, 64); const deviceStatus = phoneFreeText(device.status, 64); const lastSeenAt = boundedText(device.lastSeenAt, 64); return isUuidV7(deviceId) && platform && deviceStatus && isUtcIso8601Z(lastSeenAt) ? { id: deviceId, lastSeenAt, platform, status: deviceStatus } : null; });
      const loginRecords = boundedList(session.loginRecords, (record) => { const recordId = boundedText(record.id, 128); const deviceLabel = phoneFreeText(record.deviceLabel, 128); const recordStatus = phoneFreeText(record.status, 64); const occurredAt = boundedText(record.occurredAt, 64); return isUuidV7(recordId) && deviceLabel && recordStatus && isUtcIso8601Z(occurredAt) ? { deviceLabel, id: recordId, occurredAt, status: recordStatus } : null; });
      if (!accountDisplayName || !isMaskedMainlandPhone(accountPhoneMasked) || account.phone !== undefined || !accountStatus || !directoryStatuses.has(accountStatus) || accountStatus !== identityStatus || !registrationSource || !spendingTier || !tags || !isUtcIso8601Z(createdAt) || !sessionStatus || !isUtcIso8601Z(lastActiveAt) || !devices || !loginRecords) return null;
      tabs.push({ account: { createdAt, displayName: accountDisplayName, phoneMasked: accountPhoneMasked, registrationSource, status: accountStatus as 'ACTIVE' | 'SUSPENDED' | 'CLOSED', spendingTier, tags: tags.map((tag) => (tag as { value: string }).value) }, id, session: { devices: devices as never, lastActiveAt, loginRecords: loginRecords as never, status: sessionStatus }, status: status as UserDetailTabStatus });
      continue;
    }
    if (id === 'wallet') {
    ...
    if (id === 'audit') {
      ...
      const items = tab.items.map((item) => {
        ...
        return action && isUuidV7(auditId) && isUtcIso8601Z(occurredAt) ? { action, id: auditId, occurredAt } : null;
      });
      if (!items.every((item) => item !== null)) return null;
      tabs.push({ id, items, status: status as UserDetailTabStatus });
      continue;
    }
    const allowedStatuses = id === 'tasks' ? taskStatuses : id === 'orders' ? paymentStatuses : ticketStatuses;
  ...
  const account = tabs.find((tab): tab is Extract<UserDetailTab, { id: 'account' }> => tab.id === 'account');
  if (!account || account.account.displayName !== displayName || account.account.phoneMasked !== phoneMasked || account.account.status !== identityStatus) return null;
  const deniedTabValues = candidate.deniedTabs as unknown[];
  if (!deniedTabValues.every((id): id is UserDetailTabId => typeof id === 'string' && detailTabIds.has(id as UserDetailTabId))) return null;
  const deniedTabs = deniedTabValues;
  if (new Set(deniedTabs).size !== deniedTabs.length || deniedTabs.includes('account') || deniedTabs.some((id) => seen.has(id)) || new Set([...seen, ...deniedTabs]).size !== USER_DETAIL_TAB_IDS.length) return null;
  ...
  const expectedTransition = account.account.status === 'ACTIVE' ? 'SUSPENDED' : account.account.status === 'SUSPENDED' ? 'ACTIVE' : undefined;
  if (allowedStatusTransitions.length > 1 || !allowedStatusTransitions.every((transition) => transition === 'ACTIVE' || transition === 'SUSPENDED') || (allowedStatusTransitions.length === 1 && allowedStatusTransitions[0] !== expectedTransition) || (!expectedTransition && allowedStatusTransitions.length !== 0)) return null;
  ...
  if (!eligibleApprovers || new Set(eligibleApprovers.map((approver) => (approver as { id: string }).id)).size !== eligibleApprovers.length) return null;
  return { allowedStatusTransitions: allowedStatusTransitions as readonly ('ACTIVE' | 'SUSPENDED')[], canRequestWalletAdjustment: candidate.canRequestWalletAdjustment, deniedTabs: deniedTabs as readonly UserDetailTabId[], eligibleApprovers: eligibleApprovers as readonly Readonly<{ displayName: string; id: string }>[], tabs, user: { displayName, id: userId, phoneMasked, status: identityStatus as 'ACTIVE' | 'SUSPENDED' | 'CLOSED' } };
}
...
export function createHttpUserOperationPorts(
  ...
  async function protectedFetch<T>(
    ...
    consume: (response: Response, signal: AbortSignal, requestContext: OutboundRequestContext) => Promise<T> | T,
  ): Promise<T> {
  ...
  function baseHeaders(
    ...
    const resolvedContext = requestContext === undefined
  ...
  return {
    exactPhonePort: {
      async lookupExactPhone(input) {
        ...
        if (!input.trustedSessionToken || !isMainlandPhone(input.phone)) throw new Error('Invalid exact phone lookup');
        ...
        return protectedFetch('/v1/admin/users/exact-phone-lookups', { body: JSON.stringify({ phone: input.phone, scope: input.scope }), cache: 'no-store', headers, method: 'POST' }, operation, async (response, signal, requestContext) => {
          if (!response.ok) {
            const reason = response.status === 401 || response.status === 403 ? 'DOWNSTREAM_DENIED' : 'UPSTREAM_FAILURE';
            throw recordedFailure(operation, reason, requestContext, new Error('Exact phone lookup denied'));
          }
          let payload: unknown; try { payload = await response.json(); } catch (error) { if (signal.aborted) throw error; payload = null; }
          if (!payload || typeof payload !== 'object') throw recordedFailure(operation, 'MALFORMED_RESPONSE', requestContext, new Error('Invalid exact phone lookup response'));
          const candidate = payload as Record<string, unknown>;
          const items = parseDirectoryRows(candidate.items);
          if (!items || !isValidSearchHandle(candidate.searchHandle) || !isValidShortLivedSearchExpiry(candidate.expiresAt)) throw recordedFailure(operation, 'MALFORMED_RESPONSE', requestContext, new Error('Invalid exact phone lookup response'));
      ...
      async getUserDetail(input) {
        ...
        return protectedFetch(
          ...
          async (response, signal, requestContext) => {
            if (!response.ok) {
              const code = response.status === 404 ? 'NOT_FOUND' : response.status === 401 || response.status === 403 ? 'FORBIDDEN' : 'DEPENDENCY';
              if (code === 'NOT_FOUND') throw new UserDetailPortError(code);
              throw recordedFailure(operation, code === 'FORBIDDEN' ? 'DOWNSTREAM_DENIED' : 'UPSTREAM_FAILURE', requestContext, new UserDetailPortError(code));
            }
            let payload: unknown;
            try { payload = await response.json(); } catch (error) { if (signal.aborted) throw error; payload = null; }
            const view = parseUserDetail(payload);
            if (!view) {
              throw recordedFailure(operation, 'MALFORMED_RESPONSE', requestContext, new UserDetailPortError('DEPENDENCY'));
            }
      ...
      async getUserScope(input) {
        ...
        return protectedFetch(
          ...
          async (response, signal, requestContext) => {
            if (!response.ok) {
              const reason = response.status === 401 || response.status === 403 ? 'DOWNSTREAM_DENIED' : 'UPSTREAM_FAILURE';
              throw recordedFailure(operation, reason, requestContext, new Error('Unable to resolve resource scope'));
            }
            ...
            if (!payload || typeof payload !== 'object') {
              throw recordedFailure(operation, 'MALFORMED_RESPONSE', requestContext, new Error('Invalid resource scope response'));
            }
            ...
            if (
              ...
              new Set(candidate.assignedAdminIds.map((id) => id.toLowerCase())).size !== candidate.assignedAdminIds.length ||
              (candidate.ownerAdminId !== null &&
                !isUuidV7(candidate.ownerAdminId))
            ) {
              throw recordedFailure(operation, 'MALFORMED_RESPONSE', requestContext, new Error('Invalid resource scope response'));
            }
    ...
    statusPort: {
      async requestStatusChange(input) {
        ...
        if (!input.trustedSessionToken || !isUuidV7(input.userId) || !isUuidV7(input.audit.idempotencyKey) || !isPhoneFreeBoundedText(input.reason, 200)) throw new Error('Invalid status change context');
        ...
        return protectedFetch(`/v1/admin/users/${encodeURIComponent(input.userId)}/status-change-requests`, { body: JSON.stringify({ reason: input.reason, targetStatus: input.targetStatus }), cache: 'no-store', headers, method: 'POST' }, operation, async (response, signal, requestContext) => {
          if (response.status !== 202) { const reason = response.status === 401 || response.status === 403 || response.status === 409 ? 'DOWNSTREAM_DENIED' : 'UPSTREAM_FAILURE'; throw recordedFailure(operation, reason, requestContext, new Error('Status change denied')); }
          let payload: unknown; try { payload = await response.json(); } catch (error) { if (signal.aborted) throw error; payload = null; }
          if (!payload || typeof payload !== 'object') throw recordedFailure(operation, 'MALFORMED_RESPONSE', requestContext, new Error('Invalid status change response'));
          const candidate = payload as Record<string, unknown>; const auditRecordId = boundedText(candidate.auditRecordId, 128); const requestId = boundedText(candidate.requestId, 128);
          if (!isUuidV7(auditRecordId) || !isUuidV7(requestId) || candidate.status !== undefined) throw recordedFailure(operation, 'MALFORMED_RESPONSE', requestContext, new Error('Invalid status change response'));
    ...
    adjustmentPort: {
      ...
      async getEligibleApprovers(input) {
        ...
        const headers = baseHeaders(input.requestContext, operation); headers.set('Content-Type', 'application/json'); headers.set('X-Admin-Session-Token', input.trustedSessionToken);
        return protectedFetch(`/v1/admin/users/${encodeURIComponent(input.userId)}/eligible-approvers`, { body: JSON.stringify({ scope: input.dataScope }), cache: 'no-store', headers, method: 'POST' }, operation, async (response, signal, requestContext) => {
          if (!response.ok) { const reason = response.status === 401 || response.status === 403 ? 'DOWNSTREAM_DENIED' : 'UPSTREAM_FAILURE'; throw recordedFailure(operation, reason, requestContext, new Error('Eligible approvers denied')); }
          let payload: unknown; try { payload = await response.json(); } catch (error) { if (signal.aborted) throw error; payload = null; }
          if (!Array.isArray(payload) || payload.length > 100) throw recordedFailure(operation, 'MALFORMED_RESPONSE', requestContext, new Error('Invalid eligible approvers'));
          const approvers = payload.map((value) => value && typeof value === 'object' ? value as Record<string, unknown> : null);
          const normalizedIds = approvers.map((approver) => typeof approver?.id === 'string' ? approver.id.toLowerCase() : '');
          if (!approvers.every((approver) => approver && isUuidV7(approver.id) && isPhoneFreeBoundedText(approver.displayName, 256)) || new Set(normalizedIds).size !== approvers.length) throw recordedFailure(operation, 'MALFORMED_RESPONSE', requestContext, new Error('Invalid eligible approvers'));
          return approvers.map((approver) => ({ displayName: (approver as Record<string, string>).displayName as string, id: (approver as Record<string, string>).id as string }));
        });
      },
      async previewAdjustment(input) {
        ...
        if (!input.trustedSessionToken || !isUuidV7(input.userId) || !isUuidV7(input.approverId) || !isUuidV7(input.audit.idempotencyKey) || (direction !== 'CREDIT' && direction !== 'DEBIT') || input.points <= 0n || !isPhoneFreeBoundedText(input.reason, 200)) throw new Error('Invalid adjustment preview context');
        const headers = baseHeaders(input.requestContext, operation); headers.set('Content-Type', 'application/json'); headers.set('Idempotency-Key', input.audit.idempotencyKey); headers.set('X-Admin-Session-Token', input.trustedSessionToken);
        return protectedFetch(`/v1/admin/users/${encodeURIComponent(input.userId)}/wallet-adjustment-previews`, { body: JSON.stringify({ approverId: input.approverId, direction, points: input.points.toString(), reason: input.reason }), cache: 'no-store', headers, method: 'POST' }, operation, async (response, signal, requestContext) => {
          if (!response.ok) { const reason = response.status === 401 || response.status === 403 ? 'DOWNSTREAM_DENIED' : 'UPSTREAM_FAILURE'; throw recordedFailure(operation, reason, requestContext, new Error('Wallet adjustment preview was denied')); }
          let payload: unknown; try { payload = await response.json(); } catch (error) { if (signal.aborted) throw error; payload = null; }
          if (!payload || typeof payload !== 'object') throw recordedFailure(operation, 'MALFORMED_RESPONSE', requestContext, new Error('Invalid wallet adjustment preview'));
          const p = payload as Record<string, unknown>;
          if (!isPointsString(p.before) || !isPointsString(p.after) || !isPointsString(p.points) || (p.direction !== 'CREDIT' && p.direction !== 'DEBIT') || !isCoherentPointsAdjustment(p.before, p.after, p.points, p.direction) || !isUtcIso8601Z(p.expiresAt) || !isPhoneFreeBoundedText(p.impact, 512) || !isPhoneFreeBoundedText(p.policy, 512) || !isPhoneFreeBoundedText(p.previewToken, 512)) throw recordedFailure(operation, 'MALFORMED_RESPONSE', requestContext, new Error('Invalid wallet adjustment preview'));
          return { after: p.after, before: p.before, direction: p.direction, expiresAt: p.expiresAt, impact: p.impact, points: p.points, policy: p.policy, previewToken: p.previewToken } as WalletAdjustmentPreview;
        });
      },
      async submitAdjustmentRequest(input) {
        const operation = 'operations.user.wallet-adjustment-request';
        const direction: unknown = input.direction;
        if (!input.trustedSessionToken || !isUuidV7(input.userId) || !isUuidV7(input.approverId) || !isUuidV7(input.audit.idempotencyKey) || (direction !== 'CREDIT' && direction !== 'DEBIT') || input.points <= 0n || !isPhoneFreeBoundedText(input.reason, 200) || !isPhoneFreeBoundedText(input.previewToken, 512)) {
        ...
        return protectedFetch(
          ...
          async (response, signal, requestContext) => {
            if (!response.ok) {
              const reason = response.status === 401 || response.status === 403 ? 'DOWNSTREAM_DENIED' : 'UPSTREAM_FAILURE';
              throw recordedFailure(operation, reason, requestContext, new Error('Wallet adjustment request was denied'));
            }
            let payload: unknown; try { payload = await response.json(); } catch (error) { if (signal.aborted) throw error; payload = null; }
            if (!payload || typeof payload !== 'object') throw recordedFailure(operation, 'MALFORMED_RESPONSE', requestContext, new Error('Invalid wallet adjustment request'));
            const candidate = payload as Record<string, unknown>; const requestId = boundedText(candidate.requestId, 128); const auditRecordId = boundedText(candidate.auditRecordId, 128);
            if (!isUuidV7(requestId) || !isUuidV7(auditRecordId) || candidate.status !== 'PENDING_APPROVAL') throw recordedFailure(operation, 'MALFORMED_RESPONSE', requestContext, new Error('Invalid wallet adjustment request'));
    ...
    exportPort: {
      async requestCsvExport(input) {
        ...
        if (!isPhoneFreeBoundedText(input.reason, 200) || [input.query, ...Object.values(input.filters ?? {})].some(containsSensitivePhoneLikeValue)) throw new Error('Sensitive CSV query is forbidden');
        if (input.searchHandle !== undefined && (!isValidSearchHandle(input.searchHandle) || input.query !== undefined || input.filters !== undefined)) {
        ...
        return protectedFetch(
          ...
          async (response, signal, requestContext) => {
            if (!response.ok) {
              const reason = response.status === 401 || response.status === 403 ? 'DOWNSTREAM_DENIED' : 'UPSTREAM_FAILURE';
              throw recordedFailure(operation, reason, requestContext, new Error('CSV export request was denied'));
            }
            let payload: unknown;
            try {
              payload = await response.json();
            ...
            const responsePayload = payload && typeof payload === 'object' ? payload as { auditRecordId?: unknown; downloadUrl?: unknown; expiresAt?: unknown } : undefined;
            ...
            if (
              !isUuidV7(auditRecordId) || !isPhoneFreeHttpsUrl(downloadUrl) || !isUtcIso8601Z(expiresAt)
            ) {
              throw recordedFailure(operation, 'MALFORMED_RESPONSE', requestContext, new Error('Invalid CSV export audit response'));
            }
    ...
    directoryPort: {
      async searchUsers(input) {
        ...
        if ([input.query, input.cursor, ...Object.values(input.filters ?? {})].some(containsSensitivePhoneLikeValue)) throw new Error('Sensitive user directory query is forbidden');
        if (input.cursor !== undefined && !isSafeDirectoryCursor(input.cursor)) throw new Error('Invalid user directory cursor');
        if (isMainlandPhone(input.query.trim())) throw new Error('Use protected exact lookup for phone-shaped queries');
        ...
        if (filters.registrationSource) parameters.set('registrationSource', filters.registrationSource);
        ...
        return protectedFetch(
          ...
          async (response, signal, requestContext) => {
            if (!response.ok) {
              const reason = response.status === 401 || response.status === 403 ? 'DOWNSTREAM_DENIED' : 'UPSTREAM_FAILURE';
              throw recordedFailure(operation, reason, requestContext, new Error('User directory request was denied'));
            }
            let payload: unknown;
            try {
              payload = await response.json();
            ...
            if (!payload || typeof payload !== 'object') {
              throw recordedFailure(operation, 'MALFORMED_RESPONSE', requestContext, new Error('Invalid user directory response'));
            }
            const candidate = payload as Partial<{
              items: unknown;
