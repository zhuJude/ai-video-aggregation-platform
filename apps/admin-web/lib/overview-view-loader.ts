...
export type OverviewMeasure =
  | Readonly<{ currency: 'CNY'; direction?: OverviewMoneyDirection; id: string; label: string; minorUnits: string }>
...
export interface OverviewPort {
  getOverview(input: Readonly<{ requestContext: OutboundRequestContext; trustedSessionToken: string }>): Promise<OverviewView>;
}

export async function loadOverviewView(dependencies: Readonly<{
