...
function parseDataset(
  ...
  if (
    ...
    !phoneFreeText(candidate.label) ||
    !statusValues.has(candidate.status as OverviewSourceStatus) ||
    !Array.isArray(candidate.measures)
  )
    return malformed(telemetry, requestContext);
  if (candidate.status === 'ERROR') {
    if (
      !phoneFreeText(candidate.reason) ||
  ...
  if (
    ...
    !phoneFreeText(candidate.warning)
  )
    return malformed(telemetry, requestContext);
  const expected = requiredMeasures[candidate.id];
  if (!expected || candidate.measures.length !== expected.length)
    return malformed(telemetry, requestContext);
  const measures: OverviewDataset['measures'] = candidate.measures.map((measure) => {
    ...
    if (!safeText(item.id) || !phoneFreeText(item.label))
      return malformed(telemetry, requestContext);
    if (moneyMeasureIds.has(item.id) || signedMoneyMeasureIds.has(item.id)) {
      if (
        !isPointsString(item.minorUnits) ||
        item.currency !== 'CNY' ||
    ...
    if (item.id === durationMeasureId) {
      if (
        !isPointsString(item.value) ||
        item.unit !== 'SECONDS' ||
        item.minorUnits !== undefined ||
    ...
    if (
      ...
      item.currency !== undefined ||
      item.direction !== undefined ||
      ((pointMeasureIds.has(item.id) || countMeasureIds.has(item.id)) &&
        !isPointsString(item.value)) ||
  ...
  if (
    new Set(measures.map((measure) => measure.id)).size !== expected.length ||
    !expected.every((id) => measures.some((measure) => measure.id === id))
  )
    return malformed(telemetry, requestContext);
  return {
    id: candidate.id,
    ...
    status: candidate.status as OverviewSourceStatus,
...
export function createHttpOverviewPort(
  environment: OverviewEnvironment = {
    apiUrl: process.env.ADMIN_REPORTING_API_URL,
    kmsIdentityReference: process.env.ADMIN_REPORTING_KMS_IDENTITY_REF,
  },
  ...
  try {
    if (
      !environment.apiUrl ||
      !environment.kmsIdentityReference?.trim() ||
    ...
    async getOverview({ requestContext, trustedSessionToken }): Promise<OverviewView> {
      ...
      try {
        return await fetchWithDeadline(
          fetchImpl,
          new URL('/v1/admin/reporting/overview', baseUrl),
          {
            cache: 'no-store',
            headers: {
              Accept: 'application/json',
              'X-Admin-Session-Token': trustedSessionToken,
              'X-Correlation-Id': safeContext.correlationId,
              'X-Service-Identity-Ref': kmsIdentityReference,
              'X-Trace-Id': safeContext.traceId,
            },
            method: 'GET',
          },
          deadlineMs,
          async (response, signal) => {
            ...
            try {
              ...
              return malformed(telemetry, safeContext);
            }
            if (
              !payload ||
            ...
            if (datasets.length !== Object.keys(requiredMeasures).length)
              return malformed(telemetry, safeContext);
            const parsed = datasets.map((dataset) => parseDataset(dataset, telemetry, safeContext));
            if (
              new Set(parsed.map((dataset) => dataset.id)).size !== parsed.length ||
              Object.keys(requiredMeasures).some(
                (id) => !parsed.some((dataset) => dataset.id === id),
              )
            )
              return malformed(telemetry, safeContext);
