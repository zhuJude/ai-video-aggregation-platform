...
type Session = Readonly<{
  ...
  lastActiveAt: string;
  loginRecords: readonly Readonly<{
    deviceLabel: string;
    id: string;
    occurredAt: string;
    status: string;
...
type WalletHistory<Direction extends 'CREDIT' | 'DEBIT'> = Readonly<{
  direction: Direction;
  id: string;
  occurredAt: string;
  points: string;
...
export type WalletAdjustmentHistory = WalletHistory<'CREDIT' | 'DEBIT'> &
  Readonly<{ approverId?: string; requestedById?: string; version?: number }>;
...
export type UserDetailTab =
  ...
  | Readonly<{
      adjustmentHistory: readonly WalletAdjustmentHistory[];
      balance: string;
      consumptionHistory: readonly WalletHistory<'DEBIT'>[];
      frozenBalance: string;
      id: 'wallet';
      rechargeHistory: readonly WalletHistory<'CREDIT'>[];
      status: UserDetailTabStatus;
      unit: 'POINTS';
    }>
  | Readonly<{
      id: 'orders';
      items: readonly Row<UserPaymentStatus>[];
      status: UserDetailTabStatus;
    }>
  | Readonly<{
      id: 'tickets';
      items: readonly Row<UserTicketStatus>[];
      status: UserDetailTabStatus;
    }>
  | Readonly<{
      id: 'audit';
      items: readonly Readonly<{ action: string; id: string; occurredAt: string }>[];
      status: UserDetailTabStatus;
    }>;
...
export async function loadUserDetailView(
  ...
  try {
    ...
    if (
      !account ||
      !isMaskedMainlandPhone(view.user.phoneMasked) ||
      !isMaskedMainlandPhone(account.account.phoneMasked) ||
      view.user.phoneMasked !== account.account.phoneMasked ||
      view.user.status !== account.account.status ||
      (view.user as { phone?: unknown }).phone !== undefined ||
      (account.account as { phone?: unknown }).phone !== undefined ||
      !['ACTIVE', 'SUSPENDED', 'CLOSED'].includes(view.user.status)
    )
    ...
    const tabs = view.tabs.filter((tab) =>
      hasPermission(authorization.claims, TAB_PERMISSION[tab.id]),
    );
    const deniedTabs = USER_DETAIL_TAB_IDS.filter((id) => !tabs.some((tab) => tab.id === id));
    const walletVisible = tabs.some((tab) => tab.id === 'wallet');
    const canRequestWalletAdjustment =
      walletVisible &&
      view.canRequestWalletAdjustment &&
      hasPermission(authorization.claims, 'wallet:adjust');
    const wallet = tabs.find(
      (tab): tab is Extract<UserDetailTab, { id: 'wallet' }> => tab.id === 'wallet',
    );
    const canApproveWalletAdjustments = Boolean(
      wallet &&
      hasPermission(authorization.claims, 'wallet:adjust') &&
      wallet.adjustmentHistory.some(
        (entry) =>
          entry.status === 'PENDING_APPROVAL' &&
          entry.approverId &&
          isSameUuidV7(entry.approverId, authorization.claims.subjectId) &&
          entry.requestedById &&
          !isSameUuidV7(entry.requestedById, authorization.claims.subjectId),
      ),
    );
    return {
      ok: true,
      view: {
        ...
        canChangeStatus:
          ...
          Boolean(
            requiredTransition && view.allowedStatusTransitions?.includes(requiredTransition),
          ),
        canRequestWalletAdjustment,
        canApproveWalletAdjustments,
        currentActorId: authorization.claims.subjectId,
        eligibleApprovers: canRequestWalletAdjustment
          ? (view.eligibleApprovers?.filter(
              (approver) => !isSameUuidV7(approver.id, authorization.claims.subjectId),
            ) ?? [])
          : [],
      },
    };
