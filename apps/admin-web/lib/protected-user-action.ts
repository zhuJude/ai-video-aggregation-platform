...
export interface UserStatusPort {
  ...
  ): Promise<Readonly<{ auditRecordId: string; requestId: string }>>;
}

type UserStatusActionDependencies = Readonly<{
...
export function createUserStatusAction({
  createCorrelationId: makeCorrelationId = createUuidV7,
  createTraceId: makeTraceId = createTraceId,
  guardContext,
  port,
  scopePort,
}: UserStatusActionDependencies) {
  return async function userStatusAction(
    formData: FormData,
    ...
    const userId =
      typeof formData.get('userId') === 'string' ? (formData.get('userId') as string) : '';
    ...
    if (
      ...
      !reason ||
      reason.length > 200 ||
      containsSensitivePhoneLikeValue(reason) ||
      formData.get('highRiskConfirmed') !== 'true'
    )
      throw new Error('状态变更无效');
    ...
    const result = await port.requestStatusChange({
