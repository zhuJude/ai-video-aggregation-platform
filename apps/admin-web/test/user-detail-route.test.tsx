...
describe('configured user detail route', () => {
  it('renders the authoritative user detail view instead of the unavailable fallback', async () => {
    ...
    render(
      await renderUserDetailRoute(userId, { context: { sessionToken: token, signingKey }, port }),
    );
  ...
  it('renders explicit 400, 403, 404, and dependency states without exposing detail data', async () => {
    ...
    const denied: UserDetailPort = {
      async getUserDetail() {
        throw new UserDetailPortError('FORBIDDEN');
      },
    };
    const missing: UserDetailPort = {
      async getUserDetail() {
        throw new UserDetailPortError('NOT_FOUND');
      },
    };
    const unavailable: UserDetailPort = {
      async getUserDetail() {
        throw new UserDetailPortError('DEPENDENCY');
      },
    };
  ...
  it('selects and renders every authoritative operational section from the real route', async () => {
    const account = view.tabs.find(
      (tab): tab is Extract<UserDetailView['tabs'][number], { id: 'account' }> =>
        tab.id === 'account',
    );
    ...
    const fullToken = await signAdminSession(
      {
        dataScope: 'ALL',
        expiresAt: Date.now() + 60_000,
        permissions: ['users:read', 'tasks:read', 'finance:read', 'tickets:read', 'audit:read'],
        subjectId: '0198f7a4-c6d2-7b39-8a4e-73af0c1d2e3f',
        sessionInstanceId: '0198f7a4-c6da-7b39-8a4e-73af0c1d2e3f',
      },
      signingKey,
    ...
    render(
      await renderUserDetailRoute(userId, {
        context: { sessionToken: fullToken, signingKey },
        port,
      }),
    );
  ...
  it('intersects real HTTP-authorized tabs with local permissions before rendering', async () => {
    vi.stubGlobal(
      ...
      vi.fn(
        async () =>
          new Response(JSON.stringify(authoritativePayload()), {
            headers: { 'Content-Type': 'application/json' },
          }),
      ),
    );
    const restrictedToken = await signAdminSession(
      {
        dataScope: 'ALL',
        expiresAt: Date.now() + 60_000,
        permissions: ['users:read'],
    ...
    const restrictedPort = createHttpUserOperationPorts({
      apiUrl: 'https://operations.example.invalid',
      kmsIdentityReference: 'kms://service/admin-web',
    }).detailPort;
    const { rerender } = render(
      await renderUserDetailRoute(userId, {
        context: { sessionToken: restrictedToken, signingKey },
        port: restrictedPort,
      }),
    );
    ...
    expect(
      within(screen.getByRole('region', { name: '充值历史' })).getByText('CREDIT'),
    ).toBeVisible();
    expect(
      within(screen.getByRole('region', { name: '消费历史' })).getByText('DEBIT'),
    ).toBeVisible();
  ...
  it('rejects PENDING at the real route boundary and renders CLOSED without a status action', async () => {
    ...
    const pendingPort = createHttpUserOperationPorts({
      apiUrl: 'https://operations.example.invalid',
      kmsIdentityReference: 'kms://service/admin-web',
    }).detailPort;
    const { rerender } = render(
      await renderUserDetailRoute(userId, {
        context: { sessionToken: token, signingKey },
        port: pendingPort,
      }),
    );
    expect(screen.getByText(/依赖服务错误/u)).toBeVisible();
    ...
    vi.stubGlobal(
      ...
      vi.fn(
        async () =>
          new Response(JSON.stringify(closed), { headers: { 'Content-Type': 'application/json' } }),
      ),
    );
    const closedPort = createHttpUserOperationPorts({
      apiUrl: 'https://operations.example.invalid',
      kmsIdentityReference: 'kms://service/admin-web',
    }).detailPort;
    rerender(
  ...
  it('renders a real HTTP detail with an authoritative empty transition set and hides status actions', async () => {
    ...
    const port = createHttpUserOperationPorts({
      apiUrl: 'https://operations.example.invalid',
      kmsIdentityReference: 'kms://service/admin-web',
    }).detailPort;
