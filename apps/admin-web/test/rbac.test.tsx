...
import {
  ...
  SystemConsole,
  TicketConsole,
} from '../components/governance/governance-console';
import {
  canTransitionTicket,
...
const systemPayload = {
  alerts: [
    {
      closeCondition: 'DLQ 小于 1',
      id: auditId,
      ownerMasked: 'payments-****',
      runbookUrl: 'https://ops.example.com/runbooks/refund',
      severity: 'HIGH',
      summary: '退款队列积压',
    },
  ],
  config: {
    ...
    publicSettings: {
      current: {
        publicCallbackUrl: 'https://api.example.com/callback',
        publicDomain: 'https://www.example.com',
      },
      diff: ['publicDomain changed'],
      draft: {
        publicCallbackUrl: 'https://api.example.com/callback-v2',
        publicDomain: 'https://app.example.com',
      },
      history: [{ publishedAt: '2026-09-10T00:00:00.000Z', version: 19 }],
      secretReferences: [
        {
          kmsReference: 'kms://admin-web/payment-callback-signing',
          masked: 'sk_***91',
          name: 'callback signing',
        },
      ],
      validation: { errors: [], valid: true },
    },
...
describe('Task 7 permission catalog and content publication', () => {
  ...
  it('preserves BigInt plan values and accepts only structured rich text', () => {
    expect(parseContentDirectory(contentPayload).items[0]?.draft.planPoints).toBe(
      '9007199254740993',
    );
    expect(() =>
      parseContentDirectory({
        ...
        items: [
          {
            ...
            draft: {
              ...
              body: {
                blocks: [{ href: '&#x6a;avascript:alert(1)', text: '点我', type: 'PARAGRAPH' }],
              },
            },
          },
        ],
  ...
  it('publishes only an authoritative validated preview and binds the receipt', async () => {
    ...
    expect(executeContentOperation).toHaveBeenCalledWith(
  ...
  it('saves a safe BigInt draft and validates the authoritative version before preview', async () => {
    ...
    expect(saveContentDraft).toHaveBeenCalledWith(
      expect.objectContaining({ planPoints: '9007199254740993' }),
    );
...
describe('ticket workflow', () => {
  ...
  it('enforces the state machine, admin-public-reply resolve gate and seven-day reopen window', () => {
    expect(
      canTransitionTicket('OPEN', 'IN_PROGRESS', null, false, Date.parse('2026-09-11T00:00:00Z')),
    ).toBe(true);
    expect(
      canTransitionTicket('OPEN', 'RESOLVED', null, true, Date.parse('2026-09-11T00:00:00Z')),
    ).toBe(false);
    expect(
      canTransitionTicket(
        'IN_PROGRESS',
  ...
  it('uses distinct public reply and internal note ports after re-fetching scope/version', async () => {
    const addPublicReply = vi.fn(async () => ({
      auditRecordId: auditId,
      idempotencyKey: intentId,
      messageVisibility: 'PUBLIC_REPLY',
      ok: true,
      requestId: roleId,
      ticketId,
      version: 6,
    ...
    expect(addPublicReply).toHaveBeenCalledWith(
      expect.objectContaining({ attachmentFileIds: [attachmentId] }),
    );
...
describe('authoritative IAM controls', () => {
  it('disables permissions absent from the authoritative grantable matrix', () => {
    render(
      <RoleEditor
        actorPermissions={['iam:role-write', 'users:read']}
        directory={parseIamDirectory(iamPayload)}
        roleId={roleId}
      />,
    );
            ...
            roles: [
              {
                ...
                preview: {
                  ...
                  added: [],
                  operation: 'DELETE',
                  proposedDataScope: null,
                  proposedPermissions: [],
                  removed: ['users:read', 'tickets:read'],
                },
              },
            ],
          };
  ...
  it('allows deleting an ordinary role only with iam:role-delete and a bound DELETE preview', async () => {
    ...
    const deletePayload = {
      ...iamPayload,
      admins: [{ ...iamPayload.admins[0], roleIds: [contentId] }],
  ...
  it('rejects dangling role assignments and inconsistent authoritative IAM counts', () => {
    ...
    expect(() =>
      parseIamDirectory({ ...iamPayload, roles: [{ ...iamPayload.roles[0], adminCount: 0 }] }),
    ).toThrow('权限响应无效');
  ...
  it('rejects IAM mutations when the authoritative actor does not match the session subject', async () => {
    ...
    form.set('operation', 'UPDATE_SCOPE');
    form.set('dataScope', 'ALL');
  ...
  it('expands target roles and rejects assignment of a role whose permissions are not held by the actor', async () => {
    ...
    const targetAdmin = {
      ...
      preview: {
        ...
        preflightToken: 'admin-assignment-token',
        proposedDataScope: 'OWN',
        proposedRoleIds: [contentId],
    ...
    const directory = {
      ...iamPayload,
      admins: [...iamPayload.admins, targetAdmin],
      grantablePermissions: [...iamPayload.grantablePermissions, 'content:publish'],
      roles: [...iamPayload.roles, restrictedRole],
    };
    const updateAdmin = vi.fn();
    const action = createAdminUpdateAction({
      context: { sessionToken: await session(['iam:admin-write']), signingKey },
      port: {
        async getIamDirectory() {
          return directory;
        },
    ...
    form.set('operation', 'UPDATE_ASSIGNMENTS');
    form.set('dataScope', 'OWN');
    form.set('status', 'ACTIVE');
    form.append('roleId', contentId);
  ...
  it('derives self-escalation and last-superadmin protection instead of trusting preview booleans', async () => {
    ...
    const selfEscalating = {
      ...
      admins: [
        {
          ...
          preview: {
            ...
            expiresAt: '2099-09-11T00:00:00.000Z',
            operation: 'UPDATE_ASSIGNMENTS',
            preflightToken: 'admin-self-token',
            proposedDataScope: 'ALL',
            proposedRoleIds: [roleId, contentId],
            proposedStatus: 'ACTIVE',
            removesLastSuperAdmin: false,
            resultVersion: 4,
          },
      ...
      grantablePermissions: [...iamPayload.grantablePermissions, 'content:publish'],
      roles: [...iamPayload.roles, elevatedRole],
    };
    const selfAction = createAdminUpdateAction({
      context: {
        sessionToken: await session([
          'iam:admin-write',
          'users:read',
    ...
    selfForm.set('operation', 'UPDATE_ASSIGNMENTS');
    selfForm.set('dataScope', 'ALL');
    selfForm.set('status', 'ACTIVE');
    selfForm.append('roleId', roleId);
    selfForm.append('roleId', contentId);
      ...
      admins: [
        {
          ...
          preview: {
            ...
            expiresAt: '2099-09-11T00:00:00.000Z',
            operation: 'UPDATE_ASSIGNMENTS',
            preflightToken: 'admin-super-token',
            proposedDataScope: 'ALL',
            proposedRoleIds: [],
            proposedStatus: 'ACTIVE',
            removesLastSuperAdmin: false,
    ...
    const superAction = createAdminUpdateAction({
      context: { sessionToken: await session(['iam:admin-write']), signingKey },
      port: {
        async getIamDirectory() {
          return removesSuper;
    ...
    superForm.set('operation', 'UPDATE_ASSIGNMENTS');
    superForm.set('dataScope', 'ALL');
    superForm.set('status', 'ACTIVE');
...
describe('audit and system operations', () => {
  it('strictly parses read-only audit data with cursor and trace filters', () => {
    ...
    expect(() => parseAuditDirectory({ ...auditPayload, editAllowed: true })).toThrow(
      '审计响应无效',
    );
  });
  ...
  it('rejects an audit export receipt whose fingerprint is not bound to the requested filters', async () => {
    const action = createAuditExportAction({
      ...
      port: {
        async getAuditExportPreview() {
          return {
            expiresAt: auditPayload.exportPreview.expiresAt,
            filterFingerprint: auditPayload.exportPreview.filterFingerprint,
            preflightToken: auditPayload.exportPreview.preflightToken,
    ...
    const form = new FormData();
    for (const [key, value] of Object.entries(auditPayload.exportPreview.filters))
      form.set(key, value);
    form.set('format', 'CSV');
    form.set('filterFingerprint', 'audit-filter-fingerprint');
    form.set('intentId', intentId);
      ...
      parseSystemSnapshot(
        {
          ...systemPayload,
          config: {
            ...systemPayload.config,
            publicSettings: {
              ...systemPayload.config.publicSettings,
              secretReferences: [
                { kmsReference: 'PRIVATE KEY=leaked', masked: '***', name: 'bad' },
              ],
            },
          },
        },
  ...
  it('uses only explicitly configured production HTTPS origins and rejects URL authority tricks', () => {
    expect(parseTrustedObservabilityOrigins('https://ops.example.com')).toEqual(
      trustedObservabilityOrigins,
    );
    for (const value of [
      'https://user@ops.example.com',
      'https://ops.example.com/path?token=x',
      'https://127.0.0.1',
      'https://10.1.2.3',
      'https://observability.internal.example',
    ]) {
      expect(parsePublicHttpsUrl(value)).toBeNull();
    }
    expect(() =>
      parseTrustedObservabilityOrigins('https://observability.internal.example'),
    ).toThrow('可信观测域配置无效');
  });
      ...
      parseSystemSnapshot(
        {
          ...
          config: {
            ...systemPayload.config,
            featureFlags: {
              ...systemPayload.config.featureFlags,
              validation: { errors: ['invalid flag'], valid: false },
            },
          },
        },
        trustedObservabilityOrigins,
      ...
      parseSystemSnapshot(
        {
          ...
          config: {
            ...systemPayload.config,
            allowedOperations: systemPayload.config.allowedOperations.filter(
              (operation) => operation !== 'REDRIVE_DLQ',
            ),
          },
        },
        trustedObservabilityOrigins,
  ...
  it('refuses DLQ redrive unless every authoritative machine safety gate passes', async () => {
    const unsafe = {
      ...
      queues: [
        {
          ...systemPayload.queues[0],
          preview: {
            ...systemPayload.queues[0]?.preview,
            billingSafe: false,
          },
        },
      ],
  ...
  it('uses system:config-write for a validated public-setting draft and keeps its secret write-only', async () => {
    const draftSnapshot = {
      ...
      config: {
        ...systemPayload.config,
        allowedOperations: ['SAVE_SETTING_DRAFT', 'REDRIVE_DLQ'],
        preview: {
          ...systemPayload.config.preview,
          operation: 'SAVE_SETTING_DRAFT',
          preflightToken: 'settings-draft-token',
          resultVersion: 21,
        },
      },
    };
    const executeSystemOperation = vi.fn(
      async (input: Parameters<GovernanceOperationsPort['executeSystemOperation']>[0]) => {
        void input;
        return {
          auditRecordId: auditId,
          idempotencyKey: intentId,
          ...
          operation: 'SAVE_SETTING_DRAFT',
          requestId: roleId,
          resourceId: 'public-settings',
          version: 21,
        };
      },
    );
    const action = createSystemMutationAction({
      context: { sessionToken: await session(['system:config-write']), signingKey },
      port: {
        executeSystemOperation,
        async getSystemSnapshot() {
    ...
    form.set('operation', 'SAVE_SETTING_DRAFT');
    form.set('publicDomain', 'https://app.example.com');
    form.set('publicCallbackUrl', 'https://api.example.com/callback-v2');
    form.set('replacementSecret', 'replacement-secret-value');
    ...
    expect(executeSystemOperation.mock.calls[0]?.[0].draft).toMatchObject({
      publicDomain: 'https://app.example.com/',
      replacementSecret: 'replacement-secret-value',
    });
  });
