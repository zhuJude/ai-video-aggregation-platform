...
describe('exact-phone signed search descriptor', () => {
  it('seals the upstream handle into one bounded base64url token and verifies its binding', async () => {
    ...
    const token = await sealExactPhoneSearchDescriptor(
      { expiresAt, handle, scope: 'ASSIGNED', sessionInstanceId, subjectId },
      { now: () => now, signingKey },
    );
    expect(token).toMatch(/^[A-Za-z0-9_-]{32,2048}$/u);
    expect(token).not.toContain(handle);
    await expect(
      verifyExactPhoneSearchDescriptor(token, {
        now: () => now,
        scope: 'ASSIGNED',
        sessionInstanceId,
        signingKey,
    ...
    ).resolves.toEqual({ expiresAt, handle });
  });
  ...
  it.each([
    [
      ...
      async (token: string) =>
        verifyExactPhoneSearchDescriptor(
          ...
          { now: () => now, scope: 'ASSIGNED', sessionInstanceId, signingKey, subjectId },
        ),
    ],
    [
      'wrong subject',
    ...
    [
      'new session for the same subject',
      async (token: string) =>
        verifyExactPhoneSearchDescriptor(token, {
          now: () => now,
          scope: 'ASSIGNED',
          sessionInstanceId: otherSessionInstanceId,
          signingKey,
          subjectId,
        }),
    ],
    ...
    [
      ...
      async (token: string) =>
        verifyExactPhoneSearchDescriptor(token, {
          now: () => Date.parse('2026-09-03T04:10:00.001Z'),
          scope: 'ASSIGNED',
          sessionInstanceId,
          signingKey,
          subjectId,
    ...
    [
      ...
      async (token: string) =>
        verifyExactPhoneSearchDescriptor(token, {
          ...
          scope: 'ASSIGNED',
          sessionInstanceId,
          signingKey: 'different-descriptor-key-at-least-32-bytes',
          subjectId,
        }),
    ],
  ...
  it('rejects a non-canonical base64url alias even when it decodes to the signed bytes', async () => {
    ...
    await expect(
      verifyExactPhoneSearchDescriptor(alternate, {
        now: () => now,
        scope: 'ASSIGNED',
        sessionInstanceId,
        signingKey,
