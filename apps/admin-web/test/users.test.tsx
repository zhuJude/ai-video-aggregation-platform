...
import { AdjustmentDialog, validateAdjustmentRequest } from '../components/adjustment-dialog';
...
function statusView(status: 'ACTIVE' | 'SUSPENDED' | 'CLOSED'): UserDetailView {
  ...
  return {
    ...
    allowedStatusTransitions:
      status === 'CLOSED' ? [] : [status === 'ACTIVE' ? 'SUSPENDED' : 'ACTIVE'],
...
describe('user operations', () => {
  ...
  it('reuses a stable status intent on retry, suppresses double click, and locks after success', async () => {
    ...
    const view: UserDetailView = {
      ...
      user: { ...userDetailView().user, id: '0198f7a4-c6d1-7b39-8a4e-73af0c1d2e3f' },
    };
    render(
      <UserDetail
        view={view}
        onStatusChange={async (form) => {
          forms.push(form);
          attempt += 1;
    ...
    const button = screen.getByRole('button', { name: '封禁用户' });
    fireEvent.click(button);
    fireEvent.click(button);
    await screen.findByText('状态变更被拒绝或暂时不可用');
    expect(forms).toHaveLength(1);
    fireEvent.click(button);
    await screen.findByText(/状态变更请求已受理/u);
  ...
  it.each([
    ...
    'keeps %s authoritative until refreshed to %s, keeps unchanged props locked, then safely recomputes the reverse action',
    async (currentStatus, actionName, returnedStatus, reverseActionName) => {
      ...
      const onStatusChange = async (form: FormData) => {
        submittedForms.push(form);
        return { auditRecordId, ok: true as const, requestId };
      };
      const { rerender } = render(
        <UserDetail view={statusView(currentStatus)} onStatusChange={onStatusChange} />,
      );
      fireEvent.change(screen.getByLabelText('状态变更原因'), { target: { value: '违反规则' } });
      fireEvent.click(screen.getByRole('checkbox', { name: '我确认这是高风险状态变更' }));
      ...
      expect(
        await screen.findByText(new RegExp(`${requestId}.*${auditRecordId}`, 'u')),
      ).toBeVisible();
  ...
  it.each([
    [
      ...
      { auditRecordId: '0198f7a4-c6d4-7b39-8a4e-73af0c1d2e3f', ok: true, requestId: 'request-1' },
    ],
    [
      'malformed audit identity',
      { auditRecordId: 'audit-1', ok: true, requestId: '0198f7a4-c6d5-7b39-8a4e-73af0c1d2e3f' },
    ],
...
describe('user status server action', () => {
  it('requires confirmation and forwards trusted scope, trace and idempotency metadata for an active user ban', async () => {
    ...
    ).not.toBe(requestContext.correlationId);
  });
  ...
  it('fails closed on invalid or duplicate state transitions before invoking the port', async () => {
    ...
    const port: UserStatusPort = {
      async requestStatusChange() {
        calls += 1;
        return { auditRecordId: 'audit-record-1', requestId: 'request-1' };
      },
    };
    const form = new FormData();
    form.set('userId', 'not-a-uuid');
    form.set('currentStatus', 'PENDING');
    form.set('reason', '原因');
    form.set('highRiskConfirmed', 'true');
    await expect(
      createUserStatusAction({
    ...
    const action = createUserStatusAction({
      guardContext: {
        sessionToken: await signAdminSession(
          ...
          'status-action-signing-key-at-least-32-bytes',
        ),
        signingKey: 'status-action-signing-key-at-least-32-bytes',
      },
      port: {
        async requestStatusChange() {
          mutationCalls += 1;
  ...
  it('rejects missing reason or confirmation and backend conflict without a success outcome', async () => {
    ...
    await expect(action(form)).rejects.toThrow('状态变更无效');
    expect(calls).toBe(0);
    form.set('reason', '违反规则');
    form.set('highRiskConfirmed', 'true');
    form.set('intentId', '0198f7a4-c6d5-7b39-8a4e-73af0c1d2e3f');
    await expect(action(form)).rejects.toThrow('backend conflict');
    expect(calls).toBe(1);
  });
});
  ...
  it('never invokes a direct mutation when wallet adjustment permission is absent', async () => {
    ...
    const adjustmentPort: WalletAdjustmentRequestPort = {
      async getEligibleApprovers() {
        return [{ displayName: '复核管理员', id: '0198f7a4-c6d3-7b39-8a4e-73af0c1d2e3f' }];
      },
