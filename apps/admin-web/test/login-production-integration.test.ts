...
vi.mock('next/headers', () => ({
  cookies: () =>
    cookieAccessFailure
      ? Promise.reject(cookieAccessFailure)
      : Promise.resolve({
          delete(name: string) {
            if (cookieOperationFailure) throw cookieOperationFailure;
            responseWrites.set(name, null);
          },
          get(name: string) {
            if (cookieOperationFailure) throw cookieOperationFailure;
...
describe('production MFA preflight and delivered cookie boundary', () => {
  beforeEach(() => {
    ...
    process.env.ADMIN_MFA_CHALLENGE_SIGNING_KEY =
      'production-mfa-test-signing-key-at-least-32-bytes';
  ...
  it('requires an applied preflight Set-Cookie and reuses the credential key when the password response is discarded', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn((_input: RequestInfo | URL, init?: RequestInit) => {
        ...
        return Promise.resolve(
          Response.json({ challengeId: 'A'.repeat(43), expiresInSeconds: 600 }),
        );
      }),
    );
    await expect(submitPasswordAction('/overview', null, passwordForm())).resolves.toEqual({
      step: 'password',
    ...
    await expect(preparePasswordAction('operator@example.invalid')).resolves.toEqual({
      status: 'READY',
    });
  ...
  it('keeps TOTP response-loss retries stable and derives a different key for a different code', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (requestUrl.includes('/password/challenges'))
          return Promise.resolve(
            Response.json({ challengeId: 'A'.repeat(43), expiresInSeconds: 600 }),
          );
    ...
    requestJar.set(
      ADMIN_MFA_CHALLENGE_COOKIE,
      responseWrites.get(ADMIN_MFA_CHALLENGE_COOKIE) as string,
    );
    responseWrites = new Map();
    await submitPasswordAction('/overview', null, passwordForm());
    requestJar.set(
    ...
    expect(
      totpHeaders.every(
        (headers) => headers.get('X-Correlation-Id') === totpHeaders[0]?.get('X-Correlation-Id'),
      ),
    ).toBe(true);
  });
  ...
  it('rejects a tainted authenticated subject without issuing a session or tainted challenge cookie', async () => {
    ...
    await expect(submitTotpAction(null, totpForm('111111'))).resolves.toMatchObject({
      status: 'INVALID_TOTP',
    });
    expect(responseWrites.has(ADMIN_SESSION_COOKIE)).toBe(false);
    const claims = await verifyAdminMfaChallenge(
      requestJar.get(ADMIN_MFA_CHALLENGE_COOKIE),
      process.env.ADMIN_MFA_CHALLENGE_SIGNING_KEY,
      Date.now(),
    );
  ...
  it.each([
    ...
    'rejects %s IAM permissions before the production action writes a session cookie',
  ...
  it.each([
    [
      'IAM configuration',
      () => {
        process.env.ADMIN_AUTH_API_URL = '';
      },
      'iam.config',
    ],
    [
    ...
    'records exactly one source-owned event for missing %s through the exported action',
    async (_label, invalidate, operation) => {
      vi.stubEnv('NODE_ENV', 'production');
      const warnings = vi.spyOn(console, 'warn').mockImplementation(() => undefined);
      invalidate();
  ...
  it.each(['headers access', 'cookie operation'])(
    'records one standalone action event per replayed %s failure',
    async (variant) => {
      vi.stubEnv('NODE_ENV', 'production');
      const warnings = vi.spyOn(console, 'warn').mockImplementation(() => undefined);
