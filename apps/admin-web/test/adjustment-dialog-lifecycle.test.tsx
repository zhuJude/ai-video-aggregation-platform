...
describe('wallet adjustment dialog lifecycle', () => {
  it('opens from the authorized trigger, cancels, restores focus, and reopens with fresh state', async () => {
    render(
      <UserDetail view={view()} onAdjustmentPreview={preview} onAdjustmentRequest={accepted} />,
    );
    ...
    await waitFor(() => {
      expect(screen.queryByRole('dialog', { name: '申请调整点数' })).not.toBeInTheDocument();
    });
  ...
  it('cannot close or replay while submission is pending, then offers an explicit completion close', async () => {
    ...
    const onRequest = vi.fn(
      () =>
        new Promise<Awaited<ReturnType<typeof accepted>>>((resolve) => {
          finish = resolve;
        }),
    );
    render(
      <UserDetail view={view()} onAdjustmentPreview={preview} onAdjustmentRequest={onRequest} />,
    );
    openAndFill();
  ...
  it('does not place the trigger or dialog in the DOM without server authorization', () => {
    render(
      <UserDetail
        view={view(false)}
        onAdjustmentPreview={preview}
        onAdjustmentRequest={accepted}
      />,
    );
