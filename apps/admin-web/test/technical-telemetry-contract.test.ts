...
function expectGeneratedContextEvent(event: unknown, operation: string, reason: string): void {
  ...
  expect(Reflect.ownKeys(candidate).sort()).toEqual([
    'correlationId',
    'operation',
    'reason',
    'traceId',
...
describe('technical telemetry contract', () => {
  it('accepts only plain, exact, data-property outbound contexts', () => {
    ...
    expect(isOutboundRequestContext({ correlationId, traceId, secret: 'must-not-pass' })).toBe(
      false,
    );
  ...
  it('rejects a raw same-shape context before overview and user-operation fetch', async () => {
    ...
    const overview = createHttpOverviewPort(
      ...
      },
      {
        fetchImpl: async () => {
          fetchCount += 1;
          return Response.json(payloadForUnreachableFetch());
        },
    ...
    await expect(
      Reflect.apply(overview.getOverview.bind(overview), undefined, [
        { requestContext: rawContext, trustedSessionToken: 'session-secret' },
      ]),
    ).rejects.toThrow();
    await expect(
      Reflect.apply(
        operations.directoryPort.searchUsers.bind(operations.directoryPort),
        undefined,
        [{ query: 'member', requestContext: rawContext, trustedSessionToken: 'session-secret' }],
      ),
    ).rejects.toThrow();
  ...
  it('rejects an extra-field context before overview fetch and records one standalone event', async () => {
    ...
    await expect(
      Reflect.apply(port.getOverview.bind(port), undefined, [
        { requestContext: aliasedContext, trustedSessionToken: 'session-secret' },
      ]),
    ).rejects.toThrow();
  ...
  it('rejects an extra-field context before user-operation fetch and records one standalone event', async () => {
    ...
    await expect(
      Reflect.apply(ports.directoryPort.searchUsers.bind(ports.directoryPort), undefined, [
        { query: 'member', requestContext: aliasedContext, trustedSessionToken: 'session-secret' },
      ]),
    ).rejects.toThrow();
  ...
  it('never spreads an invalid context into a safe telemetry event', () => {
    const aliasedContext = { correlationId, traceId, secret: 'must-not-pass' };
    expect(() => {
      Reflect.apply(createSafeTelemetryEvent, undefined, [
        'overview.read',
        'UPSTREAM_FAILURE',
        aliasedContext,
      ]);
  ...
  it('uses a safely projected event when an unstable context accompanies a denied user operation', async () => {
    const capture = captureTelemetry();
    let ownKeysReads = 0;
    const unstableContext = new Proxy(
      { correlationId, traceId },
      {
        ownKeys(target) {
          ownKeysReads += 1;
    ...
    await expect(
      Reflect.apply(ports.detailPort.getUserDetail, ports.detailPort, [
        {
          requestContext: unstableContext,
          trustedSessionToken: 'session-secret',
          userId: 'invalid-user-id',
        },
      ]),
    ).rejects.not.toThrow('Invalid safe telemetry event');
  ...
  it.each([
    ...
    const capture = captureTelemetry();
    expect(() => {
      recordSafeTelemetry(capture.telemetry, event as never);
    }).toThrow('Invalid safe telemetry event');
    expect(capture.events).toEqual([]);
  });
  ...
  it.each([
    ...
    await expect(
      port.getOverview({ requestContext, trustedSessionToken: 'session-secret' }),
    ).rejects.toThrow();
  ...
  it.each([
    [
      ...
      (ports: ReturnType<typeof createHttpUserOperationPorts>) =>
    ...
    [
      ...
      'operations.user.detail-read',
      (ports: ReturnType<typeof createHttpUserOperationPorts>) =>
        ports.detailPort.getUserDetail({
          requestContext,
          trustedSessionToken: 'session-secret',
          userId,
        }),
    ...
    [
      ...
      (ports: ReturnType<typeof createHttpUserOperationPorts>) =>
        ports.statusPort.requestStatusChange({
    ...
    [
      ...
      (ports: ReturnType<typeof createHttpUserOperationPorts>) =>
        ports.adjustmentPort.getEligibleApprovers({
          dataScope: 'ALL',
          requestContext,
          trustedSessionToken: 'session-secret',
          userId,
    ...
    [
      ...
      (ports: ReturnType<typeof createHttpUserOperationPorts>) =>
        ports.adjustmentPort.previewAdjustment?.({
          approverId,
          audit: { idempotencyKey: intentId },
          direction: 'CREDIT',
          points: 1n,
    ...
    [
      ...
      (ports: ReturnType<typeof createHttpUserOperationPorts>) =>
        ports.adjustmentPort.submitAdjustmentRequest({
          approverId,
          audit: { idempotencyKey: intentId },
          direction: 'CREDIT',
          points: 1n,
    ...
    [
      ...
      (ports: ReturnType<typeof createHttpUserOperationPorts>) =>
        ports.exportPort.requestCsvExport({
          audit: { idempotencyKey: intentId },
          reason: 'manual review',
          requestContext,
          scope: 'ALL',
    ...
    'records one malformed-response event for the %s parser',
    async (_label, operation, invoke) => {
      const requests: Headers[] = [];
      const capture = captureTelemetry();
      const ports = createHttpUserOperationPorts(
        {
        ...
        },
        {
          fetchImpl: async (_input, init) => {
            ...
            return Response.json(
              {},
              { status: operation === 'operations.user.status-change' ? 202 : 200 },
            );
          },
          telemetry: capture.telemetry,
        },
      ...
      expect(JSON.stringify(capture.events)).not.toMatch(
        /13800138000|session-secret|previewToken|manual review/u,
      );
    },
  );
  ...
  it.each([
    [
      'iam.config',
      (telemetry: ReturnType<typeof captureTelemetry>['telemetry']) =>
        createHttpAdminAuthPort({ apiUrl: '', kmsIdentityReference: '' }, { telemetry }),
    ],
    [
      'overview.config',
      (telemetry: ReturnType<typeof captureTelemetry>['telemetry']) =>
        createHttpOverviewPort({ apiUrl: '', kmsIdentityReference: '' }, { telemetry }),
    ],
    [
      'operations.config',
      (telemetry: ReturnType<typeof captureTelemetry>['telemetry']) =>
        createHttpUserOperationPorts({ apiUrl: '', kmsIdentityReference: '' }, { telemetry }),
    ],
  ])('records one standalone context for %s', (operation, construct) => {
    const capture = captureTelemetry();

    expect(() => {
      construct(capture.telemetry);
    }).toThrow();
