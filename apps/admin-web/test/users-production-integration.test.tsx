...
vi.mock('next/headers', () => ({
  cookies: async () => ({
    get: (name: string) =>
      name === '__Host-admin_session' && serverHeaders.sessionToken
        ? { value: serverHeaders.sessionToken }
        : undefined,
  }),
}));
...
describe('production users route, shell, actions, and HTTP adapter integration', () => {
  it('fails closed on a phone-like upstream cursor before production markup or pagination', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => Response.json({ items: [], nextCursor: 'cursor_13800138000' })),
    );
    serverHeaders.sessionToken = await signAdminSession(
      {
        dataScope: 'OWN',
    ...
    const { container } = render(
      await renderSecureLayout(await renderUsersPage({ query: 'member' })),
    );
  ...
  it.each([
    'member-138/0013/8000',
    `member-138\u200b0013\u200b8000`,
    'member-x%ZZ%31%33%38%30%30%31%33%38%30%30%30',
    'member-١٣٨٠٠١٣٨٠٠٠',
  ])(
    ...
    async (displayName) => {
      vi.stubGlobal(
        'fetch',
        vi.fn(async () => Response.json({ items: [{ ...row, displayName }], nextCursor: null })),
      );
      serverHeaders.sessionToken = await signAdminSession(
        {
          dataScope: 'OWN',
  ...
  it('sanitizes every browser parameter before production markup, hrefs, or the directory GET', async () => {
    ...
    for (const anchor of container.querySelectorAll('a'))
      expect(anchor.getAttribute('href')).not.toMatch(/13800138000|１３８００１３８０００/u);
  ...
  it('keeps plaintext phone inside only the authorized exact lookup POST body', async () => {
    ...
    const fetchImpl: typeof fetch = async (input, init) => {
      ...
      if (url.includes('/exact-phone-lookups'))
        return Response.json({
          expiresAt: new Date(now + 60_000).toISOString(),
          items: [row],
          searchHandle: 'internal_exact_handle_1234567890',
        });
      if (url.includes('/csv-export-requests'))
        return Response.json({
          auditRecordId: '0198f7a4-c6d4-7b39-8a4e-73af0c1d2e3f',
          downloadUrl: 'https://download.example.invalid/exact.csv',
          expiresAt: new Date(now + 60_000).toISOString(),
        });
    ...
    serverHeaders.sessionToken = await signAdminSession(
      {
        dataScope: 'ASSIGNED',
        expiresAt: Date.now() + 60_000,
        permissions: ['users:read', 'users:phone-exact', 'users:export'],
        sessionInstanceId,
        subjectId,
      },
      sessionSigningKey,
    ...
    expect(screen.getByRole('link', { name: '下一页' })).toHaveAttribute(
      'href',
      '/users?query=member&cursor=safe-next-page',
    );
    expect(container.innerHTML).toContain('138****8000');
    expect(container.innerHTML).not.toContain('13800138000');

    const exact = screen.getByRole('region', { name: '受保护的精确手机号查询' });
    fireEvent.change(within(exact).getByRole('textbox', { name: '精确手机号' }), {
    ...
    serverHeaders.sessionToken = await signAdminSession(
      {
        ...
        expiresAt: Date.now() + 60_000,
        permissions: ['users:read', 'users:phone-exact', 'users:export'],
        sessionInstanceId: '0198f7a4-c6db-7b39-8a4e-73af0c1d2e3f',
        subjectId,
      },
      sessionSigningKey,
    );
    ...
    expect(directoryRequest).toMatchObject({
      method: 'GET',
      url: 'https://operations.example.invalid/v1/admin/users?query=member',
    });
    expect(exactRequest).toMatchObject({
      method: 'POST',
      url: 'https://operations.example.invalid/v1/admin/users/exact-phone-lookups',
      body: JSON.stringify({ phone: '13800138000', scope: 'ASSIGNED' }),
    });
    expect(exportRequest?.url).not.toContain('13800138000');
    expect(JSON.parse(exportRequest?.body ?? '{}')).toMatchObject({
      searchHandle: 'internal_exact_handle_1234567890',
      scope: 'ASSIGNED',
    });
