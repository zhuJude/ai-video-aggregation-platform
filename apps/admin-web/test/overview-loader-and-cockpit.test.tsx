...
const configuredView: OverviewView = {
  datasets: [
    {
      id: 'operations', label: '用户与点数', sourceTimestamp, status: 'READY',
      measures: [
    ...
    {
      id: 'tasks', label: '任务与队列', sourceTimestamp, status: 'PARTIAL', warning: '队列统计延迟',
      measures: [
    ...
    {
      id: 'finance', label: '财务', sourceTimestamp, status: 'STALE', warning: '成本源等待刷新',
      measures: [
        { currency: 'CNY', id: 'income', label: '收入', minorUnits: '1000000' },
        { currency: 'CNY', id: 'provider-cost', label: '供应商成本', minorUnits: '450000' },
        { currency: 'CNY', direction: 'CREDIT', id: 'gross-margin', label: '毛利', minorUnits: '550000' },
    ...
    {
      id: 'supplier-risk', label: '供应商与风险', sourceTimestamp, status: 'EMPTY', warning: '当前范围没有支付异常',
      measures: [
    ...
    {
      id: 'upstream-error', label: '独立故障源', status: 'ERROR', reason: 'UPSTREAM_FAILURE', measures: [],
    },
  ],
};
...
describe('overview server loader', () => {
  it('authorizes overview reads and passes the trusted session to the configured authoritative port', async () => {
    const token = await signAdminSession({ dataScope: 'ALL', expiresAt: Date.now() + 60_000, permissions: ['overview:read'], subjectId: '0198f7a4-c6d2-7b39-8a4e-73af0c1d2e3f' , sessionInstanceId: "0198f7a4-c6da-7b39-8a4e-73af0c1d2e3f"}, signingKey);
    let received: unknown;
    const port: OverviewPort = { async getOverview(input) { received = input; return configuredView; } };

    await expect(loadOverviewView({ context: { sessionToken: token, signingKey }, port })).resolves.toEqual(configuredView);
  ...
  it('formats a huge validated minor-unit amount in the UI without Number precision loss', () => {
    ...
    render(<OverviewCockpit datasets={[{ id: 'finance', label: '财务', measures: [{ currency: 'CNY', id: 'income', label: '收入', minorUnits: huge }], sourceTimestamp: '2026-08-31T08:00:00.000Z', status: 'READY' } as never]} />);
  ...
  it('formats an explicit DEBIT gross margin as negative without accepting a signed numeric scalar', () => {
    render(<OverviewCockpit datasets={[{ id: 'finance', label: '财务', measures: [{ currency: 'CNY', direction: 'DEBIT', id: 'gross-margin', label: '毛利', minorUnits: '5500' }], sourceTimestamp, status: 'READY' }]} />);
  ...
  it('denies missing overview permission before invoking the port', async () => {
    ...
    let calls = 0;
    const port: OverviewPort = { async getOverview() { calls += 1; return configuredView; } };

    await expect(loadOverviewView({ context: { sessionToken: token, signingKey }, port })).rejects.toMatchObject({ code: 'FORBIDDEN' });
    expect(calls).toBe(0);
  });
});
