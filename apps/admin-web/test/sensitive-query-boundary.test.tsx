...
describe('single sensitive-query boundary', () => {
  ...
  it.each([
    ...
    'detects exact, wrapped, international, separated, and full-width phone-like values: %s',
    (value) => {
      expect(containsSensitivePhoneLikeValue(value)).toBe(true);
    },
  );

  it.each(unicodePhoneDigits)(
    'maps every Unicode Nd script before detecting a phone: %s',
    (value) => {
  ...
  it.each([
    ...
    '普通😀文本',
    'cafe\u0301',
  ])('does not reject harmless malformed or bare percent text: %s', (value) => {
  ...
  it('drops unknown parameters, removes malformed known values, and reports a safe rejection without echoing input', () => {
    const result = sanitizeUsersSearchParams({
      cursor: 'bad cursor',
      future: 'prefix-13800138000',
      query: 'member',
      status: 'PENDING',
    });
...
describe('controlled ordinary user search', () => {
  ...
  it.each([
    ...
    '138%0A0013%0D8000',
    'x%ZZ%31%33%38%30%30%31%33%38%30%30%30',
  ])('blocks phone-like text before it can enter router history: %s', (value) => {
  ...
  it('constructs a GET URL only after a normal search value passes the shared detector', () => {
    ...
    expect(screen.getByRole('button', { name: '服务端查询' }).closest('form')).toHaveAttribute(
      'method',
      'post',
    );
    fireEvent.change(screen.getByRole('textbox', { name: /用户名/u }), {
      target: { value: 'member' },
    });
    fireEvent.click(screen.getByRole('button', { name: '服务端查询' }));
...
describe('unauthenticated return-location minimization', () => {
  it.each([
    ...
    'canonicalizes an invalid or sensitive user detail pathname before authentication: %s',
    async (path) => {
      ...
      expect(decodeURIComponent(location)).toBe(
        'https://admin.ai-video.internal/users?notice=sensitive-query-removed',
      );
  ...
  it.each(['0198f7a4-c6d1-7b39-8a4e-73af0c1d2e3f', '0198F7A4-C6D1-7B39-8A4E-73AF0C1D2E3F'])(
    'preserves a valid UUIDv7 user detail return path: %s',
    async (userId) => {
      const response = await proxy(
        new NextRequest(`https://admin.ai-video.internal/users/${userId}`),
      );
      expect(response.status).toBe(307);
      expect(decodeURIComponent(response.headers.get('location') ?? '')).toBe(
        `https://admin.ai-video.internal/login?next=/users/${userId}`,
      );
    },
  );
  ...
  it('allows a valid uppercase UUIDv7 detail path after canonicalization and authorization', async () => {
    ...
    try {
      const response = await proxy(
        new NextRequest(
          'https://admin.ai-video.internal/users/0198F7A4-C6D1-7B39-8A4E-73AF0C1D2E3F',
  ...
  it.each(['/users/', '/users/0198f7a4-c6d1-7b39-8a4e-73af0c1d2e3f/'])(
    ...
    async (path) => {
      ...
      expect(response.headers.get('location')).toBe(
        `https://admin.ai-video.internal${path.slice(0, -1)}`,
      );
    },
  );
  ...
  it.each([
    ...
    'sanitizes a recursively encoded login return location before the public-route exemption: %s',
    async (path) => {
      const response = await proxy(new NextRequest(`https://admin.ai-video.internal${path}`));
      expect(response.status).toBe(307);
  ...
  it('sanitizes a forbidden users request and a detail unknown parameter before authorization', async () => {
    ...
    try {
      const forbidden = await proxy(
        new NextRequest('https://admin.ai-video.internal/users?tag=member-%2B86%20138-0013-8000', {
          headers: { cookie: `__Host-admin_session=${sessionToken}` },
        }),
      );
      expect(forbidden.status).toBe(307);
      expect(decodeURIComponent(forbidden.headers.get('location') ?? '')).toBe(
        'https://admin.ai-video.internal/users?notice=sensitive-query-removed',
      );
      const detail = await proxy(
        new NextRequest(
          'https://admin.ai-video.internal/users/0198f7a4-c6d1-7b39-8a4e-73af0c1d2e3f?future=prefix-１３８００１３８０００',
          { headers: { cookie: `__Host-admin_session=${sessionToken}` } },
        ),
      );
  ...
  it('immediately canonicalizes an authenticated sensitive URL without echoing it', async () => {
    ...
    try {
      const response = await proxy(
        new NextRequest('https://admin.ai-video.internal/users?query=member&tag=vip-13800138000', {
          headers: { cookie: `__Host-admin_session=${sessionToken}` },
        }),
      );
      ...
      expect(location).toBe(
        'https://admin.ai-video.internal/users?query=member&notice=sensitive-query-removed',
      );
      expect(location).not.toContain('13800138000');
    } finally {
      delete process.env.ADMIN_SESSION_SIGNING_KEY;
