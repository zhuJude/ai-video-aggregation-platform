...
vi.mock('../lib/user-view-loaders', async (importOriginal) => {
  ...
  return { ...actual, REGISTRATION_SOURCES: ['WEB'], SPENDING_TIERS: ['HIGH'], USER_STATUSES: ['ACTIVE', 'SUSPENDED', 'CLOSED'], loadUsersView: mocks.loadUsersView };
});
vi.mock('../lib/http-user-operation-port', () => ({ createHttpUserOperationPorts: () => ({ directoryPort: {} }) }));
vi.mock('../app/(secure)/users/actions', () => ({ lookupExactPhoneAction: vi.fn(), requestUsersCsvExportAction: vi.fn() }));

import UsersPage from '../app/(secure)/users/page';

const baseView = { canExport: false, canUseExactPhone: false, filters: {}, items: [], nextCursor: null };
...
describe('UsersPage exact-phone privacy boundary', () => {
  it('never echoes a phone-shaped GET query and directs the actor to the protected lookup', async () => {
    ...
    const { container } = render(await UsersPage({ searchParams: Promise.resolve({ exactPhone: 'true', query: '13800138000' }) }));
    ...
    expect(mocks.loadUsersView).toHaveBeenCalledWith({ filters: {}, query: '' }, expect.any(Object));
  });
  ...
  it.each([
    ...
    { future: '１３８００１３８０００' },
  ])('sanitizes sensitive values from every known or future search parameter before rendering or loading', async (searchParams) => {
    mocks.loadUsersView.mockResolvedValue(baseView);
    const { container } = render(await UsersPage({ searchParams: Promise.resolve(searchParams) }));
  ...
  it('keeps ordinary pagination URLs free of exact-phone flags and renders frozen statuses', async () => {
    mocks.loadUsersView.mockResolvedValue({ ...baseView, items: [{ displayName: '已关闭用户', id: '0198f7a4-c6d1-7b39-8a4e-73af0c1d2e3f', phoneMasked: '138****8000', status: 'CLOSED' }], nextCursor: 'next_1' });
    render(await UsersPage({ searchParams: Promise.resolve({ query: 'member' }) }));

    const statusFilter = screen.getByRole('combobox', { name: '账户状态' });
    expect(statusFilter).toHaveTextContent('ACTIVE'); expect(statusFilter).toHaveTextContent('SUSPENDED'); expect(statusFilter).toHaveTextContent('CLOSED'); expect(statusFilter).not.toHaveTextContent('PENDING');
