...
describe('HTTP overview port', () => {
  ...
  it('fails closed on malformed response shapes without coercing authoritative values', async () => {
    ...
    expect(events).toEqual([
      { ...requestContext, operation: 'overview.read', reason: 'MALFORMED_RESPONSE' },
    ]);
  });
  ...
  it.each([
    ...
    'measure-label',
    'warning',
    'symbol-separated-warning',
    'control-separated-warning',
  ])('rejects phone-like overview render text in %s', async (field) => {
    const malformedPayload = payload();
    ...
    if (field === 'measure-label') {
      const measure = operations.measures[0];
      if (!measure) throw new Error('missing measure fixture');
      measure.label = '+86 138-0013-8000';
    }
    if (field === 'warning')
      Object.assign(operations, { status: 'STALE', warning: '告警-１３８００１３８０００' });
    if (field === 'symbol-separated-warning')
      Object.assign(operations, { status: 'STALE', warning: '告警-138😀0013🚀8000' });
    if (field === 'control-separated-warning')
      Object.assign(operations, { status: 'STALE', warning: '告警-138\u00000013\ue0008000' });
    const port = createHttpOverviewPort(environment, {
      fetchImpl: async () => response(malformedPayload),
    });
  ...
  it.each(
    [
      ...
      'payment-anomalies',
      'service-alerts',
    ].flatMap((metricId) => ['1.5', '-1', '01', 'NaN'].map((value) => [metricId, value] as const)),
  ...
  it.each(
    ['success-rate', 'supplier-failure-rate', 'gross-margin-rate', 'repeat-purchase-rate'].flatMap(
      (metricId) =>
        ['-1.00%', '101.00%', '01.00%', '1.000%', 'NaN%', '1%'].map(
          (value) => [metricId, value] as const,
        ),
    ),
  ...
  it.each([
    ...
    const duration = tasks?.measures.find(
      (candidate) => candidate.id === 'average-generation-duration',
    );
  ...
  it.each(
    ['operations', 'tasks', 'finance', 'supplier-risk'].flatMap((datasetId) =>
      ['2026-08-31T16:00:00+08:00', '2026-08-31T08:00:00', '2026-02-30T08:00:00Z'].map(
        (timestamp) => [datasetId, timestamp] as const,
      ),
    ),
  )(
  ...
  it('fails closed on response body timeout and emits sanitized telemetry', async () => {
    ...
    await expect(
      port.getOverview({ requestContext, trustedSessionToken: 'sensitive-session' }),
    ).rejects.toThrow();
