...
test('fixture rejects unsupported methods for every known endpoint', async ({ request }) => {
  const knownPaths = [
    ...
    '/v1/admin/reporting/overview',
    '/v1/admin/providers',
    `/v1/admin/providers/${IDS.provider}`,
    '/v1/admin/models',
    `/v1/admin/models/${IDS.model}/capabilities`,
    `/v1/admin/models/${IDS.model}/capabilities/commands`,
    ...
    '/admin/tasks',
    `/admin/tasks/${IDS.task}`,
    `/admin/tasks/${IDS.task}/raw`,
    `/v1/admin/users/${IDS.user}/detail`,
    `/v1/admin/users/${IDS.user}/authorization-scope`,
    `/v1/admin/users/${IDS.user}/eligible-approvers`,
    `/v1/admin/users/${IDS.user}/wallet-adjustment-previews`,
    `/v1/admin/users/${IDS.user}/wallet-adjustment-requests`,
    ...
    `/admin/finance/reconciliation/${IDS.reconciliation}`,
    ...
    '/admin/content',
    `/admin/content/${IDS.content}`,
    `/admin/content/${IDS.content}/operations`,
    '/admin/tickets',
    `/admin/tickets/${IDS.ticket}`,
    `/admin/tickets/${IDS.ticket}/transitions`,
    '/admin/iam',
  ];
...
test('fixture rejects missing or incorrect mutation identity headers', async ({ request }) => {
  ...
  const wrongCatalogIdentity = await request.post(
    fixtureUrl(`/v1/admin/models/${IDS.model}/capabilities/commands`),
    {
  ...
  const wrongUserIdentity = await request.post(
    fixtureUrl(`/v1/admin/users/${IDS.user}/eligible-approvers`),
    {
      data: { scope: 'ALL' },
...
test('fixture rejects malformed bodies for every known mutation without changing state', async ({
  request,
}) => {
...
test('fixture binds wallet approval audit identity to its header and existing request state', async ({
  request,
}) => {
  await request.post(fixtureUrl('/__reset'));
