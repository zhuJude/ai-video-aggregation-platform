...
export function UserDirectorySearchForm({
  initialFilters,
  initialQuery,
}: Readonly<{ initialFilters: UserFilters; initialQuery: string }>) {
  const router = useRouter();
  const [query, setQuery] = useState(initialQuery);
  const [status, setStatus] = useState(initialFilters.status ?? '');
  const [tag, setTag] = useState(initialFilters.tag ?? '');
  const [registrationSource, setRegistrationSource] = useState(
    initialFilters.registrationSource ?? '',
  );
  ...
  function submit(event: SyntheticEvent<HTMLFormElement>) {
    ...
    if (values.some(containsSensitivePhoneLikeValue)) {
      setQuery('');
      setTag('');
      setError('手机号相关查询不得进入地址栏，请使用受保护的精确手机号查询');
      return;
    }
    setError(undefined);
    ...
    <form method="post" onSubmit={submit}>
      ...
      <Field label="注册来源">
        <Select
          ...
          onChange={(event) => {
            setRegistrationSource(event.target.value);
          }}
        >
          <option value="">全部</option>
          {REGISTRATION_SOURCES.map((value) => (
            <option key={value} value={value}>
              {value}
            </option>
          ))}
      ...
      <Field label="消费等级">
        <Select
          ...
          value={spendingTier}
          onChange={(event) => {
            setSpendingTier(event.target.value);
          }}
        >
          <option value="">全部</option>
          {SPENDING_TIERS.map((value) => (
            <option key={value} value={value}>
              {value}
            </option>
          ))}
