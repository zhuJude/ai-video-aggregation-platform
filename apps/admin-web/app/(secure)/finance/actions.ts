...
export async function createCompensationRequest(form: FormData) {
  await createCompensationRequestAction({ port: createHttpFinanceOperationsPort() })(
    domainFormData(form),
  );
}

export async function approveCompensationRequest(form: FormData) {
  await createCompensationApprovalAction({ port: createHttpFinanceOperationsPort() })(
    domainFormData(form),
  );
}

export async function transitionInvoice(form: FormData) {
  await createInvoiceTransitionAction({ port: createHttpFinanceOperationsPort() })(
    domainFormData(form),
  );
}

export async function executeOrderOperation(form: FormData) {
  await createOrderOperationAction({ port: createHttpFinanceOperationsPort() })(
    domainFormData(form),
  );
}
