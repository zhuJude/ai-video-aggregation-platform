...
} from '../../../lib/http-user-operation-port';
...
import {
  ExactPhoneLookup,
  type ExactPhoneLookupProps,
} from '../../../components/exact-phone-lookup';
...
export async function renderUsersPage(
  ...
  dependencies: UsersPageDependencies = {},
) {
  const sanitized = sanitizeUsersSearchParams(params);
  const query = sanitized.params.query ?? '';
  const cursor = sanitized.params.cursor;
  const filters = {
    registrationSource: sanitized.params.registrationSource ?? '',
    spendingTier: sanitized.params.spendingTier ?? '',
    status: sanitized.params.status ?? '',
    tag: sanitized.params.tag ?? '',
  };
  let view: Awaited<ReturnType<typeof loadUsersView>> | undefined;
  let error: string | undefined;
  try {
    view = await loadUsersView(
      {
        query,
        filters: {
          ...(filters.status ? { status: filters.status } : {}),
          ...(filters.tag ? { tag: filters.tag } : {}),
          ...(filters.registrationSource ? { registrationSource: filters.registrationSource } : {}),
          ...(filters.spendingTier ? { spendingTier: filters.spendingTier } : {}),
        },
        ...(cursor ? { cursor } : {}),
      },
      {
        ...(dependencies.context ? { context: dependencies.context } : {}),
        directoryPort: dependencies.directoryPort ?? createHttpUserOperationPorts().directoryPort,
      },
    );
  } catch {
    error = '用户目录服务不可用或当前查询未获授权';
  }
  if (sanitized.rejected || sanitized.params.notice === SENSITIVE_QUERY_NOTICE)
    error = '敏感查询参数已移除，请使用受保护的精确手机号查询';
  const filterQuery = usersSearchParamsToString({
    query,
    ...(filters.status ? { status: filters.status } : {}),
    ...(filters.tag ? { tag: filters.tag } : {}),
    ...(filters.registrationSource ? { registrationSource: filters.registrationSource } : {}),
  ...
  return (
    <section aria-labelledby="users-heading">
      <Title2 id="users-heading">用户检索</Title2>
      <UserDirectorySearchForm initialFilters={view?.filters ?? {}} initialQuery={query} />
      {view?.canUseExactPhone ? (
        <ExactPhoneLookup
          canExport={view.canExport}
          onExport={dependencies.onExport ?? requestUsersCsvExportAction}
          onLookup={dependencies.onLookup ?? lookupExactPhoneAction}
        />
      ) : null}
      {showNormalResults && view ? (
        <CsvExportForm
          canExport={view.canExport}
      ...
      {showNormalResults && view?.items.length === 0 ? <Text>没有匹配用户</Text> : null}
      {showNormalResults && view ? (
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHeaderCell>注册来源</TableHeaderCell>
              <TableHeaderCell>消费等级</TableHeaderCell>
            </TableRow>
          </TableHeader>
          <TableBody>
            {view.items.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell>{item.registrationSource ?? '-'}</TableCell>
                <TableCell>{item.spendingTier ?? '-'}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
      ) : null}
      {showNormalResults && view?.nextCursor ? (
        <Link
          href={`/users?${filterQuery}${filterQuery ? '&' : ''}cursor=${encodeURIComponent(view.nextCursor)}`}
        >
          下一页
        </Link>
      ) : null}
