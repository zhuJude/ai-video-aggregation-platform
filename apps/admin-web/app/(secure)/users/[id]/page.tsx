...
function DetailError({ code }: Readonly<{ code: 'INVALID_ID' | 'FORBIDDEN' | 'NOT_FOUND' | 'DEPENDENCY' }>) {
  const content = {
  ...
  return <section aria-labelledby="user-detail-error-heading"><Title2 id="user-detail-error-heading">{title}</Title2><Text role="alert">{message}</Text></section>;
}
