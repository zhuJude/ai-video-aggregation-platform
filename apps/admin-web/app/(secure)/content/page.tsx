      ...
      <form method="get">
        <Field label="内容状态">
          <Select defaultValue={dependencies.status ?? ''} name="status">
            ...
            <option value="DRAFT_VALIDATED">已校验</option>
            <option value="PUBLISHED">已发布</option>
          </Select>
        </Field>
        <Button type="submit">筛选内容</Button>
      </form>
      <ContentConsole
        onOperation={operateContent}
        onSave={saveContentDraft}
        onValidate={validateContentDraft}
        permissions={authorization.claims.permissions}
        view={view}
      />
      {view.nextCursor ? (
        <Link href={`/content?cursor=${encodeURIComponent(view.nextCursor)}`}>下一页</Link>
      ) : null}
    </>
  );
}
