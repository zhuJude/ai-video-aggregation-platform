...
describe('production operations composition', () => {
  it('wires Prisma publication/outbox, verified admin auth and HTTP controllers', () => {
    const result = createOperationsSupportingServices({
      ...
      userTokenVerifier: { verify: () => Promise.reject(new Error('not called during composition')) },
      assetAuthorizationGateway: { findAvailableAsset: () => Promise.resolve(null), reserveSupportUploadSession: () => Promise.resolve({ outcome: 'NOT_FOUND' }), finalizeSupportUploadSession: () => Promise.resolve(), releaseSupportUploadSession: () => Promise.resolve(), lookupSupportUploadReservation: () => Promise.resolve(null) },
