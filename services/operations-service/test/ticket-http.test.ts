...
describe('ticket HTTP authorization and DTOs', () => {
  it('derives the ticket owner from a verified raw bearer token and rejects body owner fields', async () => {
    ...
    const rejected = await http.handle({
      ...
      body: { subject: '问题', body: '描述', ownerId: OTHER_USER },
    });
  ...
  it('returns the same TICKET_NOT_FOUND response for missing and cross-user tickets', async () => {
    ...
    const created = await first.http.handle({
      method: 'POST',
      path: '/v1/tickets',
      headers: auth('user-token'),
      body: { subject: '问题', body: '描述' },
    ...
    const crossUser = await other.http.handle({
      method: 'GET',
      path: `/v1/tickets/${ticketId}`,
      headers: auth('other-token'),
    });
    const missing = await other.http.handle({
      method: 'GET',
      path: '/v1/tickets/01990f24-2ba2-7000-8000-000000000099',
      headers: auth('other-token'),
    });
    expect(crossUser).toMatchObject({
      status: 404,
      body: { code: 'TICKET_NOT_FOUND', retryable: false },
    });
  ...
  it('requires action-specific admin permissions and never accepts identity headers', async () => {
    ...
    const denied = await http.handle({
      method: 'POST',
      path: `/admin/v1/tickets/${ticket.id}/claim`,
      headers: { authorization: 'Bearer admin-token', 'x-admin-id': ADMIN },
      body: { expectedRevision: ticket.revision },
    });
    expect(denied).toMatchObject({ status: 403, body: { code: 'FORBIDDEN' } });
    const headerOnly = moduleFor({ userId: null }).http;
    expect(
      await headerOnly.handle({
        method: 'GET',
        path: '/v1/tickets',
  ...
  it('allows VIEWER reads but rejects support POST actions even with the action permission', async () => {
    const { http } = moduleFor({
      userId: USER,
      adminRole: 'VIEWER',
      adminPermissions: ['operations:tickets:read', 'operations:tickets:claim'],
    });
    const created = await http.handle({
      method: 'POST',
      path: '/v1/tickets',
      headers: auth('user-token'),
      body: { subject: 'viewer', body: 'initial' },
    });
    const ticket = created.body as { id: string; revision: number };
    await expect(
      http.handle({
        method: 'GET',
        path: `/admin/v1/tickets/${ticket.id}`,
  ...
  it('requires idempotency-key for messages and returns no internal notes from user GET', async () => {
    const { http } = moduleFor({
      userId: USER,
      adminPermissions: ['operations:tickets:claim', 'operations:tickets:note'],
    });
    const created = await http.handle({
      method: 'POST',
      path: '/v1/tickets',
    ...
    expect(
      await http.handle({
        method: 'POST',
        path: `/v1/tickets/${ticket.id}/messages`,
        headers: auth('user-token'),
        body: { body: '追问', expectedRevision: ticket.revision },
      }),
    ).toMatchObject({ status: 400, body: { code: 'INVALID_REQUEST' } });
    const claimed = await http.handle({
      method: 'POST',
      path: `/admin/v1/tickets/${ticket.id}/claim`,
      headers: auth('admin-token'),
      body: { expectedRevision: ticket.revision },
    ...
    await http.handle({
      method: 'POST',
      path: `/admin/v1/tickets/${ticket.id}/internal-notes`,
      headers: auth('admin-token'),
      body: { body: 'secret-internal-note', expectedRevision: revision },
    });
    const userView = await http.handle({
      method: 'GET',
  ...
  it('authenticates only verified UUIDv7 user claims', async () => {
    const authenticator = new JwksUserAuthenticator(
      {
        verify: () =>
          Promise.resolve({
            sub: USER,
    ...
    await expect(authenticator.authenticate({ headers: auth('signed-token') })).resolves.toEqual({
      userId: USER,
    });
    const wrongUse = new JwksUserAuthenticator(
      {
        verify: () =>
          Promise.resolve({
            sub: USER,
  ...
  it.each(['P1001', 'P2024', 'ECONNRESET', 'SECRET_DATABASE_FAILURE'])(
    ...
    async (code) => {
      ...
      const response = await moduleFor({ userId: USER, ticket }).http.handle({
        method: 'GET',
        path: '/v1/tickets',
        headers: { ...auth('user-token'), 'x-trace-id': TRACE },
      });
      expect(response).toEqual({
        status: 500,
        headers: { 'x-trace-id': TRACE },
  ...
  it('exposes only a whitelisted cross-service domain error code', async () => {
    const ticket = new TicketService({
      ...
      feedbackSubjectAuthorization: {
        assertTaskOwned: () =>
          Promise.reject(
            Object.assign(new Error('private task lookup details'), {
    ...
    const response = await moduleFor({ userId: USER, ticket }).http.handle({
      ...
      body: {
        kind: 'FAILED_TASK',
        taskId: '01990f24-2ba2-7000-8000-000000000004',
    ...
    expect(response).toEqual({
      ...
      body: {
        code: 'FEEDBACK_SUBJECT_NOT_FOUND',
        message: 'FEEDBACK_SUBJECT_NOT_FOUND',
        traceId: TRACE,
        retryable: false,
      },
    });
...
function moduleFor(input: {
  ...
  const http = new OperationsHttpModule({
    ...
    ticket,
    userAuthenticator: {
      authenticate: () => Promise.resolve(input.userId === null ? null : { userId: input.userId }),
    },
    adminAuthenticator: {
      authenticate: () =>
        Promise.resolve({
          ...
          permissions: input.adminPermissions ?? [],
        }),
    },
  });
  return { http, ticket };
