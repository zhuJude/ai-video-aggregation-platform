...
describe('secure support ticket workflow', () => {
  it('returns TICKET_NOT_FOUND when one user reads another user ticket', async () => {
    ...
    await expect(service.get(ticket.id, OTHER_USER)).rejects.toMatchObject({
  ...
  it('requires an agent public reply before resolving', async () => {
    ...
    await expect(
      service.resolve(ticket.id, claimed.revision, ADMIN, context),
    ).rejects.toMatchObject({ code: 'TICKET_REPLY_REQUIRED' });
  });
  ...
  it('allows reopen exactly seven days after resolution but rejects one millisecond later and never reopens CLOSED', async () => {
    ...
    const replied = await service.reply(
      ticket.id,
      { body: '已处理', expectedRevision: claimed.revision },
      ADMIN,
      'reply-1',
      context,
    );
    ...
    const repliedAgain = await service.reply(
      ticket.id,
      { body: '再次处理', expectedRevision: reopened.revision },
      ADMIN,
      'reply-2',
      context,
    );
    const resolvedAgain = await service.resolve(
      ticket.id,
      repliedAgain.ticket.revision,
      ADMIN,
      context,
    );
    clock.set('2026-09-15T00:00:00.001Z');
    await expect(
      service.reopen(ticket.id, resolvedAgain.revision, USER, context),
    ).rejects.toMatchObject({ code: 'TICKET_REOPEN_WINDOW_EXPIRED' });
    const closed = await service.close(ticket.id, resolvedAgain.revision, ADMIN, context);
    await expect(service.reopen(ticket.id, closed.revision, USER, context)).rejects.toMatchObject({
  ...
  it('requires a new public agent reply in every reopened resolution cycle', async () => {
    ...
    const firstReply = await service.reply(
      created.id,
      { body: 'cycle zero reply', expectedRevision: claimed.revision },
      ADMIN,
      'cycle-zero',
      context,
    );
    ...
    await expect(
      service.resolve(created.id, reopened.revision, ADMIN, context),
    ).rejects.toMatchObject({ code: 'TICKET_REPLY_REQUIRED' });
    const secondReply = await service.reply(
      created.id,
      { body: 'cycle one reply', expectedRevision: reopened.revision },
      ADMIN,
      'cycle-one',
      context,
    );
    await expect(
      service.resolve(created.id, secondReply.ticket.revision, ADMIN, context),
    ).resolves.toMatchObject({ status: 'RESOLVED', resolutionCycle: 1 });
  });
  ...
  it('keeps internal notes out of user DTOs and every outbox event', async () => {
    ...
    await service.addInternalNote(
      ticket.id,
      { body: secret, expectedRevision: claimed.revision },
      ADMIN,
    );
    ...
    expect((await service.getForAdmin(ticket.id)).internalNotes).toEqual([
  ...
  it('authorizes attachments from trusted asset ownership or a live one-use support session', async () => {
    ...
    await expect(
      service.create(
        { subject: '附件', body: '我的', attachments: [{ assetId: ASSET }] },
        USER,
        context,
      ),
      ...
      service.create(
        {
          subject: '附件',
          body: '授权',
          attachments: [{ assetId: OTHER_ASSET, supportUploadSessionId: SESSION }],
        },
        USER,
        context,
      ),
    ...
    ).rejects.toMatchObject({ code: 'SUPPORT_UPLOAD_SESSION_USED' });
      ...
      service.create(
        {
          subject: '附件',
          body: '过期',
          attachments: [{ assetId: OTHER_ASSET, supportUploadSessionId: expiredSession }],
        },
        USER,
        context,
      ),
    ).rejects.toMatchObject({ code: 'SUPPORT_UPLOAD_SESSION_EXPIRED' });
  });
  ...
  it('uses CAS for concurrent claims and deduplicates concurrent replies by idempotency key', async () => {
    ...
    const replies = await Promise.all([
      service.reply(
        ticket.id,
        { body: '公开回复', expectedRevision: claimed.revision },
        claimed.assigneeId,
        'same-key',
        context,
      ),
    ...
    expect(
      (await service.get(ticket.id, USER)).messages.filter(
    ...
    await expect(
      service.reply(
        ticket.id,
        { body: '不同内容', expectedRevision: replies[0].ticket.revision },
        claimed.assigneeId,
        'same-key',
        context,
      ),
  ...
  it('rejects illegal state transitions and stale revisions', async () => {
    ...
    await expect(service.resolve(ticket.id, ticket.revision, ADMIN, context)).rejects.toMatchObject(
      { code: 'TICKET_INVALID_TRANSITION' },
    );
    const claimed = await service.claim(ticket.id, ticket.revision, ADMIN, context);
    await expect(service.claim(ticket.id, ticket.revision, ADMIN, context)).rejects.toMatchObject({
      code: 'TICKET_REVISION_CONFLICT',
    });
    await expect(service.close(ticket.id, claimed.revision, ADMIN, context)).rejects.toMatchObject({
      code: 'TICKET_INVALID_TRANSITION',
    });
  });
  ...
  it('creates feedback only for the authenticated user and owned task, without putting content in events', async () => {
    const feedbackSubjects: FeedbackSubjectAuthorizationPort = {
      assertTaskOwned: (taskId, userId) =>
        taskId === TASK && userId === USER
          ? Promise.resolve()
          : Promise.reject(
              Object.assign(new Error('FEEDBACK_SUBJECT_NOT_FOUND'), {
                code: 'FEEDBACK_SUBJECT_NOT_FOUND',
              }),
            ),
    };
    const { service, repository } = fixture(undefined, undefined, feedbackSubjects);
    const created = await service.createFeedback(
      { kind: 'FAILED_TASK', taskId: TASK, content: '失败详情是隐私', rating: 2 },
      USER,
      context,
    );
    expect(created.userId).toBe(USER);
    await expect(service.getFeedback(created.id, OTHER_USER)).rejects.toMatchObject({
      code: 'FEEDBACK_NOT_FOUND',
    });
    await expect(
      service.createFeedback(
        { kind: 'FAILED_TASK', taskId: TASK, content: '冒用', rating: 6 },
        OTHER_USER,
        context,
      ),
  ...
  it('rejects malformed ticket EventEnvelope ids, types and request context before persistence', () => {
    const base = {
      ...
      type: 'operations.ticket.created.v1',
      version: 1,
      occurredAt: '2026-09-01T00:00:00.000Z',
      traceId: '0123456789abcdef0123456789abcdef',
      correlationId: '01990f24-2ba2-7000-8000-000000000002',
      producer: 'operations-service',
      data: {},
    };
    expect(() => {
      validateTicketEventEnvelope({ ...base, id: 'bad-id' });
    }).toThrow(expect.objectContaining({ code: 'INVALID_REQUEST_CONTEXT' }));
    expect(() => {
      validateTicketEventEnvelope({ ...base, type: 'BAD_TYPE' });
    }).toThrow(expect.objectContaining({ code: 'INVALID_REQUEST_CONTEXT' }));
    expect(() => {
      validateTicketEventEnvelope({ ...base, traceId: 'bad-trace' });
    }).toThrow(expect.objectContaining({ code: 'INVALID_REQUEST_CONTEXT' }));
    expect(() => {
      validateTicketEventEnvelope({ ...base, correlationId: 'bad-correlation' });
    }).toThrow(expect.objectContaining({ code: 'INVALID_REQUEST_CONTEXT' }));
  });
});
...
function fixture(
  ...
  const service = new TicketService({
    repository,
    attachmentAuthorization: attachments,
    feedbackSubjectAuthorization: feedbackSubjects,
    now: () => clock.now(),
    id: idGenerator(),
  });
...
class FakeAttachmentAuthorization implements AttachmentAuthorizationPort {
  ...
  readonly #sessions = new Map<
    string,
    { assetId: string; ownerId: string; expiresAt: Date; used: boolean }
  >();
  constructor(private readonly clock: MutableClock) {}
  own(assetId: string, ownerId: string): void {
    this.#owners.set(assetId, ownerId);
  }
  session(id: string, assetId: string, ownerId: string, expiresAt: string): void {
    this.#sessions.set(id, { assetId, ownerId, expiresAt: new Date(expiresAt), used: false });
  }
  authorize(input: {
    assetId: string;
    ownerId: string;
    ...
    const session =
      input.supportUploadSessionId === undefined
        ? undefined
        : this.#sessions.get(input.supportUploadSessionId);
    if (
      session === undefined ||
      ...
      return Promise.reject(
        Object.assign(new Error('ATTACHMENT_NOT_AUTHORIZED'), {
          code: 'ATTACHMENT_NOT_AUTHORIZED',
        }),
      );
    if (session.used)
      return Promise.reject(
        Object.assign(new Error('SUPPORT_UPLOAD_SESSION_USED'), {
          code: 'SUPPORT_UPLOAD_SESSION_USED',
        }),
      );
    if (session.expiresAt < this.clock.now())
      return Promise.reject(
        Object.assign(new Error('SUPPORT_UPLOAD_SESSION_EXPIRED'), {
          code: 'SUPPORT_UPLOAD_SESSION_EXPIRED',
        }),
      );
    session.used = true;
