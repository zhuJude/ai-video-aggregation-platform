...
describe('production attachment authorization', () => {
  it('accepts an available asset only after a server-side owner lookup', async () => {
    ...
    await expect(
      new SecureAttachmentAuthorization(gateway).reserve({
        operationId: 'op',
  ...
  it('atomically consumes and validates a restricted support session for a non-owned asset', async () => {
    ...
    const reservation = await authorization.reserve({
      operationId: 'op',
      remoteOperationId: REMOTE_OPERATION,
      generation: 0,
      fence: FENCE,
      ...
      assetId: ASSET,
      ownerId: USER,
      supportUploadSessionId: SESSION,
      now: NOW,
    });
  ...
  it('returns the opaque reservation so the service can persist it before validation', async () => {
    ...
    ).resolves.toMatchObject({ ownerId: OTHER, purpose: 'GENERAL_UPLOAD' });
  });
});
