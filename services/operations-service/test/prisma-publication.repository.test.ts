...
describe('PrismaPublicationRepository transaction boundary', () => {
  it('rolls back a version transition when the outbox insert fails', async () => {
    ...
    await expect(
      repository.transact({ kind: 'all' }, (state) => {
        ...
        state.outbox.push({
          id: '01990f24-2ba2-7000-8000-000000000011',
          type: 'operations.package.published.v1',
          version: 1,
          occurredAt: '2026-08-31T12:00:00.000Z',
          traceId: '0123456789abcdef0123456789abcdef',
          correlationId: '01990f24-2ba2-7000-8000-000000000012',
  ...
  it('uses a targeted public-package query without loading purchases or outbox history', async () => {
    ...
    const client = {
      $transaction: <T>(work: (tx: Record<string, unknown>) => Promise<T>) =>
        work({
          rechargePackageVersion: {
            findMany: () => {
              queried.push('packages');
              return Promise.resolve([]);
            },
          },
          rechargePackagePurchaseSnapshot: forbidden('purchases'),
          contentEntry: forbidden('entries'),
          contentVersion: forbidden('content'),
          bannerPlacement: forbidden('placements'),
          bannerSlotRevision: forbidden('slots'),
          helpCategory: forbidden('categories'),
          publicSystemSettingVersion: forbidden('settings'),
          featureFlagVersion: forbidden('flags'),
          outboxEvent: forbidden('outbox'),
        }),
    };
    const repository = new PrismaPublicationRepository(client as never);
  ...
  it('does not let direct repository writes bypass public-setting validation', async () => {
    ...
    await expect(
      repository.transact({ kind: 'all' }, (state) =>
        state.settings.set('01990f24-2ba2-7000-8000-000000000020', {
          id: '01990f24-2ba2-7000-8000-000000000020',
          settingKey: 'cdn.publicBaseUrl',
          version: 1,
          revision: 0,
          status: 'DRAFT',
          basePublishedVersionId: null,
          publicValue: { url: 'https://user:password@cdn.example.com' },
          kmsSecretReferenceId: null,
          createdBy: '01990f24-2ba2-7000-8000-000000000002',
  ...
  it('requires every direct banner mutation to advance its aggregate revision', async () => {
    ...
    await expect(
      repository.transact({ kind: 'all' }, (state) =>
        state.placements.set('01990f24-2ba2-7000-8000-000000000021', {
          id: '01990f24-2ba2-7000-8000-000000000021',
          contentVersionId: '01990f24-2ba2-7000-8000-000000000022',
          slot: 'HOME_HERO',
          sortOrder: 0,
          activeFrom: null,
          activeUntil: null,
...
function fakePrisma(options: { failOutbox?: boolean; conflict?: boolean }) {
  const rows: Record<string, Array<Record<string, unknown>>> = {
    rechargePackageVersion: [
      {
        ...
        packageId: '01990f24-2ba2-7000-8000-000000000010',
        version: 1,
        revision: 0,
        status: 'DRAFT',
        name: 'old',
    ...
    rechargePackagePurchaseSnapshot: [],
    contentEntry: [],
    contentVersion: [],
    bannerPlacement: [],
    bannerSlotRevision: [],
    helpCategory: [],
    publicSystemSettingVersion: [],
    featureFlagVersion: [],
    outboxEvent: [],
  };
  return {
    ...
    async $transaction<T>(work: (tx: Record<string, unknown>) => Promise<T>): Promise<T> {
      ...
      const tx = Object.fromEntries(
        Object.keys(draft).map((name) => [name, delegate(draft[name] ?? [], name, options)]),
      );
...
function delegate(
  ...
  name: string,
  options: { failOutbox?: boolean; conflict?: boolean },
) {
  return {
    findMany: () => Promise.resolve(structuredClone(rows)),
    create: ({ data }: { data: Record<string, unknown> }) => {
      if (name === 'outboxEvent' && options.failOutbox)
        return Promise.reject(new Error('OUTBOX_WRITE_FAILED'));
    ...
    updateMany: ({
      ...
      if (name === 'rechargePackageVersion' && options.conflict)
