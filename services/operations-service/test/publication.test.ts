...
describe('recharge package publication', () => {
  it('keeps published versions immutable when a later draft is edited', async () => {
    ...
    const draft = await service.createPackageDraftFrom(
      published.id,
      { bonusPoints: 10_000n },
      OWNER,
    );
  ...
  it('stores an immutable package snapshot for a purchase', async () => {
    ...
    const later = await service.createPackageDraftFrom(
      published.id,
      { bonusPoints: 99_000n },
      OWNER,
    );
  ...
  it('rejects non-v7 purchase ids and non-CNY package currency', async () => {
    ...
    const published = await publishedPackage(service);
    await expect(
      service.capturePackagePurchase('purchase-not-uuidv7', published.id, USER),
    ).rejects.toMatchObject({ code: 'INVALID_PURCHASE_ID' });
    await expect(
      service.capturePackagePurchase(
        '01990f24-2ba2-7000-8000-000000000024',
        published.id,
        '01990f24-2ba2-4000-8000-000000000099',
      ),
  ...
  it('rejects non-v7 actor identities before persistence', () => {
    ...
    expect(() =>
      service.createPackageDraft(
        ...
        '01990f24-2ba2-4000-8000-000000000002',
      ),
  ...
  it('re-reads a concurrent purchase insert and returns or rejects it idempotently', async () => {
    const existing = {
      purchaseId: '01990f24-2ba2-7000-8000-000000000021',
      packageVersionId: '01990f24-2ba2-7000-8000-000000000010',
      buyerId: USER,
      name: 'stable',
      amountMinor: 100n,
      currency: 'CNY',
    ...
    const state = emptyPublicationState();
    state.purchases.set(existing.purchaseId, existing);
    ...
    await expect(
      service.capturePackagePurchase(existing.purchaseId, existing.packageVersionId, USER),
    ).resolves.toEqual(existing);
    await expect(
      service.capturePackagePurchase(
        existing.purchaseId,
        existing.packageVersionId,
        '01990f24-2ba2-7000-8000-000000000099',
      ),
  ...
  it('rejects purchasing a draft or retired package', async () => {
    ...
    await expect(
      service.capturePackagePurchase('01990f24-2ba2-7000-8000-000000000022', draft.id, USER),
    ...
    await expect(
      service.capturePackagePurchase('01990f24-2ba2-7000-8000-000000000023', retired.id, USER),
    ).rejects.toMatchObject({ code: 'PACKAGE_NOT_ACTIVE' });
  });
  ...
  it('publishes the package and its outbox event atomically', async () => {
    ...
    await expect(service.publishPackage(draft.id, draft.revision, ADMIN)).rejects.toThrow(
      'OUTBOX_WRITE_FAILED',
    );
...
describe('CMS publication', () => {
  it('enforces the draft-published-retired state machine dynamically', async () => {
    ...
    await expect(service.retirePackage(draft.id, draft.revision, ADMIN)).rejects.toMatchObject({
      code: 'VERSION_NOT_PUBLISHED',
    });
    const published = await service.publishPackage(draft.id, draft.revision, ADMIN);
    await expect(
      service.updatePackageDraft(published.id, published.revision, { name: 'forbidden' }, ADMIN),
    ).rejects.toMatchObject({ code: 'PUBLISHED_VERSION_IMMUTABLE' });
    const retired = await service.retirePackage(published.id, published.revision, ADMIN);
    await expect(service.retirePackage(retired.id, retired.revision, ADMIN)).rejects.toMatchObject({
      code: 'VERSION_NOT_PUBLISHED',
    });
  });
  it('publishes, rolls back by publishing a new copy, and never mutates old versions', async () => {
    ...
    const firstDraft = await service.createContentDraft(
      entry.id,
      {
        title: 'v1',
        summary: 'first',
    ...
    const secondDraft = await service.createContentDraft(
      entry.id,
      {
        title: 'v2',
        summary: 'second',
    ...
    const context = {
      ...
      correlationId: '01990f24-2ba2-7000-8000-000000000090',
    };
  ...
  it('uses optimistic concurrency for draft edits and publication', async () => {
    ...
    const draft = await service.createContentDraft(
      entry.id,
      {
        title: 'Start',
        summary: '',
    ...
    const edited = await service.updateContentDraft(
      draft.id,
      draft.revision,
      { title: 'Edited' },
      ADMIN,
    );
    await expect(service.publishContent(draft.id, draft.revision, ADMIN)).rejects.toMatchObject({
      code: 'VERSION_CONFLICT',
    });
    expect((await service.publishContent(edited.id, edited.revision, ADMIN)).status).toBe(
      'PUBLISHED',
    );
  });
  ...
  it('reorders banners with a collection revision and returns only active placements', async () => {
    ...
    for (const key of ['one', 'two']) {
      ...
      ids.push(
        (
          await service.placeBanner(
            {
              contentVersionId: version.id,
              slot: 'HOME_HERO',
              sortOrder: ids.length,
              expectedRevision: ids.length,
              activeFrom: null,
              activeUntil: null,
            },
            ADMIN,
    ...
    expect(
      (await service.listPublicBanners('HOME_HERO', NOW)).map((item) => item.placement.id),
    ).toEqual([...ids].reverse());
    await expect(service.reorderBanners('HOME_HERO', 2, ids, ADMIN)).rejects.toMatchObject({
      code: 'VERSION_CONFLICT',
    });
  });
  ...
  it('serializes concurrent banner placement and reorder with one collection revision', async () => {
    ...
    for (const key of ['existing', 'new']) {
      ...
      const draft = await service.createContentDraft(
        entry.id,
        { title: key, summary: '', bodyHtml: '<p>x</p>', sortOrder: 0 },
        ADMIN,
      );
    ...
    if (existingVersion === undefined || newVersion === undefined)
      throw new Error('missing banner versions');
    const first = await service.placeBanner(
      {
  ...
  it('returns public announcements and help content only after publication', async () => {
    const { service } = setup();
    const announcement = await service.createContentEntry(
      { kind: 'ANNOUNCEMENT', key: 'notice' },
      ADMIN,
    );
    const draft = await service.createContentDraft(
      announcement.id,
    ...
    expect(
      (await service.listPublicContent('ANNOUNCEMENT', NOW)).map((item) => item.title),
    ).toEqual(['Notice']);
  });

  it('rejects invalid content windows and category links outside active HELP content', async () => {
    const { service } = setup();
    const inactive = await service.createHelpCategory(
      { key: 'disabled', name: 'Disabled', sortOrder: 0, active: false },
      ADMIN,
    );
    const help = await service.createContentEntry({ kind: 'HELP', key: 'hidden-help' }, ADMIN);
    await expect(
      service.createContentDraft(
        help.id,
        {
          title: 'hidden',
          summary: '',
          bodyHtml: '<p>x</p>',
          sortOrder: 0,
          helpCategoryId: inactive.id,
        },
        ADMIN,
      ),
    ).rejects.toMatchObject({ code: 'HELP_CATEGORY_NOT_ACTIVE' });
    const announcement = await service.createContentEntry(
      { kind: 'ANNOUNCEMENT', key: 'wrong-category' },
      ADMIN,
    );
    await expect(
      service.createContentDraft(
    ...
    ).rejects.toMatchObject({ code: 'HELP_CATEGORY_NOT_ALLOWED' });
    await expect(
      service.createContentDraft(
        announcement.id,
        {
          ...
          bodyHtml: '<p>x</p>',
          sortOrder: 0,
          activeFrom: new Date('2026-09-02T00:00:00Z'),
          activeUntil: new Date('2026-09-01T00:00:00Z'),
        },
        ADMIN,
      ),
...
describe('publication input safety', () => {
  it.each([
    ...
    expect(() =>
      sanitizeRichText(html, { trustedIframeOrigins: ['https://media.example.cn'] }),
  ...
  it('removes non-allowlisted benign markup and permits a trusted iframe', () => {
    expect(
      ...
      '<p>ok <strong>bold</strong></p>note<iframe src="https://media.example.cn/embed/1" allowfullscreen></iframe>',
    );
  });
  ...
  it('stores either public configuration or a KMS reference, never a raw secret', async () => {
    const { service } = setup();
    await expect(
      service.createSystemSettingDraft(
        {
          key: 'wechat.apiSecret',
          publicValue: 'plain-secret',
        },
        ADMIN,
      ),
    ).rejects.toMatchObject({ code: 'RAW_SECRET_FORBIDDEN' });
    await expect(
      service.createSystemSettingDraft(
        {
          key: 'wechat.apiSecret',
          kmsSecretReferenceId: 'not-a-kms-reference',
        },
        ADMIN,
      ),
    ).rejects.toMatchObject({ code: 'INVALID_KMS_REFERENCE' });
    const draft = await service.createSystemSettingDraft(
      {
        key: 'wechat.apiSecret',
        kmsSecretReferenceId: 'kms://operations/wechat-api-secret',
      },
      ADMIN,
    );
  ...
  it('publishes settings with a revision guard and keeps the published value immutable', async () => {
    const { service } = setup();
    const draft = await service.createSystemSettingDraft(
      { key: 'site.publicConfig', publicValue: { theme: 'dark' } },
      ADMIN,
    );
    const edited = await service.updateSystemSettingDraft(
      draft.id,
      draft.revision,
      { publicValue: { theme: 'light' } },
      ADMIN,
    );
    const published = await service.publishSystemSetting(edited.id, edited.revision, ADMIN);
    await expect(
      service.updateSystemSettingDraft(
        published.id,
        published.revision,
        { publicValue: { theme: 'other' } },
        ADMIN,
      ),
    ).rejects.toMatchObject({ code: 'PUBLISHED_VERSION_IMMUTABLE' });
    expect(await service.getSystemSettingVersion(published.id)).toMatchObject({
      status: 'PUBLISHED',
      publicValue: { theme: 'light' },
    });
  });
  ...
  it('applies strict per-key public configuration schemas and recursively rejects secrets', async () => {
    ...
    await expect(
      service.createSystemSettingDraft(
        { key: 'site.publicConfig', publicValue: { theme: 'dark', apiSecret: 'leak' } },
        ADMIN,
      ),
    ).rejects.toMatchObject({ code: 'INVALID_PUBLIC_CONFIGURATION' });
    await expect(
      service.createSystemSettingDraft(
        { key: 'site.publicConfig', publicValue: { theme: 'dark', nested: { token: 'leak' } } },
        ADMIN,
      ),
    ).rejects.toMatchObject({ code: 'INVALID_PUBLIC_CONFIGURATION' });
    await expect(
      service.createSystemSettingDraft(
        {
          key: 'site.publicConfig',
          publicValue: { theme: 'dark', privateKey: '-----BEGIN PRIVATE KEY-----' },
        },
        ADMIN,
      ),
    ).rejects.toMatchObject({ code: 'INVALID_PUBLIC_CONFIGURATION' });
    await expect(
      service.createSystemSettingDraft(
        { key: 'site.publicConfig', publicValue: { theme: 'dark', unknown: true } },
        ADMIN,
      ),
    ).rejects.toMatchObject({ code: 'INVALID_PUBLIC_CONFIGURATION' });
    await expect(
      service.createSystemSettingDraft(
        {
          key: 'site.publicConfig',
          publicValue: { theme: 'light', locale: 'zh-CN', registrationEnabled: true },
        },
        ADMIN,
      ),
  ...
  it.each([
    ...
    const { service } = setup();
    await expect(
      service.createSystemSettingDraft({ key: 'cdn.publicBaseUrl', publicValue: { url } }, ADMIN),
    ).rejects.toMatchObject({ code: 'INVALID_PUBLIC_CONFIGURATION' });
    await expect(
      service.createSystemSettingDraft(
        { key: 'cdn.publicBaseUrl', publicValue: { url: 'https://cdn.example.com/assets' } },
        ADMIN,
      ),
  ...
  it('publishes feature flags with optimistic concurrency and transaction outbox', async () => {
    const { repository, service } = setup();
    const draft = await service.createFeatureFlagDraft(
      { flagKey: 'new-workbench', enabled: false, rules: { percent: 0 } },
      ADMIN,
    );
    repository.failNextOutboxWrite();
    await expect(service.publishFeatureFlag(draft.id, draft.revision, ADMIN)).rejects.toThrow(
      'OUTBOX_WRITE_FAILED',
    );
    expect(await service.getFeatureFlagVersion(draft.id)).toMatchObject({
      status: 'DRAFT',
      revision: 0,
    });
    const results = await Promise.allSettled([
