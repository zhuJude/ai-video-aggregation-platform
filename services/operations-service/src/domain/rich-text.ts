...
const ALLOWED_TAGS = new Set([
  ...
  'h1', 'h2', 'h3', 'h4', 'a', 'img', 'code', 'pre', 'iframe',
]);
const VOID_TAGS = new Set(['br', 'img']);
const DANGEROUS_ELEMENT = /<\s*\/?\s*(?:script|style|object|embed|svg|math|form|input|button|link|meta|base)\b/i;
...
export function sanitizeRichText(html: string, policy: RichTextPolicy): string {
  if (typeof html !== 'string' || html.length > 200_000) throw new PublicationError('INVALID_RICH_TEXT');
  if (DANGEROUS_ELEMENT.test(html) || EVENT_HANDLER.test(html)) throw new PublicationError('UNSAFE_RICH_TEXT');

  const trustedOrigins = new Set(policy.trustedIframeOrigins.map(normalizeOrigin));
  return html.replace(/<\/?([a-zA-Z][\w-]*)([^>]*)>/g, (raw, rawName: string, rawAttributes: string) => {
    ...
    if (name === 'a') {
      ...
      if (src === null) throw new PublicationError('UNSAFE_RICH_TEXT');
      let origin: string;
      try { origin = new URL(src).origin; } catch { throw new PublicationError('UNSAFE_RICH_TEXT'); }
...
function normalizeOrigin(value: string): string {
  try { return new URL(value).origin; } catch { throw new PublicationError('INVALID_IFRAME_ORIGIN'); }
}
