...
export class OperationsOutboxDispatcher {
  ...
  async dispatch(eventId: string): Promise<void> {
    ...
    const event = await this.store.claim(
      eventId,
      claimedAt,
      claimToken,
    ...
    try {
      const { id, type, version, occurredAt, traceId, correlationId, causationId, producer, data } =
        event;
      ...
      await this.store.failed(
        ...
        claimToken,
        failedAt,
        new Date(failedAt.getTime() + backoffMs(event.attempts)),
      );
      throw error;
    }
  }
  ...
  async dispatchDue(limit = 100): Promise<number> {
    ...
    for (const id of ids) {
      try {
        ...
        /* each row retains its durable retry schedule */
      }
    }
    return ids.length;
  }
...
type RawOutboxRecord = Omit<OutboxRecord, 'occurredAt' | 'causationId'> & {
