...
async function loadAllState(tx: PrismaTransactionClient): Promise<PublicationState> {
  const [
    ...
    tx.rechargePackageVersion.findMany(),
    tx.rechargePackagePurchaseSnapshot.findMany(),
    tx.contentEntry.findMany(),
    tx.contentVersion.findMany(),
    tx.bannerPlacement.findMany(),
    tx.bannerSlotRevision.findMany(),
    tx.helpCategory.findMany(),
    tx.publicSystemSettingVersion.findMany(),
    tx.featureFlagVersion.findMany(),
    tx.outboxEvent.findMany(),
  ]);
  return {
    packages: mapById(packages as RechargePackageVersion[]),
    purchases: new Map(
      (purchases as RechargePackagePurchaseSnapshot[]).map((row) => [
        row.purchaseId,
        structuredClone(row),
      ]),
    ),
    ...
    slotRevisions: new Map(
      (slotRevisions as Array<{ slot: string; revision: number }>).map((row) => [
        row.slot,
        row.revision,
      ]),
    ),
    ...
    outbox: (
      outbox as Array<
        Omit<OperationsOutboxEvent, 'occurredAt' | 'causationId'> & {
...
async function loadPackageScope(
  tx: PrismaTransactionClient,
  state: PublicationState,
  scope: Extract<PublicationScope, { kind: 'package' }>,
): Promise<void> {
  if (scope.versionId !== undefined) {
    const target = (await tx.rechargePackageVersion.findUnique({
      where: { id: scope.versionId },
    })) as RechargePackageVersion | null;
    if (target !== null)
      addRows(
        state.packages,
        (await tx.rechargePackageVersion.findMany({
          where: { packageId: target.packageId },
        })) as RechargePackageVersion[],
      );
  } else if (scope.publicAt !== undefined) {
    addRows(
      state.packages,
      (await tx.rechargePackageVersion.findMany({
        where: activeWhere(scope.publicAt),
        orderBy: [{ sortOrder: 'asc' }, { version: 'asc' }],
      })) as RechargePackageVersion[],
    );
  }
  if (scope.purchaseId !== undefined) {
    const purchase = (await tx.rechargePackagePurchaseSnapshot.findUnique({
      where: { purchaseId: scope.purchaseId },
    })) as RechargePackagePurchaseSnapshot | null;
...
async function loadContentScope(
  tx: PrismaTransactionClient,
  state: PublicationState,
  scope: Extract<PublicationScope, { kind: 'content' }>,
): Promise<void> {
  ...
  if (scope.versionId !== undefined) {
    ...
    if (target !== null) {
      const versions = (await tx.contentVersion.findMany({
        where: { entryId: target.entryId },
      })) as ContentVersion[];
      addRows(state.content, versions);
      const entry = (await tx.contentEntry.findUnique({
        where: { id: target.entryId },
      })) as ContentEntry | null;
      if (entry !== null) state.entries.set(entry.id, structuredClone(entry));
      for (const row of versions)
        if (row.helpCategoryId !== null) categoryIds.add(row.helpCategoryId);
    }
  }
  if (scope.entryId !== undefined) {
    const entry = (await tx.contentEntry.findUnique({
      where: { id: scope.entryId },
    })) as ContentEntry | null;
    if (entry !== null) state.entries.set(entry.id, structuredClone(entry));
    const versions = (await tx.contentVersion.findMany({
      where: { entryId: scope.entryId },
    })) as ContentVersion[];
    addRows(state.content, versions);
    for (const row of versions)
      if (row.helpCategoryId !== null) categoryIds.add(row.helpCategoryId);
  ...
  if (scope.entryKind !== undefined && scope.entryKey !== undefined) {
    addRows(
      ...
      (await tx.contentEntry.findMany({
        where: { kind: scope.entryKind, key: scope.entryKey },
      })) as ContentEntry[],
    );
  }
  if (scope.publicKind !== undefined && scope.publicAt !== undefined) {
    const versions = (await tx.contentVersion.findMany({
      where: { entry: { kind: scope.publicKind }, ...activeWhere(scope.publicAt) },
      orderBy: [{ sortOrder: 'asc' }, { version: 'asc' }],
    })) as ContentVersion[];
    addRows(state.content, versions);
    ...
    if (entryIds.length > 0)
      addRows(
        state.entries,
        (await tx.contentEntry.findMany({ where: { id: { in: entryIds } } })) as ContentEntry[],
      );
    for (const row of versions)
      if (row.helpCategoryId !== null) categoryIds.add(row.helpCategoryId);
  }
  if (scope.slot !== undefined) {
    const placements = (await tx.bannerPlacement.findMany({
      where: {
        slot: scope.slot,
        ...(scope.publicAt === undefined ? {} : windowWhere(scope.publicAt)),
      },
    })) as BannerPlacement[];
    addRows(state.placements, placements);
    const slotRow = (await tx.bannerSlotRevision.findUnique({ where: { slot: scope.slot } })) as {
      slot: string;
      revision: number;
    } | null;
    ...
    const versions =
      versionIds.length === 0
        ? []
        : ((await tx.contentVersion.findMany({
  ...
  if (scope.categoryId !== undefined && scope.categoryId !== null)
    categoryIds.add(scope.categoryId);
  if (scope.categoryKey !== undefined)
    addRows(
      state.categories,
      (await tx.helpCategory.findMany({ where: { key: scope.categoryKey } })) as HelpCategory[],
    );
  if (categoryIds.size > 0)
    addRows(
      state.categories,
      (await tx.helpCategory.findMany({
        where: { id: { in: [...categoryIds] } },
      })) as HelpCategory[],
    );
}

async function loadSettingScope(
  tx: PrismaTransactionClient,
  state: PublicationState,
  scope: Extract<PublicationScope, { kind: 'setting' }>,
): Promise<void> {
  if (scope.versionId !== undefined) {
    const target = (await tx.publicSystemSettingVersion.findUnique({
      where: { id: scope.versionId },
    })) as PublicSystemSettingVersion | null;
    if (target !== null)
      addRows(
        state.settings,
        (await tx.publicSystemSettingVersion.findMany({
          where: { settingKey: target.settingKey },
        })) as PublicSystemSettingVersion[],
      );
  } else if (scope.settingKey !== undefined)
    addRows(
      state.settings,
      (await tx.publicSystemSettingVersion.findMany({
        where: { settingKey: scope.settingKey },
      })) as PublicSystemSettingVersion[],
    );
}

async function loadFlagScope(
  tx: PrismaTransactionClient,
  state: PublicationState,
  scope: Extract<PublicationScope, { kind: 'flag' }>,
): Promise<void> {
  if (scope.versionId !== undefined) {
    const target = (await tx.featureFlagVersion.findUnique({
      where: { id: scope.versionId },
    })) as FeatureFlagVersion | null;
    if (target !== null)
      addRows(
        state.flags,
        (await tx.featureFlagVersion.findMany({
          where: { flagKey: target.flagKey },
        })) as FeatureFlagVersion[],
      );
  } else if (scope.flagKey !== undefined)
    addRows(
      state.flags,
      (await tx.featureFlagVersion.findMany({
        where: { flagKey: scope.flagKey },
      })) as FeatureFlagVersion[],
    );
}
...
function validateBannerCollectionRevision(before: PublicationState, after: PublicationState): void {
  ...
  for (const [id, row] of after.placements) {
    ...
    if (previous === undefined || !isDeepStrictEqual(previous, row)) {
      ...
      if (previous !== undefined) changedSlots.add(previous.slot);
    }
  }
  for (const [id, row] of before.placements)
    if (!after.placements.has(id)) changedSlots.add(row.slot);
  for (const slot of changedSlots)
    if ((after.slotRevisions.get(slot) ?? 0) !== (before.slotRevisions.get(slot) ?? 0) + 1)
...
async function persistCreated<T>(
  delegate: any,
  before: Map<string, T>,
  after: Map<string, T>,
): Promise<void> {
  for (const [id, row] of after) {
    ...
    if (previous === undefined) await delegate.create({ data: row });
      ...
      throw new PublicationError('IMMUTABLE_RECORD_MODIFIED');
  }
}
...
async function persistSlotRevisions(
  delegate: any,
...
async function persistPlacements(
  delegate: any,
  before: Map<string, BannerPlacement>,
  after: Map<string, BannerPlacement>,
): Promise<void> {
  for (const [id, row] of after) {
    const previous = before.get(id);
    if (previous === undefined) await delegate.create({ data: row });
      ...
      const changed = await delegate.updateMany({
        where: { id, slot: previous.slot },
        data: mutableData(row),
      });
      if (changed.count !== 1) throw new PublicationError('VERSION_CONFLICT');
    }
  }
...
function settingData(row: PublicSystemSettingVersion): Record<string, unknown> {
  validateSystemSetting(
    row.settingKey,
    row.kmsSecretReferenceId === null
      ? { publicValue: row.publicValue }
      : { kmsSecretReferenceId: row.kmsSecretReferenceId },
  );
