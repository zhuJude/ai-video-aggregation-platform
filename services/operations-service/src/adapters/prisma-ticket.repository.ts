...
async function loadState(
  tx: PrismaTicketTransactionClient,
  scope: TicketScope,
): Promise<TicketState> {
  ...
  if (scope.kind === 'all') {
    ...
    addRows(
      state.compensations,
      (await tx.supportUploadCompensation.findMany()) as SupportUploadCompensation[],
    );
    return state;
  }
    ...
    if (scope.ticketId !== undefined) {
      const ticket =
        scope.userId === undefined
          ...
          : ((await tx.ticket.findFirst({
              where: { id: scope.ticketId, userId: scope.userId },
            })) as Ticket | null);
      if (ticket !== null) {
        ...
        if (scope.userId === undefined)
          addRows(
            state.notes,
            (await tx.ticketInternalNote.findMany({
      ...
      addRows(
        ...
        (await tx.ticket.findMany({
          where: { userId: scope.userId, ...pageWhere(scope.page?.before) },
          orderBy: [{ createdAt: 'desc' }, { id: 'desc' }],
          ...(scope.page === undefined ? {} : { take: scope.page.limit }),
        })) as Ticket[],
      );
    }
    return state;
  ...
  if (scope.feedbackId !== undefined) {
    const row =
      scope.userId === undefined
        ? ((await tx.feedback.findUnique({
            where: { id: scope.feedbackId },
          })) as FeedbackRow | null)
        : ((await tx.feedback.findFirst({
            where: { id: scope.feedbackId, userId: scope.userId },
          })) as FeedbackRow | null);
...
async function loadBindings(
  ...
  addRows(
    state.bindings,
    (await tx.supportUploadBinding.findMany({
      where: { id: { in: [...bindingIds] } },
    })) as SupportUploadBinding[],
  );
}
...
async function loadMessages(
  ...
  const attachments =
    ids.length === 0
      ? []
      : ((await tx.ticketMessageAttachment.findMany({
          where: { messageId: { in: ids } },
        })) as MessageAttachmentRow[]);
  for (const row of messages) {
    state.messages.set(row.id, {
      ...structuredClone(row),
      attachments: attachments.filter((item) => item.messageId === row.id).map(stripMessageId),
    });
  }
}
...
async function loadFeedbackRows(
  ...
  const attachments =
    ids.length === 0
      ? []
      : ((await tx.feedbackAttachment.findMany({
          where: { feedbackId: { in: ids } },
        })) as FeedbackAttachmentRow[]);
  for (const row of rows)
    state.feedback.set(row.id, {
      ...structuredClone(row),
      attachments: attachments.filter((item) => item.feedbackId === row.id).map(stripFeedbackId),
    });
}
...
async function persistState(
  ...
  for (const [id, row] of after.notes)
    if (!before.notes.has(id)) await tx.ticketInternalNote.create({ data: row });
  ...
  for (const [id, row] of after.bindings) {
    ...
    if (previous === undefined) await tx.supportUploadBinding.create({ data: row });
      ...
      const changed = await tx.supportUploadBinding.updateMany({
        where: {
          id,
          generation: previous.generation,
          remoteOperationId: previous.remoteOperationId,
  ...
  for (const [id, row] of after.compensations) {
    ...
    if (previous === undefined) await tx.supportUploadCompensation.create({ data: row });
      ...
      const changed = await tx.supportUploadCompensation.updateMany({
        where: {
          id,
          status: previous.status,
          claimToken: previous.claimToken,
  ...
  for (const event of after.outbox)
    if (!existingEvents.has(event.id)) await tx.outboxEvent.create({ data: outboxData(event) });
}

async function persistConsumption(
  tx: PrismaTicketTransactionClient,
  attachment: { id: string; assetId: string; supportUploadSessionId: string | null },
  ownerId: string,
  ...
  await tx.supportUploadConsumption.create({
    data: {
      id: attachment.id,
...
function mutableData(row: Ticket): Record<string, unknown> {
...
function stripMessageId(row: MessageAttachmentRow): {
  id: string;
  assetId: string;
  supportUploadSessionId: string | null;
} {
  return { id: row.id, assetId: row.assetId, supportUploadSessionId: row.supportUploadSessionId };
}
function stripFeedbackId(row: FeedbackAttachmentRow): {
  id: string;
  assetId: string;
  supportUploadSessionId: string | null;
} {
...
function mapPrismaError(error: unknown): unknown {
  ...
  if (text.includes('TICKET_INVALID_TRANSITION'))
    return new TicketError('TICKET_INVALID_TRANSITION');
  if (text.includes('TICKET_REPLY_REQUIRED')) return new TicketError('TICKET_REPLY_REQUIRED');
  if (text.includes('TICKET_REOPEN_WINDOW_EXPIRED'))
    return new TicketError('TICKET_REOPEN_WINDOW_EXPIRED');
  if (code === 'P2002' || code === '23505') {
    const target =
      typeof error === 'object' && error !== null && 'meta' in error
        ? JSON.stringify(error.meta)
        : text;
    if (target.includes('operationId') || target.includes('SupportUploadBinding_operationId'))
      return new TicketError('IDEMPOTENCY_CONFLICT');
    if (target.includes('SupportUploadConsumption') || target.includes('sessionId'))
      return new TicketError('SUPPORT_UPLOAD_SESSION_USED');
    return new TicketError('IDEMPOTENCY_CONFLICT');
  }
  return error;
}
