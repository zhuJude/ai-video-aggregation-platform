...
export async function bootstrap(
  ...
  try {
    ...
    const created = await createIamApplication({
      environment,
      lifecycle,
      ...(cloud ? { cloud } : {}),
    });
...
export async function gracefulShutdown(
  ...
  process.stdout.write(
    `${JSON.stringify({ level: 'info', event: 'iam_service_stopping', signal })}\n`,
  );
  await app.close();
}
...
if (typeof script === 'string' && import.meta.url === pathToFileURL(script).href) {
  bootstrap().catch((error: unknown) => {
    ...
    process.stderr.write(
      `${JSON.stringify({ level: 'fatal', event: 'iam_service_start_failed', code })}\n`,
    );
    process.exitCode = 1;
  });
}
