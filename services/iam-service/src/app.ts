...
import {
  KmsAdminAccessTokenIssuer,
  type KmsEdDsaSigner,
} from './adapters/kms-admin-access-token.issuer.js';
...
import {
  ...
  type KmsHmacClient,
} from './adapters/redis-admin-login-throttle.js';
  ...
  if (!lifecycle.has('cloud'))
    lifecycle.register('cloud', async () => {
      ...
      process.stdout.write(
        `${JSON.stringify({ level: 'info', event: 'iam_cloud_resources_closed' })}\n`,
      );
    });
      ...
      metrics,
      {
        recordFailure: () => {
          process.stderr.write(
            `${JSON.stringify({ level: 'error', event: 'pending_session_cleanup_failed', code: 'IAM_PENDING_CLEANUP_FAILED' })}\n`,
          );
        },
      },
    ...
    const auth = new AdminAuthService({
      ...
      cleanupObserver: {
        recordCleanupFailure: (event) => {
          process.stderr.write(
            `${JSON.stringify({ level: 'error', event: 'admin_auth_cleanup_failed', code: event.code, operation: event.operation })}\n`,
          );
        },
      },
    });
...
function unavailableCloudInfrastructure(environment: IamEnvironment): IamCloudInfrastructure {
  ...
  return {
    ...
    recoveryPepperKeyring: {
      current: {
        version: keyVersion(environment.recoveryPepperKeyReference),
        key: randomBytes(32),
      },
      previous: environment.previousRecoveryPepperKeyReferences.map((reference) => ({
        version: keyVersion(reference),
        key: randomBytes(32),
      })),
    },
...
function assertIamCloudInfrastructure(cloud: unknown): asserts cloud is IamCloudInfrastructure {
  if (
    !hasMethod(member(cloud, 'cryptographyClient'), 'encrypt') ||
    !hasMethod(member(cloud, 'cryptographyClient'), 'decrypt') ||
    !hasMethod(member(cloud, 'signer'), 'sign') ||
    !hasMethod(member(cloud, 'signer'), 'verify') ||
    !hasMethod(member(cloud, 'hmacClient'), 'signHmac') ||
    !hasMethod(member(cloud, 'accessVerifier'), 'verify') ||
    !hasMethod(member(cloud, 'bootstrapAuthorizer'), 'authorize') ||
...
function validateRecoveryPepperKeyring(
  ...
  if (
    ...
    expected.some((version, index) => version !== actual[index])
  ) {
