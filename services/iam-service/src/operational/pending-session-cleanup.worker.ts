...
export class PendingSessionCleanupWorker {
  ...
  constructor(
    ...
    if (!Number.isSafeInteger(intervalMs) || intervalMs < 1_000 || intervalMs > 3_600_000) throw stableError('INVALID_PENDING_CLEANUP_INTERVAL');
    if (!Number.isSafeInteger(batchSize) || batchSize < 1 || batchSize > 500) throw stableError('INVALID_PENDING_CLEANUP_BATCH');
  }
  start(): void {
  ...
  runOnce(): Promise<void> {
    ...
    const work = this.repository.cleanupExpiredPendingSessions(this.now(), this.batchSize)
      .then((count) => {
        if (!Number.isSafeInteger(count) || count < 0 || count > this.batchSize) throw stableError('INVALID_PENDING_CLEANUP_RESULT');
      ...
      .catch(async () => {
        ...
        try { await this.observer.recordFailure({ code: 'IAM_PENDING_CLEANUP_FAILED' }); }
        catch { process.emitWarning('IAM_PENDING_CLEANUP_OBSERVER_FAILED', { code: 'IAM_PENDING_CLEANUP_OBSERVER_FAILED' }); }
      });
    const wrapped = work.finally(() => { if (this.current === wrapped) this.current = undefined; });
    this.current = wrapped;
    return wrapped;
  }
  healthy(): boolean { return !this.closed && this.available; }
