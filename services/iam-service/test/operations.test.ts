...
const environment = Object.freeze({
  host: '127.0.0.1',
  port: 39002,
  databaseUrl: 'postgresql://unused:unused@127.0.0.1:1/iam',
  redisUrl: 'redis://127.0.0.1:1/15',
  signingKeyReference: key('jwt'),
  totpKeyReference: key('totp'),
  previousTotpKeyReferences: [],
  previousSigningKeyReferences: [],
  loginHmacKeyReference: key('hmac'),
  previousLoginHmacKeyReferences: [],
  recoveryPepperKeyReference: key('pepper'),
  previousRecoveryPepperKeyReferences: [],
  kmsIdentity: { mode: 'ecs_ram_role' as const, roleName: 'iam-service-role' },
  dummyPasswordHash:
    '$argon2id$v=19$m=65536,p=1,t=3$aWFtLWR1bW15LXNhbHQtdjEh$sS8ky5sVrjEWO/XGr1C11lT6qVhj8IbY+eDsxB3/eys',
  bootstrapProofReference: key('bootstrap'),
  readinessTimeoutMs: 10,
  readinessAbortGraceMs: 10,
  databaseOperationTimeoutMs: 5_000,
  redisOperationTimeoutMs: 5_000,
  eventLoopStallThresholdMs: 250,
  pendingCleanupIntervalMs: 60_000,
...
describe('iam production operations', () => {
  it('strictly parses the environment and rejects access keys', () => {
    const env = {
      IAM_DATABASE_URL: environment.databaseUrl,
      IAM_REDIS_URL: environment.redisUrl,
      IAM_JWT_SIGNING_KMS_KEY_REF: key('jwt'),
      IAM_TOTP_KMS_KEY_REF: key('totp'),
      IAM_LOGIN_HMAC_KMS_KEY_REF: key('hmac'),
      IAM_RECOVERY_PEPPER_KMS_KEY_REF: key('pepper'),
      IAM_DUMMY_PASSWORD_HASH: environment.dummyPasswordHash,
      IAM_BOOTSTRAP_PROOF_KMS_REF: key('bootstrap'),
    ...
    expect(() => parseIamEnvironment({ ...env, ALIBABA_CLOUD_ACCESS_KEY_ID: 'forbidden' })).toThrow(
  ...
  it('reports recovery and fixed-cardinality metrics', async () => {
    ...
    const health = new ServiceHealth(
      [{ name: 'redis', check: () => Promise.resolve(redisUp) }],
      { healthy: () => true },
      10,
      0,
    );
    ...
    metrics.increment('iam_mfa_failures_total');
    metrics.increment('iam_authorization_denials_total');
  ...
  it('exposes operations and fails closed when the KMS SDK wiring is absent', async () => {
    ...
    try {
      ...
      const mfa = await server.inject({
        method: 'POST',
        url: '/v1/admin/auth/mfa/totp',
        payload: { challengeId: 'challenge', token: '123456', deviceName: 'browser' },
      });
    ...
    const cloudFactory = vi.fn(() =>
      Promise.resolve({
        cryptographyClient: {
          ...
          decrypt: () => Promise.resolve('secret'),
        },
        signer: {
          sign: () => Promise.resolve(new Uint8Array(64).fill(3)),
          verify: () => Promise.resolve(true),
        },
        hmacClient: {
          signHmac: ({ data }: { data: Uint8Array }) =>
  ...
  it.skipIf(!process.env['IAM_TEST_DATABASE_URL'])(
    ...
    async () => {
      ...
      const countSessions = async () =>
        Number(
          (
            await observerPrisma.$queryRaw<Array<{ count: bigint }>>`
        ...
        await expect(
          createIamApplication({
            ...
            cloud: {
              ...
              hmacClient: {
                signHmac: ({ data }) => Promise.resolve(createHash('sha256').update(data).digest()),
              },
              recoveryPepperKeyring: {
                current: { version: 'v1', key: new Uint8Array(32).fill(4) },
              },
              accessVerifier: { verify: () => Promise.reject(new Error('unused')) },
              bootstrapAuthorizer: { authorize: () => Promise.resolve(false) },
              kmsReady: () => Promise.resolve(false),
    ...
    async () => {
      ...
      try {
        ...
        expect(response.json()).toEqual({
          status: 'ready',
          checks: { postgres: 'up', redis: 'up', kms: 'up', pending_cleanup: 'up' },
        });
      } finally {
        await app.close();
      }
  ...
  it.skipIf(!process.env['IAM_TEST_DATABASE_URL'] || !process.env['IAM_TEST_REDIS_URL'])(
    ...
    async () => {
      ...
      const adminId = generateUuidV7(),
        ...
        proposedPermissionId = generateUuidV7();
      ...
      const identifier = createHash('sha256')
        .update(new TextEncoder().encode(`iam-service:admin-login-id:v1:${email}`))
        .digest('hex');
      ...
      const cloud = {
        cryptographyClient: {
          encrypt: () => Promise.resolve('kms-ciphertext'),
          decrypt: () => Promise.resolve(secret),
        },
        signer: {
          sign: async () => {
      ...
      try {
        await prisma.permission.create({
          data: { id: task6SentinelId, key: task6SentinelKey, description: 'task6 sentinel' },
        });
        ...
        await prisma.adminUser.create({
          data: {
            id: adminId,
            email,
            passwordHash: await new Argon2idPasswordHasher().hash(password),
            status: 'ACTIVE',
            mfaEnabled: true,
            totpSecretCiphertext: totpEnvelope,
        ...
        const permission = await prisma.permission.upsert({
          ...
          create: {
            id: proposedPermissionId,
            key: 'iam:permissions:read',
            description: 'task6 read',
          },
          update: {},
        });
        ...
        const server = app.getHttpAdapter().getInstance();
        expect((await server.inject({ method: 'GET', url: '/readyz' })).statusCode).toBe(503);
        const passwordStep = await server.inject({
          method: 'POST',
          url: '/v1/admin/auth/password',
        ...
        const mfa = await server.inject({
          method: 'POST',
          url: '/v1/admin/auth/mfa/totp',
          payload: { challengeId, token, deviceName: 'task6-browser' },
        });
        ...
        if (createdPermission)
          await prisma.permission.deleteMany({ where: { id: proposedPermissionId } });
        await prisma.adminUser.deleteMany({ where: { id: adminId } });
        await redis.del(
          buildAdminLoginThrottleKey('iam:admin-login', identifier),
          buildAdminLoginReservationKey('iam:admin-login', identifier),
        );
        await expectTask6FixturesRemoved(prisma, {
          adminId,
          roleId,
        ...
        if (task6SentinelCreated) {
          expect(
            await prisma.permission.findUnique({ where: { id: task6SentinelId } }),
          ).toMatchObject({ key: task6SentinelKey });
...
async function expectTask6FixturesRemoved(
  ...
  fixture: {
    ...
    readonly proposedPermissionId: string;
    readonly createdPermission: boolean;
  },
): Promise<void> {
  ...
  if (fixture.createdPermission)
    expect(await prisma.permission.count({ where: { id: fixture.proposedPermissionId } })).toBe(0);
  expect(
    await prisma.auditEvent.count({
      where: {
        OR: [
          { actorId: fixture.adminId },
          { resourceId: { in: [fixture.adminId, fixture.roleId] } },
        ],
      },
    }),
  ).toBe(0);
