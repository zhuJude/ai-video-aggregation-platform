...
import { AssetOutboxJob, PrismaOutboxDispatcher } from '../src/adapters/prisma-outbox.dispatcher.js';

function makeHarness(status: 'PENDING' | 'FAILED' | 'PUBLISHED', leaseUntil: Date | null = null) {
  const row = { id: 'event-1', eventType: 'asset.imported.v1', payload: { assetId: 'asset-1' }, status, claimToken: null as string | null, leaseUntil, attempts: 0, nextAttemptAt: new Date('2026-08-31T00:00:00.000Z') };
  const client = {
    $transaction: async (work: (tx: unknown) => Promise<unknown>) => work({
      outboxEvent: {
        ...
        updateMany: async ({ where, data }: { where: { claimToken?: string }; data: Record<string, unknown> }) => {
          ...
          Object.assign(row, data, data.attempts === undefined ? {} : { attempts: row.attempts + 1 });
          return { count: 1 };
        },
      },
    }),
...
describe('PrismaOutboxDispatcher', () => {
  it('claims, publishes and marks one event exactly once across replay', async () => {
    ...
    const publish = vi.fn(async (event: unknown) => { void event; });
    const dispatcher = new PrismaOutboxDispatcher(client as never, { publish }, () => 'claim', () => new Date('2026-08-31T00:00:00.000Z'));
    await dispatcher.dispatch('event-1');
    await dispatcher.dispatch('event-1');
    expect(publish).toHaveBeenCalledOnce();
    expect(publish).toHaveBeenCalledWith({ id: 'event-1', type: 'asset.imported.v1', payload: { assetId: 'asset-1' } });
  ...
  it('records a publish failure for durable retry and rethrows to the best-effort caller', async () => {
    const { row, client } = makeHarness('PENDING');
    const dispatcher = new PrismaOutboxDispatcher(client as never, { publish: vi.fn(async () => { throw new Error('broker offline'); }) }, () => 'claim', () => new Date('2026-08-31T00:00:00.000Z'));
    await expect(dispatcher.dispatch('event-1')).rejects.toThrow(/broker offline/);
    expect(row).toMatchObject({ status: 'FAILED', attempts: 1, lastError: 'EVENT_PUBLISH_FAILED', claimToken: null, leaseUntil: null });
  });
  ...
  it('backs off a broker failure and replays only when due', async () => {
    ...
    const dispatcher = new PrismaOutboxDispatcher(client as never, { publish }, () => 'claim', () => now);
