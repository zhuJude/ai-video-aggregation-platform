...
class MemoryStore implements Pick<ObjectStore, 'head' | 'delete'> {
  ...
  readonly readPrefix = vi.fn(() => Promise.resolve(Uint8Array.from([0, 0, 0, 24, 0x66, 0x74, 0x79, 0x70])));
}
...
class MemoryRepository implements ResultImportRepository {
  ...
  readonly failed: Array<{ idempotencyKey: string; claimToken: string; failedAt: Date; error: string }> = [];
  busy = false;
  ...
  reserveImport(input: { idempotencyKey: string }) {
    ...
    if (existing !== undefined || this.reserved.has(input.idempotencyKey)) {
      ...
      return Promise.resolve(this.busy
        ? { kind: 'busy' as const }
        : { kind: 'duplicate' as const, ...imported });
    }
    this.reserved.add(input.idempotencyKey);
    return Promise.resolve({ kind: 'claimed' as const, claimToken: 'claim-1' });
  }
  failAndScheduleCleanup(input: { idempotencyKey: string; claimToken: string; failedAt: Date; error: string; objectKey: string }) {
...
describe('result import', () => {
  ...
  it('publishes imported only after the destination is verified', async () => {
    ...
    expect(store.head.mock.invocationCallOrder[0]!).toBeLessThan(events.dispatch.mock.invocationCallOrder[0]!);
  });
  ...
  it.each([
    ...
    const { service } = makeService();
    await expect(service.import({ ...validInput, sourceUrl })).rejects.toBeInstanceOf(ResultImportError);
  });

  it('uses normalized exact host matching for IDN and trailing dots', async () => {
    const { service, transport } = makeService();
    await service.import({ ...validInput, sourceUrl: 'https://XN--BCHER-KVA.example./video.mp4', allowedHosts: ['bücher.example'] });
    expect(transport.copy).toHaveBeenCalledOnce();
  });

  it.each(['127.0.0.1', '169.254.169.254', '10.0.0.1', '::1', 'fe80::1', '::'])('rejects private DNS answer %s', async () => {
    const { service, transport } = makeService();
    transport.copy.mockRejectedValueOnce(new ResultImportError('RESULT_URL_PRIVATE_ADDRESS', 'blocked'));
    await expect(service.import(validInput)).rejects.toMatchObject({ code: 'RESULT_URL_PRIVATE_ADDRESS' });
  });

  it.each(['redirect-limit', 'redirect-disallowed', 'redirect-private'])('rejects unsafe redirects: %s', async (reason) => {
    const { service, transport } = makeService();
    transport.copy.mockRejectedValueOnce(new ResultImportError('RESULT_REDIRECT_NOT_ALLOWED', reason));
    await expect(service.import(validInput)).rejects.toMatchObject({ code: 'RESULT_REDIRECT_NOT_ALLOWED' });
  });
  ...
  it.each([
    ...
    await expect(service.import(validInput)).rejects.toMatchObject({ code: 'RESULT_INTEGRITY_MISMATCH' });
  ...
  it('scopes idempotency by verified provider and server authorization record', async () => {
    const { service, transport, repository } = makeService();
    const first = await service.import(validInput);
    const second = await service.import({ ...validInput, providerId: 'provider-b', authorizationId: '018f0a6a-8ac7-7d2c-8f4d-f234d61c5b72' });
  ...
  it('does not copy while another live reservation owns the import', async () => {
    ...
    repository.reserved.add(`${validInput.providerId}:${validInput.authorizationId}:${ownerId}:${validInput.providerTaskId}`);
    repository.busy = true;
    await expect(service.import(validInput)).rejects.toMatchObject({ code: 'RESULT_IMPORT_FAILED' });
  ...
  it('marks a failed reservation retryable and retains its private object for seven-day cleanup', async () => {
    ...
    await expect(service.import(validInput)).rejects.toMatchObject({ code: 'RESULT_IMPORT_FAILED' });
    expect(repository.failed).toEqual([expect.objectContaining({ idempotencyKey: `${validInput.providerId}:${validInput.authorizationId}:${ownerId}:${validInput.providerTaskId}`, claimToken: 'claim-1', error: 'RESULT_IMPORT_FAILED' })]);
    expect(repository.cleanups).toHaveLength(1);
  });
  ...
  it('attempts immediate object deletion if atomic failure persistence is unavailable', async () => {
    ...
    transport.copy.mockRejectedValueOnce(new Error('provider disconnected'));
    await expect(service.import(validInput)).rejects.toMatchObject({ code: 'RESULT_IMPORT_FAILED' });
    expect(store.delete).toHaveBeenCalledWith(`results/${ownerId}/018f0a6a-8ac7-7d2c-8f4d-f234d61c5b70`);
  });

  it('schedules the losing destination when persistence discovers a duplicate race', async () => {
    const { service, repository } = makeService();
    repository.persistImported.mockResolvedValueOnce({ kind: 'duplicate', assetId: 'winner', objectKey: 'results/winner', status: 'AVAILABLE' });
    await expect(service.import(validInput)).resolves.toMatchObject({ assetId: 'winner' });
    expect(repository.cleanups).toEqual([`results/${ownerId}/018f0a6a-8ac7-7d2c-8f4d-f234d61c5b70`]);
