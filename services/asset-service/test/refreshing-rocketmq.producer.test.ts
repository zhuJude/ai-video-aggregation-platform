...
describe('asset RocketMQ producer STS rotation', () => {
  it('single-flight refreshes before expiry and drains an in-flight send before retiring the old client', async () => {
    ...
    await Promise.all([
      producer.send({ body: Buffer.from('new-1') } as never),
      producer.send({ body: Buffer.from('new-2') } as never),
    ]);
  ...
  it('does not replace or extend an active client when STS returns the same near-expiry session', async () => {
    let now = new Date('2026-09-14T00:00:00.000Z');
    const active = client();
    ...
    expect(active.startup).toHaveBeenCalledOnce();
    now = new Date(expiresAt.getTime() + 1);
    await expect(producer.send({} as never)).rejects.toThrow('STS_CREDENTIALS_EXPIRED');
    await producer.shutdown();
  });

  it('cleans up a replacement whose startup fails without retiring the active client', async () => {
    let now = new Date('2026-09-14T00:00:00.000Z');
  ...
  it('preserves startup failure and retries replacement cleanup at shutdown', async () => {
    let now = new Date('2026-09-14T00:00:00.000Z');
    const clients = [client(), client()];
