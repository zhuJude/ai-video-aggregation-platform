...
function makeModule() {
  const resultImport = {
    import: vi.fn(async () => ({
      assetId,
      objectKey: `results/${ownerId}/${assetId}`,
  ...
  const lifecycle = new AssetLifecycleJob({
    repository: lifecycleRepository as never,
    objectStore: { delete: vi.fn() } as never,
  });
  const userAuthenticator = {
    authenticate: vi.fn(async (headers: Record<string, string | undefined>) =>
      headers.authorization === 'Bearer valid-user' ? { userId: ownerId } : null,
    ),
  };
  const providerCallbackAuthenticator = {
    authenticate: vi.fn(
      async (request: { headers: Record<string, string | string[] | undefined> }) =>
        request.headers['x-provider-signature'] === 'valid-signature'
          ? { providerId: 'provider-a' }
          : null,
    ),
  };
  const providerTaskAuthorization = {
    authorize: vi.fn(async (providerId: string, providerTaskId: string) =>
      providerId === 'provider-a' && providerTaskId === 'task-42'
        ? {
            authorizationId,
            ownerId,
            providerTaskId,
  ...
  const module = new AssetHttpModule({
    resultImport: resultImport as never,
    lifecycle: lifecycle as never,
    userAuthenticator,
    providerCallbackAuthenticator,
    providerTaskAuthorization,
  });
  return {
    module,
    resultImport,
...
describe('asset production HTTP module', () => {
  ...
  it.each([
    ...
    await expect(module.handle({ method, path, headers: {}, body: {} })).resolves.toEqual({
      status: 401,
  ...
  it('ignores a forged caller-supplied principal field', async () => {
    ...
    await expect(
      module.handle({
        ...
        headers: {},
        principal: { kind: 'user', subject: ownerId },
      } as never),
    ).resolves.toEqual({ status: 401, body: { code: 'UNAUTHENTICATED' } });
  ...
  it('rejects owner and metadata spoofing in a callback body', async () => {
    const { module, resultImport } = makeModule();
    await expect(
      module.handle({ ...validCallbackRequest(), body: { ...callbackBody, ownerId: 'spoofed' } }),
    ).resolves.toEqual({ status: 400, body: { code: 'INVALID_REQUEST' } });
  ...
  it('rejects a callback without a server-authorized provider task', async () => {
    ...
    await expect(module.handle(validCallbackRequest())).resolves.toEqual({
      status: 403,
      body: { code: 'FORBIDDEN' },
    });
  });
  ...
  it.each([
    ...
    'returns not found for cross-owner %s and derives owner only from the verified token',
    async (method, path, operation) => {
      ...
      await expect(
        module.handle({
          ...
          body: { ownerId: 'spoofed' },
        }),
      ).resolves.toEqual({ status: 404, body: { code: 'ASSET_NOT_FOUND' } });
      if (operation === 'requestDeletion') {
        expect(lifecycleRepository.requestDeletion).toHaveBeenCalledWith(
          ownerId,
          assetId,
          expect.any(Date),
        );
  ...
  it('preserves the real lifecycle boolean contract from owner guard to HTTP 202/404', async () => {
    ...
    const lifecycle = new AssetLifecycleJob({
      repository: {
        requestDeletion: vi.fn(
          async (requestedOwner: string) => owned && requestedOwner === ownerId,
        ),
      } as never,
      objectStore: { delete: vi.fn() } as never,
    });
    ...
    owned = false;
    await expect(module.handle(request)).resolves.toEqual({
      status: 404,
  ...
  it('maps import domain failures without exposing internal messages', async () => {
    const { module, resultImport } = makeModule();
    resultImport.import.mockRejectedValueOnce(
      new ResultImportError('RESULT_URL_PRIVATE_ADDRESS', 'sensitive address detail'),
    );
    await expect(module.handle(validCallbackRequest())).resolves.toEqual({
      status: 422,
      body: { code: 'RESULT_URL_PRIVATE_ADDRESS' },
    });
  });
  ...
  it('maps authentication adapter failures to a stable internal response', async () => {
    ...
    ).resolves.toEqual({ status: 500, body: { code: 'INTERNAL_ERROR' } });
  });
});
