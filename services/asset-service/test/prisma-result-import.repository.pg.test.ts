...
describe.skipIf(pgContainer === undefined)(
  'PrismaResultImportRepository PostgreSQL concurrency',
  () => {
    it('lets exactly one of two simultaneous insert-on-conflict reservations claim the key', async () => {
      const key =
        'provider-a:018f0a6a-8ac7-7d2c-8f4d-f234d61c5b7e:018f0a6a-8ac7-7d2c-8f4d-f234d61c5b7a:pg-race';
      await psql(`DELETE FROM "ResultImport" WHERE "idempotencyKey" = ${literal(key)}`);
      const makeRepository = (token: string) =>
        new PrismaResultImportRepository(pgClient(), () => token);
      const input = {
        idempotencyKey: key,
      ...
      const outcomes = await Promise.all([
        makeRepository('claim-a').reserveImport(input),
        makeRepository('claim-b').reserveImport(input),
      ]);
        ...
        resultImport: {
          ...
          findUnique: async ({ where }: { where: { idempotencyKey: string } }) => {
            const output = await psql(
              `SELECT "id" || '|' || "status" || '|' || COALESCE("claimToken", '') || '|' || COALESCE("leaseUntil"::text, '') FROM "ResultImport" WHERE "idempotencyKey" = ${literal(where.idempotencyKey)}`,
            );
            ...
            return {
              ...
              status,
              claimToken,
              leaseUntil: leaseUntil === '' ? null : new Date(leaseUntil ?? ''),
              asset: null,
