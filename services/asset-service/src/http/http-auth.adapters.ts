...
export class JwksUserAuthenticator implements UserAuthenticator {
  ...
  async authenticate(headers: RawHttpHeaders): Promise<{ userId: string } | null> {
    ...
    return identity !== null && UUID_PATTERN.test(identity.subject)
...
export class PrismaProviderNonceStore implements ProviderNonceStore {
  ...
  async claim(input: { providerId: string; nonce: string; expiresAt: Date }): Promise<boolean> {
    return this.client.$transaction(
      async (tx) =>
        (
          await tx.providerCallbackNonce.createMany({
            data: [input],
            skipDuplicates: true,
          })
        ).count === 1,
  ...
  async deleteExpired(now: Date, limit = 500): Promise<number> {
    return this.client.$transaction(async (tx) => {
      const rows = await tx.providerCallbackNonce.findMany({
        where: { expiresAt: { lte: now } },
        select: { providerId: true, nonce: true },
        take: limit,
      });
      if (rows.length === 0) return 0;
      const deleted = await tx.providerCallbackNonce.deleteMany({
        where: {
          OR: rows.map((row: { providerId: string; nonce: string }) => ({
...
export class ProviderNonceCleanupJob {
  constructor(
    private readonly store: Pick<PrismaProviderNonceStore, 'deleteExpired'>,
...
export class KmsProviderCallbackAuthenticator implements ProviderCallbackAuthenticator {
  ...
  constructor(input: {
    ...
    for (const provider of Object.values(input.providers)) {
      if (!isKmsReference(provider.kmsKeyReference))
        throw new Error('Provider callback key must be a KMS reference');
    }
  ...
  async authenticate(request: {
    ...
    if (
      ...
      signedAtMs < nowMs - this.#maxAgeMs ||
      signedAtMs > nowMs + MAX_CALLBACK_FUTURE_SKEW_MS
    )
      return null;
    const prefix = Buffer.from(`${providerId}\n${timestamp}\n${nonce}\n`, 'utf8');
    const message = Buffer.concat([prefix, Buffer.from(request.rawBody)]);
    const valid = await this.#macVerifier.verifyMac({
      kmsKeyReference: provider.kmsKeyReference,
      algorithm: 'HMAC_SHA_256',
      message,
      mac,
    });
    ...
    const claimed = await this.#nonceStore.claim({
      providerId,
      nonce,
      expiresAt: new Date(signedAtMs + this.#maxAgeMs),
    });
...
export class PrismaProviderTaskAuthorization implements ProviderTaskAuthorization {
  ...
  async authorize(providerId: string, providerTaskId: string) {
    return this.client.$transaction(async (tx) => {
      ...
      if (
        row === null ||
        !Array.isArray(row.allowedHosts) ||
        row.allowedHosts.length === 0 ||
        row.allowedHosts.some((host: unknown) => typeof host !== 'string' || host.length === 0)
      )
        return null;
      return {
        ...
        ...(row.expectedChecksum === null
          ? {}
          : { expectedChecksum: row.expectedChecksum as string }),
