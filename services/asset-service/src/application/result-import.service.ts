...
export interface ResultImportRepository {
  ...
  reserveImport(input: { idempotencyKey: string; ownerId: string; providerId: string; authorizationId: string; objectKey: string; now: Date }): Promise<
  ...
  }): Promise<{ kind: 'created'; eventId: string } | { kind: 'stale' } | ({ kind: 'duplicate' } & ImportedResult)>;
  failAndScheduleCleanup(input: { idempotencyKey: string; claimToken: string; failedAt: Date; error: ResultImportErrorCode; objectKey: string; scheduledAt: Date }): Promise<void>;
  scheduleCleanup(input: { objectKey: string; reason: 'IMPORT_FAILED' | 'DUPLICATE_RACE'; scheduledAt: Date }): Promise<void>;
}
...
export class ResultImportService {
  ...
  async import(input: ResultImportInput): Promise<ImportedResult> {
    ...
    const reservation = await this.#repository.reserveImport({ idempotencyKey, ownerId: input.ownerId, providerId: input.providerId, authorizationId: input.authorizationId, objectKey, now: this.#now() });
    ...
    try {
      ...
      if (!matchesResultSignature(input.expectedMimeType, prefix)) {
        throw new ResultImportError('RESULT_INTEGRITY_MISMATCH', 'Imported result signature does not match its MIME type');
      }
      ...
      try { await this.#events.dispatch(persistedResult.eventId); } catch { /* durable outbox retries */ }
      return result;
    } catch (error) {
      ...
      if (!persisted && !cleanupScheduled) {
        ...
        try {
          ...
          try { await this.#objectStore.delete(objectKey); } catch { /* recovered from the durable reservation */ }
        }
      }
      throw stableError;
    }
  ...
  async #scheduleCleanup(objectKey: string, reason: 'IMPORT_FAILED' | 'DUPLICATE_RACE', scheduledAt: Date): Promise<void> {
...
function assertExpectedMetadata(input: ResultImportInput): void {
  if (
    input.ownerId.length === 0 || input.providerId.length === 0 || input.authorizationId.length === 0 ||
...
function assertIntegrity(
  ...
  if (
    ...
    (expected.expectedChecksum !== undefined &&
      (copied.checksum !== expected.expectedChecksum || verified.checksum !== expected.expectedChecksum))
  ) {
    throw new ResultImportError('RESULT_INTEGRITY_MISMATCH', 'Imported result does not match expected metadata');
  }
}
...
function matchesResultSignature(mimeType: string, prefix: Uint8Array): boolean {
  ...
  if (mime === 'video/mp4') return prefix.length >= 8 && prefix[4] === 0x66 && prefix[5] === 0x74 && prefix[6] === 0x79 && prefix[7] === 0x70;
  if (mime === 'image/png') return prefix.length >= 8 && [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a].every((byte, index) => prefix[index] === byte);
