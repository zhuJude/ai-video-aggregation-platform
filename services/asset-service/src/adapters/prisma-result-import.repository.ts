...
export class PrismaResultImportRepository implements ResultImportRepository {
  ...
  async findImported(idempotencyKey: string): Promise<ImportedResult | null> {
    return this.client.$transaction(async (tx) =>
      toImported(
        await tx.resultImport.findUnique({ where: { idempotencyKey }, include: { asset: true } }),
      ),
    );
  }

  async reserveImport(input: {
  ...
  > {
    return this.client.$transaction(async (tx) => {
        ...
        data: [
          {
            ...
            reservedObjectKey: input.objectKey,
            status: 'RESERVED',
            claimToken,
            leaseUntil,
            attempts: 1,
      ...
      const row = await tx.resultImport.findUnique({
      ...
      if (row.status === 'RESERVED' && row.leaseUntil !== null && row.leaseUntil > input.now)
        return { kind: 'busy' as const };
      if (row.status !== 'FAILED' && row.status !== 'RESERVED') return { kind: 'busy' as const };
      if (
        row.reservedObjectKey !== null &&
        row.reservedObjectKey !== undefined &&
        row.reservedObjectKey !== input.objectKey
      ) {
        await tx.assetCleanup.upsert({
          where: { objectKey: row.reservedObjectKey },
          create: {
            id: randomUUID(),
            objectKey: row.reservedObjectKey,
            reason: 'STALE_IMPORT_RESERVATION',
      ...
      return changed.count === 1
        ? { kind: 'claimed' as const, claimToken }
        : { kind: 'busy' as const };
    });
  }

  async persistImported(
    input: Parameters<ResultImportRepository['persistImported']>[0],
  ): Promise<
  ...
  > {
    return this.client.$transaction(async (tx) => {
      const importRow = await tx.resultImport.findUnique({
        where: { idempotencyKey: input.idempotencyKey },
        include: { asset: true },
      });
      ...
      const changed = await tx.resultImport.updateMany({
        where: { id: importRow.id, status: 'RESERVED', claimToken: input.claimToken },
        data: {
      ...
      await tx.outboxEvent.create({
        data: {
          id: eventId,
          aggregateType: 'Asset',
          aggregateId: input.asset.assetId,
          eventType: 'asset.imported.v1',
          payload: { assetId: input.asset.assetId },
          status: 'PENDING',
        },
      });
  ...
  async failAndScheduleCleanup(
    input: Parameters<ResultImportRepository['failAndScheduleCleanup']>[0],
  ): Promise<void> {
  ...
  async scheduleCleanup(
    input: Parameters<ResultImportRepository['scheduleCleanup']>[0],
  ): Promise<void> {
    await this.client.$transaction((tx) =>
      tx.assetCleanup.upsert({
        where: { objectKey: input.objectKey },
        create: {
          id: randomUUID(),
          objectKey: input.objectKey,
