...
export class PrismaAssetLifecycleRepository implements AssetLifecycleRepository {
  ...
  async requestDeletion(ownerId: string, assetId: string, scheduledAt: Date): Promise<boolean> {
    return this.client.$transaction(async (tx) => {
      ...
      const changed = await tx.asset.updateMany({
        where: { id: assetId, ownerId, status: 'AVAILABLE' },
        data: { status: 'DELETED' },
      });
      if (changed.count === 1) {
  ...
  async claimDue(
    now: Date,
  ): Promise<Array<{ assetId: string; objectKey: string; claimToken: string }>> {
    return this.client.$transaction(async (tx) => {
      ...
      for (const row of due as Array<{ id: string; assetId: string; objectKey: string }>) {
        ...
        if (updated.count === 1)
          claims.push({ assetId: row.assetId, objectKey: row.objectKey, claimToken });
      }
      return claims;
    });
  }
    ...
    return this.client.$transaction(async (tx) => {
      const rows = await tx.resultImport.findMany({
        where: {
          ...
          OR: [
            { status: 'FAILED' },
            { status: 'RESERVED', OR: [{ leaseUntil: null }, { leaseUntil: { lte: now } }] },
          ],
        },
  ...
  async claimCleanupDue(
    ...
    return this.client.$transaction(async (tx) => {
      ...
      for (const row of due as Array<{ id: string; objectKey: string }>) {
        ...
        if (changed.count === 1)
          claims.push({ cleanupId: row.id, objectKey: row.objectKey, claimToken });
      }
      return claims;
    });
  }
      ...
      await tx.resultImport.updateMany({
        where: {
          ...
          OR: [
            ...
            {
              status: 'RESERVED',
              OR: [{ leaseUntil: null }, { leaseUntil: { lte: completedAt } }],
            },
          ],
        },
