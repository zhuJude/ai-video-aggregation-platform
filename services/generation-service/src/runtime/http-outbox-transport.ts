...
export class GenerationHttpOutboxTransport {
  private readonly notifications = new Subject<TransitionNotification>();
  ...
  readonly transitionNotifications = this.notifications.asObservable();
  ...
  private async publishBatch(): Promise<{ readonly published: number }> {
    ...
    for (const row of rows as OutboxRow[]) {
      ...
      published += 1;
      await this.emitLatestTransition(row);
    }
    return { published };
  }
  ...
  private async emitLatestTransition(row: OutboxRow): Promise<void> {
    if (row.aggregateType !== 'GenerationTask' || !UuidSchema.safeParse(row.aggregateId).success)
      return;
    const transition = await this.prisma.taskTransition.findFirst({
      where: { taskId: row.aggregateId },
      orderBy: [{ taskVersion: 'desc' }, { id: 'desc' }],
      select: {
        id: true,
        taskId: true,
        taskVersion: true,
