...
for (const target of [generatedSource, dist]) {
...
if (!pnpmCli) {
  throw new Error('PNPM_EXECUTABLE_UNAVAILABLE');
}
...
if (build.status !== 0) {
  throw new Error(`IDENTITY_BUILD_FAILED: ${String(build.status ?? 'unknown')}`);
}
...
log('Prisma clean build verification passed.');
