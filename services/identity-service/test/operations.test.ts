...
const environment = Object.freeze({
  ...
  smsChallengeKeyReference: key('sms'), privacyKeyReference: key('privacy'),
  previousPrivacyKeyReferences: [], jwtSigningKeyReference: key('jwt'),
  smsSignNameReference: key('sign'), smsTemplateCodeReference: key('template'),
  smsRoleReference: key('role'), readinessTimeoutMs: 10, readinessAbortGraceMs: 10,
...
describe('identity production operations', () => {
  ...
  it('strictly parses refs and forbids static access keys', () => {
    const env = {
      IDENTITY_DATABASE_URL: environment.databaseUrl, IDENTITY_REDIS_URL: environment.redisUrl,
      IDENTITY_SMS_CHALLENGE_KMS_KEY_REF: key('sms'), IDENTITY_PRIVACY_KMS_KEY_REF: key('privacy'),
      IDENTITY_JWT_SIGNING_KMS_KEY_REF: key('jwt'), IDENTITY_SMS_SIGN_NAME_KMS_REF: key('sign'),
      IDENTITY_SMS_TEMPLATE_KMS_REF: key('template'), IDENTITY_SMS_ROLE_KMS_REF: key('role'),
    ...
    expect(() => parseIdentityEnvironment({ ...env, ALIYUN_ACCESS_KEY_SECRET: 'secret' })).toThrow('STATIC_ACCESS_KEYS_FORBIDDEN');
    expect(() => parseIdentityEnvironment({ ...env, IDENTITY_JWT_SIGNING_KMS_KEY_REF: 'floating' })).toThrow('UNVERSIONED_KMS_REFERENCE');
  });
  ...
  it('bounds readiness, detects stalled liveness and never returns dependency details', async () => {
    ...
    expect(await health.readiness()).toEqual({ statusCode: 503, body: {
      status: 'not_ready', checks: { postgres: 'up', redis: 'down', kms: 'timeout' },
    } });
  ...
  it('single-flights 100 hanging probes without duplicating an uncooperative dependency', async () => {
    ...
    const HealthWithProtocol = ServiceHealth as unknown as new (...args: unknown[]) => ServiceHealth;
    ...
    }], { healthy: () => true }, 10, 10, 10, { adapterStuck });
  ...
  it('recovers on the next probe cycle after an abort-aware timeout and aborts on close', async () => {
    ...
    const closing = new ServiceHealth([{
      ...
      check: (signal) => new Promise<boolean>((resolve) => {
        if (signal.aborted) { aborted = true; resolve(false); return; }
  ...
  it('exposes liveness, degraded readiness, metrics and fail-closed SMS', async () => {
    ...
    const stderr = vi.spyOn(process.stderr, 'write').mockImplementation((chunk: string | Uint8Array) => {
      shutdownOutput += chunk.toString(); return true;
    });
    ...
    try {
      ...
      expect((await server.inject({ method: 'GET', url: '/metrics' })).body).toContain('identity_login_success_total');
      const sms = await server.inject({ method: 'POST', url: '/v1/auth/sms/request', payload: { phone: '13812345678', deviceId: 'device-1' } });
      expect(sms.statusCode).toBe(503);
      const login = await server.inject({ method: 'POST', url: '/v1/auth/sms/verify', payload: { phone: '13812345678', code: '123456', deviceName: 'browser' } });
  ...
  it('rolls back every allocated resource once when Nest construction fails', async () => {
    const closed: string[] = [];
    const lifecycle = new IdentityResourceLifecycle({ resourceClosed: (resource) => { closed.push(resource); } });
    const originalError = Object.assign(new Error('synthetic Nest construction failure'), { code: 'NEST_CREATE_FAILED' });
    ...
    try {
      await expect(createIdentityApplication({
        environment,
        lifecycle,
        cloud: {
          ...
          privacySecretProvider: { getPrivacyIdentifierSecret: () => Promise.resolve(new Uint8Array(32).fill(2)) },
          smsClientProvider: { getClient: () => Promise.resolve({ sendSms: () => Promise.resolve({ body: { code: 'OK' } }) }) },
      ...
      expect(closed).toEqual(['health', 'watchdog', 'readiness_redis', 'redis', 'readiness_database', 'database', 'cloud']);
  ...
  it('closes the partially created Nest app and every resource when init fails', async () => {
    const closed: string[] = [];
    const lifecycle = new IdentityResourceLifecycle({ resourceClosed: (resource) => { closed.push(resource); } });
    const originalError = Object.assign(new Error('synthetic Nest init failure'), { code: 'NEST_INIT_FAILED' });
    const partialApp = {
    ...
    } finally { create.mockRestore(); stderr.mockRestore(); }
  });
  ...
  it('continues ordered cleanup after a disposer failure and never invokes a disposer twice', async () => {
    ...
    for (const resource of ['health', 'watchdog', 'readiness_redis', 'redis', 'readiness_database', 'database', 'cloud'] as const) {
    ...
    try {
      ...
      await expect(lifecycle.close()).rejects.toThrow('RUNTIME_RESOURCE_CLOSE_FAILED');
      expect(attempts).toEqual(['health', 'watchdog', 'readiness_redis', 'redis', 'readiness_database', 'database', 'cloud']);
  ...
  it('supports an external cloud factory, real listener, and controlled graceful shutdown', async () => {
    ...
    try {
      ...
      await expect(bootstrap({
        environment: { ...environment, port: 0 },
        cloudFactory: async () => ({ ...await cloudFactory(), accessTokenIssuer: {} as never, close: creationFailureClose }),
        manageSignals: false,
  ...
  it.skipIf(!process.env['IDENTITY_TEST_DATABASE_URL'] || !process.env['IDENTITY_TEST_REDIS_URL'])(
    ...
    async () => {
      ...
      const cleanupPrisma = new PrismaClient({ adapter: new PrismaPg({ connectionString: databaseUrl }) });
      ...
      const { app } = await createIdentityApplication({
        ...
        cloud: {
          accessTokenIssuer: { issue: async () => { await delay(25); return 'test.jwt'; } },
          accessVerificationKeyProvider: { resolve: () => Promise.resolve(null) },
          challengeSecretProvider: { getSecret: () => Promise.resolve(new Uint8Array(32).fill(1)) },
          privacySecretProvider: { getPrivacyIdentifierSecret: () => Promise.resolve(new Uint8Array(32).fill(2)) },
          smsClientProvider: { getClient: () => Promise.resolve({ sendSms: async (request) => {
            await delay(25);
            const parsed: unknown = JSON.parse(request.templateParam);
            sentCode = typeof parsed === 'object' && parsed !== null && typeof Reflect.get(parsed, 'code') === 'string'
              ? String(Reflect.get(parsed, 'code')) : '';
            return { body: { code: 'OK' } };
          } }) },
          kmsConfigResolver: { resolveValue: (reference) => Promise.resolve(reference.includes('template') ? 'SMS_123' : 'approved-sign') },
      ...
      try {
        const response = await app.getHttpAdapter().getInstance().inject({ method: 'GET', url: '/readyz' });
        expect(response.statusCode).toBe(503);
        const readinessBody: unknown = response.json();
        const checks = typeof readinessBody === 'object' && readinessBody !== null ? Reflect.get(readinessBody, 'checks') as unknown : undefined;
        expect(typeof checks === 'object' && checks !== null ? Reflect.get(checks, 'kms') : undefined).toBe('timeout');
        const server = app.getHttpAdapter().getInstance();
        expect((await server.inject({ method: 'POST', url: '/v1/auth/sms/request', payload: { phone, deviceId } })).statusCode).toBe(201);
        expect(sentCode).toMatch(/^\d{6}$/);
        const login = await server.inject({ method: 'POST', url: '/v1/auth/sms/verify', payload: { phone, code: sentCode, deviceName: 'task6-browser' } });
        expect(login.statusCode).toBe(201);
        const cookie = login.headers['set-cookie'];
        expect(cookie).toContain('HttpOnly');
        const refresh = await server.inject({ method: 'POST', url: '/auth/refresh', headers: { cookie } });
        ...
        const [phoneHash, ipHash, deviceHash] = await Promise.all([
          privacy.hash('redis-phone', phoneE164), privacy.hash('redis-ip', '127.0.0.1'), privacy.hash('redis-device', deviceId),
        ]);
        await cleanupRedis.del(
          smsRateKey('cooldown', phoneHash), smsRateKey('phone', phoneHash),
          smsRateKey('ip', ipHash), smsRateKey('device', deviceHash),
        );
...
function activeHandles(): readonly unknown[] {
  const getActiveHandles = Reflect.get(process, '_getActiveHandles') as (() => readonly unknown[]) | undefined;
