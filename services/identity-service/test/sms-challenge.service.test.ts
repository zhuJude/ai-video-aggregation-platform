...
describe('SmsChallengeService', () => {
  it('counts only planned rate-limit and attempt-lock rejection codes', async () => {
    ...
    const metrics = {
      increment: (name: 'identity_sms_rate_limit_rejections_total') => {
    ...
    await expect(
      fixture.service.verify({ phoneE164: phone, code: '000000' }),
    ).rejects.toMatchObject({ code: 'SMS_CHALLENGE_LOCKED' });
    ...
    const infrastructure = makeFixture({
      ...
      store: {
        issue: () =>
          Promise.reject(
            Object.assign(new Error('REDIS_UNAVAILABLE'), { code: 'REDIS_UNAVAILABLE' }),
          ),
        verify: () =>
          Promise.reject(Object.assign(new Error('KMS_UNAVAILABLE'), { code: 'KMS_UNAVAILABLE' })),
        remove: () => Promise.resolve(),
      },
    });
    await expect(infrastructure.service.issue(input)).rejects.toThrow('REDIS_UNAVAILABLE');
    await expect(
      infrastructure.service.verify({ phoneE164: phone, code: '123456' }),
