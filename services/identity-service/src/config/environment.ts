...
export function parseIdentityEnvironment(env: NodeJS.ProcessEnv): IdentityEnvironment {
  ...
  const result: IdentityEnvironment = {
    ...
    databaseUrl: serviceUrl(required(env, 'IDENTITY_DATABASE_URL'), ['postgres:', 'postgresql:'], 'INVALID_IDENTITY_DATABASE_URL'),
    redisUrl: serviceUrl(required(env, 'IDENTITY_REDIS_URL'), ['redis:', 'rediss:'], 'INVALID_IDENTITY_REDIS_URL'),
    ...
    readinessTimeoutMs: integer(env['IDENTITY_READINESS_TIMEOUT_MS'] ?? '1000', 10, 10_000, 'INVALID_READINESS_TIMEOUT'),
    readinessAbortGraceMs: integer(env['IDENTITY_READINESS_ABORT_GRACE_MS'] ?? '500', 10, 10_000, 'INVALID_READINESS_ABORT_GRACE'),
    databaseOperationTimeoutMs: integer(env['IDENTITY_DATABASE_OPERATION_TIMEOUT_MS'] ?? '5000', 100, 60_000, 'INVALID_DATABASE_OPERATION_TIMEOUT'),
    redisOperationTimeoutMs: integer(env['IDENTITY_REDIS_OPERATION_TIMEOUT_MS'] ?? '5000', 100, 60_000, 'INVALID_REDIS_OPERATION_TIMEOUT'),
    eventLoopStallThresholdMs: integer(env['IDENTITY_EVENT_LOOP_STALL_MS'] ?? '250', 10, 10_000, 'INVALID_STALL_THRESHOLD'),
  };
  return Object.freeze(result);
}
function credentialKind(value: string): 'ecs_ram_role' | 'oidc_role_arn' {
  if (value !== 'ecs_ram_role' && value !== 'oidc_role_arn') throw stableError('INVALID_SMS_CREDENTIAL_KIND');
  return value;
}
...
function kms(value: string): string {
  assertVersionedKmsReference(value, 'UNVERSIONED_KMS_REFERENCE'); return value;
}
function references(value: string | undefined): readonly string[] {
  if (!value) return Object.freeze([]);
  const values = value.split(',').map((item) => kms(item.trim()));
  if (values.some((item) => !item) || new Set(values).size !== values.length) throw stableError('INVALID_KMS_KEYRING');
  return Object.freeze(values);
}
function serviceUrl(value: string, protocols: readonly string[], code: string): string {
  try { const url = new URL(value); if (!protocols.includes(url.protocol) || !url.hostname) throw new Error(); return value; }
  catch { throw stableError(code); }
}
function optionalHost(value: string | undefined): string {
  const host = value?.trim() || '0.0.0.0';
  if (!/^[A-Za-z0-9:.-]{1,255}$/.test(host)) throw stableError('INVALID_IDENTITY_HOST'); return host;
}
function integer(value: string, min: number, max: number, code: string): number {
  if (!/^\d+$/.test(value)) throw stableError(code); const parsed = Number(value);
  if (!Number.isSafeInteger(parsed) || parsed < min || parsed > max) throw stableError(code); return parsed;
}
function rejectAccessKeys(env: NodeJS.ProcessEnv): void {
  for (const name of ['ALIBABA_CLOUD_ACCESS_KEY_ID', 'ALIBABA_CLOUD_ACCESS_KEY_SECRET', 'ALIYUN_ACCESS_KEY_ID', 'ALIYUN_ACCESS_KEY_SECRET'])
