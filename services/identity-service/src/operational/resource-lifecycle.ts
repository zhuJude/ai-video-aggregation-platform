export type IdentityResourceName =
  'health' | 'watchdog' | 'readiness_redis' | 'redis' | 'readiness_database' | 'database' | 'cloud';
...
export class IdentityResourceLifecycle {
  ...
  has(name: IdentityResourceName): boolean {
  ...
  register(name: IdentityResourceName, close: () => void | Promise<void>): void {
    ...
    this.entries.set(name, {
      ...
      close: () => {
        result ??= boundedClose(close).finally(() => {
  ...
  private async closeAll(): Promise<void> {
    const failures: Array<{ readonly resource: IdentityResourceName; readonly reason: unknown }> =
      [];
    const phases = [...new Set([...this.entries.values()].map(({ phase }) => phase))].sort(
    ...
    for (const phase of phases) {
      ...
      results.forEach((result, index) => {
        if (result.status === 'rejected')
          failures.push({
            resource: entries[index]?.name ?? 'cloud',
    ...
    if (failures.length > 0) {
      process.stderr.write(
        `${JSON.stringify({ level: 'error', event: 'runtime_shutdown_failed', code: 'RUNTIME_RESOURCE_CLOSE_FAILED', resources: failures.map(({ resource }) => resource) })}\n`,
      );
  ...
  private notifyClosed(name: IdentityResourceName): void {
    try {
      this.observer.resourceClosed?.(name);
    } catch {
      process.emitWarning('RESOURCE_LIFECYCLE_OBSERVER_FAILED', {
        code: 'RESOURCE_LIFECYCLE_OBSERVER_FAILED',
      });
    }
  }
}
...
async function boundedClose(close: () => void | Promise<void>, timeoutMs = 5_000): Promise<void> {
  ...
  const timeout = new Promise<never>((_resolve, reject) => {
    timer = setTimeout(() => {
      reject(stableError('RESOURCE_CLOSE_TIMEOUT'));
    }, timeoutMs);
    timer.unref();
  });
  try {
    await Promise.race([Promise.resolve().then(close), timeout]);
  } finally {
