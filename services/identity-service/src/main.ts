...
export interface IdentityBootstrapOptions {
  ...
  readonly cloudFactory?: (environment: IdentityEnvironment) => Promise<IdentityCloudInfrastructure>;
  readonly manageSignals?: boolean;
}

export async function bootstrap(options: IdentityBootstrapOptions = {}): Promise<NestFastifyApplication> {
  ...
  try {
    ...
    const created = await createIdentityApplication({ environment, lifecycle, ...(cloud ? { cloud } : {}) });
    app = created.app;
    await app.listen(environment.port, environment.host);
    if (options.manageSignals !== false) installSignalHandlers(app);
    process.stdout.write(`${JSON.stringify({ level: 'info', event: 'identity_service_started' })}\n`);
    return app;
  } catch (error) {
    if (app) {
      try { await app.close(); }
...
export async function gracefulShutdown(app: NestFastifyApplication, signal: 'SIGTERM' | 'SIGINT'): Promise<void> {
  process.stdout.write(`${JSON.stringify({ level: 'info', event: 'identity_service_stopping', signal })}\n`);
  await app.close();
}
...
if (isEntrypoint()) {
  bootstrap().catch((error: unknown) => {
    ...
    process.stderr.write(`${JSON.stringify({ level: 'fatal', event: 'identity_service_start_failed', code })}\n`);
    process.exitCode = 1;
  });
}
