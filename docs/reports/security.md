...
- `tests/e2e/admin-operations.spec.ts`：PASS。以临时 HTTPS fixture 验证 MFA、RBAC、财务控制与双人审批，secure cookie 未被降级绕过。
...
浏览器级 IDOR、CSRF/CORS/CSP、SSRF、upload magic/size、DOM/log credential redaction、JWT audience、refresh reuse 与限流的完整跨服务场景仍没有全部通过证明。任何缺项均按阻断处理，不以风险接受或忽略替代。
