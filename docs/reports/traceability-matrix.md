...
| IAM MFA、RBAC、数据范围、审计           | IAM/Admin Web           | IAM 组件与隔离式 Admin MFA/RBAC E2E PASS            | IAM runbook/alerts 已合并        | PASS（隔离 E2E）  |
| Catalog 模型与能力版本                  | Catalog/Admin Web       | workspace test/build；隔离式 Admin UI E2E PASS      | Catalog runbook 已合并           | PARTIAL           |
| Routing 报价、margin、模拟与 failover   | Routing/Admin Web       | workspace test/build；隔离式 Admin UI E2E PASS      | Routing alerts/runbook 已合并    | PARTIAL           |
...
| Operations 工单、CMS、任务修复          | Operations/Admin Web    | workspace suite 与隔离式 Admin 浏览器 E2E PASS      | operations runbook 已合并        | PASS（隔离 E2E）  |
...
| Admin Web 财务/RBAC/MFA                 | Admin Web               | HTTPS 浏览器套件 1/1 PASS；完整 Compose 尚未运行    | admin runbook 已合并             | PARTIAL           |
