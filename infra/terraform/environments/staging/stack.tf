...
module "security" {
  ...
  oss_bucket_name        = "ai-video-staging-assets"
  rocketmq_instance_id   = "rmq-ai-video-staging"
...
module "storage" {
  ...
  bucket_name                    = "ai-video-staging-assets"
  kms_key_id                     = module.security.kms_key_id
  replication_destination_bucket = "ai-video-staging-critical-dr"
  replication_destination_region = "oss-cn-shanghai"
  replication_role_arn           = "acs:ram::${var.account_id}:role/oss-replication-staging"
...
module "edge" {
  ...
  hosts = {
    user  = "staging.example.cn"
    admin = "admin-staging.example.cn"
    api   = "api-staging.example.cn"
  }
  tags = local.common_tags
}
